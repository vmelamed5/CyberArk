...
.DESCRIPTION
   HELPER FUNCTION TO RETRIEVE AGENT IDS FROM EPM
#>
function Get-VPASEPMAgentIDHelper{
    [OutputType([String])]
    [CmdletBinding()]
    Param(
    ...
    Process{
        try{
            ...
            $apiLimit = 5000
            $uri = "$ManagerURL/EPM/API/Sets/$SetID/Computers?`$filter=contains(ComputerName, '$SearchQuery')&limit=$apiLimit"
            ...
            $curPolicies = $response.Computers
            $curOffset = 0
            while($curcount -gt 0){
                $curOffset += $apiLimit
                $uri = "$ManagerURL/EPM/API/Sets/$SetID/Computers?`$filter=contains(ComputerName, '$SearchQuery')&limit=$apiLimit&offset=$curOffset"
                Write-Verbose "SETTING URI: $uri"
            ...
            $response.Computers = $curPolicies

            $counter = 0
            $lastID = 0
            foreach($rec in $response.Computers){
                $recPolicyName = $rec.ComputerName
                $recPolicyID = $rec.AgentID
            ...
            if($counter -gt 1){
                Write-Verbose "MULTIPLE AGENTS ENTRIES WERE RETURNED, ADD MORE TO SEARCH TO NARROW RESULTS"
                $log = Write-VPASEPMTextRecorder -inputval "MULTIPLE AGENTS ENTRIES WERE RETURNED, ADD MORE TO SEARCH TO NARROW RESULTS" -token $token -LogType MISC -Helper
            ...
            elseif($counter -eq 0){
                ...
                Write-VPASEPMOutput -str "NO AGENTS FOUND" -type E
                $log = Write-VPASEPMTextRecorder -inputval "NO AGENTS FOUND" -token $token -LogType MISC -Helper
                $log = Write-VPASEPMTextRecorder -inputval "REST API COMMAND RETURNED: -2" -token $token -LogType MISC -Helper
                return -2
            }
            else{
                write-verbose "FOUND UNIQUE AGENT ID"
                Write-Verbose "RETURNING UNIQUE AGENT ID"
