...
.DESCRIPTION
   HELPER FUNCTION TO RETRIEVE POLICY IDS FROM EPM
#>
function Get-VPASEPMPolicyIDHelper{
    [OutputType([String])]
    [CmdletBinding()]
    Param(
    ...
    Process{
        try{
            ...
            $apiLimit = 1000
            $uri = "$ManagerURL/EPM/API/Sets/$SetID/Policies/Server/Search?limit=$apiLimit"
            ...
            while($curcount -gt 0){
                $curOffset += $apiLimit
                $uri = "$ManagerURL/EPM/API/Sets/$SetID/Policies/Server/Search?limit=$apiLimit&offset=$curOffset"
            ...
            if($counter -gt 1){
                ...
                $log = Write-VPASEPMTextRecorder -inputval "MULTIPLE POLICY ENTRIES WERE RETURNED, ADD MORE TO SEARCH TO NARROW RESULTS" -token $token -LogType MISC -Helper
                $log = Write-VPASEPMTextRecorder -inputval "REST API COMMAND RETURNED: -1" -token $token -LogType MISC -Helper
                return -1
            }
            elseif($counter -eq 0){
                Write-Verbose "NO POLICIES FOUND"
                Write-VPASEPMOutput -str "NO POLICIES FOUND" -type E
                $log = Write-VPASEPMTextRecorder -inputval "NO POLICIES FOUND" -token $token -LogType MISC -Helper
                $log = Write-VPASEPMTextRecorder -inputval "REST API COMMAND RETURNED: -2" -token $token -LogType MISC -Helper
                return -2
            }
            else{
                write-verbose "FOUND UNIQUE POLICY ID"
                Write-Verbose "RETURNING UNIQUE POLICY ID"
