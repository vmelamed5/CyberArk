...
function VRunAuditSafeTest{
    ...
    try{
        if(Test-Path -Path $ConfigFilePath){
            ...
            write-host "AuditSafeTestConfigs DIRECTORY DOES NOT EXIST...PLEASE RUN VSetAuditSafeTest COMMAND TO INITIATE TEST CASES" -ForegroundColor Red
        ...
        if(Test-Path -Path $ConfigFile){
            ...
            Write-Verbose "Returning False"
            write-host "AuditSafeTestConfigs.txt DOES NOT EXIST...PLEASE RUN VSetAuditSafeTest COMMAND TO INITIATE TEST CASES" -ForegroundColor Red
            write-host "EXITING UTILITY" -ForegroundColor Red
            if($OutputResultsToFile){
                $timestamp = Get-Date -Format "(MM-dd-yyyy HH:mm:ss)"
                write-output "$timestamp : FAILED TO RUN SAFE AUDIT TEST" | Add-Content $OutputFile
        ...
        Write-Host "ERROR READING AuditSafeTestConfigs FILE" -ForegroundColor Red
    ...
    if($NoSSL){
        $AllSafes = VGetSafes -PVWA $PVWA -token $token -searchQuery "$AuditSafeNameConvention" -limit 5000 -NoSSL
    }
    else{
        $AllSafes = VGetSafes -PVWA $PVWA -token $token -searchQuery "$AuditSafeNameConvention" -limit 5000
    }
    ...
    if($AllSafes){
        ...
        foreach($saferes in $AllSafes.value){ 
            ...
            if($AuditIgnoreSafes.Contains($safe)){
                ...
                if($AuditNumberOfSafeMembers -eq "0"){
                    ...
                    if($NoSSL){
                        $AllSafemembers = VGetSafeMembers -PVWA $PVWA -token $token -safe $safe -IncludePredefinedMembers -NoSSL
                    }
                    else{
                        $AllSafemembers = VGetSafeMembers -PVWA $PVWA -token $token -safe $safe -IncludePredefinedMembers
                    }
        ...
        Write-Verbose "Returning False"
        write-host "FAILED TO QUERY CYBERARK FOR SAFES" -ForegroundColor Red
        write-host "EXITING UTILITY" -ForegroundColor Red
        if($OutputResultsToFile){
            $timestamp = Get-Date -Format "(MM-dd-yyyy HH:mm:ss)"
            write-output "$timestamp : FAILED TO RUN SAFE AUDIT TEST" | Add-Content $OutputFile
    ...
    if($ErrorInAudit){
        ...
        write-host "AuditSafeTest RAN SUCCESSFULLY, BUT SOME ERRORS WERE DISCOVERED" -ForegroundColor Magenta
        write-host "VIEW AUDIT LOG LOCATED HERE TO VIEW MORE DETAILS: $OutputFile" -ForegroundColor Magenta
        return $false
    }
    else{
        Write-Verbose "ALL AUDIT CHECKS PASSED...RETURNING TRUE"
        Write-Host "ALL AUDIT CHECKS PASSED!!!" -ForegroundColor Green
        return $true
    }
}
