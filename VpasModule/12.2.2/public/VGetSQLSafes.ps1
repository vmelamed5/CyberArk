...
function VGetSQLSafes{
    ...
    if($NoSSL){
        $AllSafes = VGetSafes -PVWA $PVWA -token $token -searchQuery $SearchQuery -limit $EstimatedSafeCount -NoSSL
    }
    else{
        $AllSafes = VGetSafes -PVWA $PVWA -token $token -searchQuery $SearchQuery -limit $EstimatedSafeCount
    }
    ...
    foreach($safe in $AllSafes.value){
        ...
        if($NoSSL){
            $AllMembers = VGetSafeMembers -PVWA $PVWA -token $token -safe $SafeName -IncludePredefinedMembers -NoSSL
        }
        else{
            $AllMembers = VGetSafeMembers -PVWA $PVWA -token $token -safe $SafeName -IncludePredefinedMembers
        }
