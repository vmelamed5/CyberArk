...
function VDuplicatePlatform{
    ...
    try{
        ...
        if($NoSSL){
            ...
            $platID = VGetPlatformIDHelper -PVWA $PVWA -token $token -platformID $DuplicateFromPlatformID -NoSSL
        }
        else{
            Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
            $platID = VGetPlatformIDHelper -PVWA $PVWA -token $token -platformID $DuplicateFromPlatformID
        }
