<#
.Synopsis
   Delete MFA cache SSH key
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to delete an MFACache SSH key
.LINK
   https://vpasmodule.com/commands.html#Remove-VPASMFACacheSSHKey
.NOTES
   Category: PAM
   Tag: MFA Cache
   Since: v14.6.0
...
.PARAMETER CurrentUser
   Delete SSHKey from the current user authenticated via the APIs
.PARAMETER AllUsers
   Delete ALL MFA cache SSHKeys from ALL users
.PARAMETER TargetUsername
   Username of the target user that the SSHKey will be deleted for if no UserID is passed
.PARAMETER TargetUserID
   Unique ID that maps to a single user that the SSHKey will be deleted for, passing this variable will skip any query functions
.PARAMETER InputParameters
   HashTable of values containing the parameters required to make the API call
.EXAMPLE
   $RemoveMFACacheSSHKey = Remove-VPASMFACacheSSHKey -CurrentUser
.EXAMPLE
   $RemoveMFACacheSSHKey = Remove-VPASMFACacheSSHKey -TargetUsername {TARGET USERNAME VALUE}
.EXAMPLE
   $RemoveMFACacheSSHKey = Remove-VPASMFACacheSSHKey -TargetUserID {TARGET USERID VALUE}
.EXAMPLE
   $RemoveMFACacheSSHKey = Remove-VPASMFACacheSSHKey -AllUsers
.EXAMPLE
   $InputParameters = @{
        CurrentUser = $true|$false
   }
   $RemoveMFACacheSSHKey = Remove-VPASMFACacheSSHKey -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        TargetUsername = "vman"
   }
...
.OUTPUTS
   ...
   $false if failed
#>
function Remove-VPASMFACacheSSHKey{
    [OutputType('System.Object',[bool])]
    [CmdletBinding(DefaultParameterSetName='Set1')]
    Param(

        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Remove SSHKey for current user")]
        [Switch]$CurrentUser,

        [Parameter(Mandatory=$true,ParameterSetName='Set2',ValueFromPipelineByPropertyName=$true,HelpMessage="Remove SSHKey for a target user via Username (for example: Vman)")]
        [String]$TargetUsername,

        [Parameter(Mandatory=$true,ParameterSetName='Set3',ValueFromPipelineByPropertyName=$true,HelpMessage="Remove SSHKey for a target user via UserID (for example: 55)")]
        [String]$TargetUserID,

        [Parameter(Mandatory=$true,ParameterSetName='Set4',ValueFromPipelineByPropertyName=$true,HelpMessage="Remove ALL MFA cache SSHKeys for ALL users")]
        [Switch]$AllUsers,
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("CurrentUser")
                        MandatoryKeys = @("CurrentUser")
                    }
					set2 = @{
                        AcceptableKeys = @("TargetUsername")
                        MandatoryKeys = @("TargetUsername")
                    }
                    set3 = @{
                        AcceptableKeys = @("TargetUserID")
                        MandatoryKeys = @("TargetUserID")
                    }
                    set4 = @{
                        AcceptableKeys = @("AllUsers")
                        MandatoryKeys = @("AllUsers")
                    }
                }
            ...
            Write-Verbose "FAILED TO REMOVE MFA CACHE SSH KEY"
        ...
        try{
            ...
            elseif($AllUsers){
                if($NoSSL){
                    Write-Verbose "NO SSL ENABLED, USING HTTP INSTEAD OF HTTPS"
                    $uri = "http://$PVWA/PasswordVault/API/Users/Secret/SSHKeys/ClearCache/"
                }
                else{
                    Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                    $uri = "https://$PVWA/PasswordVault/API/Users/Secret/SSHKeys/ClearCache/"
                }
                ...
                if($sessionval){
                    $response = Invoke-RestMethod -Headers @{"Authorization"=$Header} -Uri $uri -Method DELETE -Body $params -ContentType "application/json" -WebSession $sessionval
                    $APICounter += 1
                }
                else{
                    $response = Invoke-RestMethod -Headers @{"Authorization"=$Header} -Uri $uri -Method DELETE -Body $params -ContentType "application/json"
                    $APICounter += 1
                }
            ...
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-Verbose "UNABLE TO REMOVE MFA CACHE SSH KEY"
            Write-VPASOutput -str $_ -type E
            return $false
        }
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUiHdYgaGjG5KnuK1rF+w+jw2n
# n82ggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFKSf6sL/gfHziKSXoWDrxu/cjxRa
# MA0GCSqGSIb3DQEBAQUABIICAB7NvPrKVUkNEB/YiHQ3dJt8/tMZbEvBGWBTED8e
# fmNw1QlKVKm6KqTY3lPJjxv8R9dTXHWmPzVJ0GCtL3S29ppjnNMPJYkEJdMCb6fp
# AcLAtMNCUj3Ac8iF2jZb5L33mITnTDnW2OeWeXB+XwnTSUREWTKAoDpAQzaBdJ04
# g9AoYvqI2CZ8nTQSlBpChc+XIUHFWD/MJ+VqGxcbb47q25IZ/E8tx8PS+SJiesNJ
# kmGjRilKGJNvU6nQK4AfZmGLe44MjW6Q22QHWMkZFh+PbS0qt9E5KrGRFClf3p7X
# 9EJ8ioYj0rIp2ZLNKt3B0LP3voy7QX+mC2PP5+y6wghHVRcHlqoaJu5a3vk+AOry
# 9+LJb+4FYq499Nl+RqpWFVCM+jQWWUvM4IiRXrXB4DNa8DPQ7NJAsJfzbbDIKr1I
# mTERyP9jinf7Yjpoe5i1syUCx0uftDFGatfAxlEfpA5xKikyBh44DCalcIMjjBxI
# ej4rOd6tue7zMsoNjGHmJ0KcSpwgR8GQtKCCAvTTTRAV3kf31bhZB75m7Wq1MYiI
# oeaYWWFGMrNhwF1vHGZBTzrFfZqciBBy2+chImJptv9G/ArZvnH78CRS0Gf9KIiV
# SlFdF+ax8WVI8c1Exmrr7zwY917tPXWPcBEYy31OoSz6cce+ZnCvt+ysqyPDnqZz
# diJzoYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkGA1UEBhMC
...
# 9w0BCQUxDxcNMjYwMjA0MDQ1MzM5WjA/BgkqhkiG9w0BCQQxMgQwhGayLgFNnfc0
# LuGm2jRpu3DIHnKutxAKXIw65zzRlDKQGAa4IFy6E1Srh2/Vs0PDMA0GCSqGSIb3
# DQEBAQUABIICAFxsAxdwpTRxkRolvJ0gtvlNaXi5mk6wcLIOMO7aHWTI6k0dPDxR
# OeZ15zovUSHaU879sQRr7/cOIo21UdDoW2gfWqfnGUw1OnVpKxl3e5QkRWbz6uON
# Kk/rinYQQGKzbuPMtnim+nY/iIfw9ewLO7rtcv64XcuPWtyG7NT2vlUa1i2cx3JT
# UsxG3P+baF5T6sJchyEqnZXHGHoznChSRFIJvFEf6bEYcU0wtEfG+CptUqk9buJ+
# UYX4ivwM1z+MCMFdkOKMypyormBPYD5Ok+uykXFoVaDVmt21f6veAFT5XMK99bg5
# THLHMBOEzEVFd8I4FOvUBf8vuyWrjJorEkIcOMjd3wFpzYnucKnUVqXnR5sVbyoV
# ulb8hwFXJLDe5TuUd08p40E3uvlY8PwRASHmla0reWK2UCBwzRUkoxgYIMrmqkdq
# Im6OJd7+BVF4kDdmThe8ht+wMbzBPXRMD3Lgpga1Bib3Xsod1Nw9kx/rrCOzaCZ4
# huRTrVeaboBtcBdYeE0hamIHkma+JdFDZxRgp2Igyp/csSOlJqcUcOeMPHLiO4sX
# RvoPmXn9PkWzzc7hglVX4Qb7LZtVg+nmwRekIgk19KnOW46lBn8oR2i9wAE5vmm5
# YRW15sPXeRRNpKIBaKYINEsIbUoftMFbhOEEfuTUtdxkNxwkHxZa60qr
