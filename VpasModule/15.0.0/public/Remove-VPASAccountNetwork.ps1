...
.DESCRIPTION
   Use this function to remove an account network from an account
.LINK
   https://vpasmodule.com/commands.html#Remove-VPASAccountNetwork
.NOTES
   Category: PAM
   Tag: Accounts
   Since: v15.0.0
...
.EXAMPLE
   $DeleteAccountNetworkStatus = Remove-VPASAccountNetwork -safe {SAFE VALUE} -username {USERNAME VALUE} -platform {PLATFORM VALUE} -address {ADDRESS VALUE}
.EXAMPLE
   $DeleteAccountNetworkStatus = Remove-VPASAccountNetwork -AcctID {ACCTID VALUE}
.EXAMPLE
   $InputParameters = @{
        safe = "TargetSafe"
   ...
   $DeleteAccountNetworkStatus = Remove-VPASAccountNetwork -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        AcctID = "22_123"
        WhatIf = $true|$false
...
.OUTPUTS
   ...
   $false if failed
#>
function Remove-VPASAccountNetwork{
    ...
    Process{
        try{
            ...
            Write-Verbose "FAILED TO REMOVE ACCOUNT NETWORK"
        ...
        try{
            ...
            elseif($AcctID -eq -2){
                ...
                if($NoSSL){
                    ...
                    $uri = "http://$PVWA/api/accounts/$AcctID/network"
                }
                else{
                    Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                    $uri = "https://$PVWA/api/accounts/$AcctID/network"
                }
                ...
                if($WhatIf){
                    ...
                    $WhatIfInfo = Get-VPASAccountNetwork -AcctID $AcctID -token $token
                    $WhatIfAccountID = $WhatIfInfo.accountId
                    $WhatIfNetworkID = $WhatIfInfo.networkId
                    $WhatIfNetworkName = $WhatIfInfo.networkName

                    if(!$HideWhatIfOutput){
                        Write-VPASOutput -str "====== BEGIN COMMAND SIMULATION ======" -type S
                        Write-VPASOutput -str "THE FOLLOWING NETWORK WOULD BE REMOVED FROM ACCOUNT:" -type S
                        Write-VPASOutput -str "AccountID       : $WhatIfAccountID" -type S
                        Write-VPASOutput -str "NetworkID       : $WhatIfNetworkID" -type S
                        Write-VPASOutput -str "Networkname     : $WhatIfNetworkName" -type S
                    ...
                    $WhatIfHash = @{
                        WhatIf = @{
                            ...
                            NetworkID = $WhatIfNetworkID
                            NetworkName = $WhatIfNetworkName
                            RestURI = $uri
                            RestMethod = "DELETE"
                            Disclaimer = "THIS NETWORK WILL BE REMOVED FROM ACCOUNT IF -WhatIf FLAG IS REMOVED"
                        }
                    }
                    ...
                    Write-Verbose "SUCCESSFULLY REMOVED NETWORK FROM ACCOUNT: $AcctID"
            ...
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-Verbose "FAILED TO REMOVE NETWORK FROM ACCOUNT"
            Write-VPASOutput -str $_ -type E
            return $false
        }
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUQe51KTh8Nx9ga2pUeHkKenDe
# SIyggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFMIzvKAdIuP60McbbAlKrv75YI1w
# MA0GCSqGSIb3DQEBAQUABIICADxSTCGKTrog8lMDZzSwTBEyh2w3YKGU4L0GWAqr
# zp2UtHZXLskHjpOdbMdrYWmgxaqp0M7ysbzTJ7z9Oy0QFPPMOBOeGStzu+n6jWGj
# 6LGqhFsafuM6LyJZnlcRTzetIk2cabDUOJoiQuvK7v04NS9DrOAH4cfm/kFF543s
