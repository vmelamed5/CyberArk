...
.DESCRIPTION
   Use this function to run a health check that performs various checks (account compliance, component status, empty safes, inactive platforms, inactive users, etc.)
.LINK
   https://vpasmodule.com/commands.html#Invoke-VPASHealthCheck
.NOTES
   Category: PAM
   Tag: Metrics
   Since: v14.4.1
...
function Invoke-VPASHealthCheck{
    ...
    Process{
        ...
        try{
            ...
            $AllCPMs = Get-VPASSystemHealth -Component CPM -token $token
            $AllPSMs = Get-VPASSystemHealth -Component PSM -token $token
            $AllPVWAs = Get-VPASSystemHealth -Component PVWA -token $token
            $AllAIMs = Get-VPASSystemHealth -Component AIM -token $token
            $VaultVersion = (Get-VPASVaultVersion -token $token).ExternalVersion
            ...
            foreach($rec in $AllAIMs.ComponentsDetails){
                ...
                $epvgroups = (Get-VPASEPVUserDetails -EPVUsername $ComponentName -token $token).groupsMembership
            ...
            if($SystemComponents.AIMPROV.UniqueNames.Count -gt 0){
                ...
                foreach($app in $AllApplications){
                    ...
                    if($appname -eq "AIMWebService"){
                        ...
                        $epvgroups = (Get-VPASEPVUserDetails -EPVUsername $appname -token $token).groupsMembership
                        foreach($grouprec in $epvgroups){
                            $group = $grouprec.groupName
                        ...
                        $ApplicationAuthmethods = Get-VPASApplicationAuthentications -AppID $appname -token $token
            ...
            foreach($safe in $AllSafes.value){
                ...
                if(!$SkipSafes.Contains($safename)){
                    ...
                    #CHECK SAFE MEMBERS
                    $AllSafeMembers = Get-VPASSafeMembers -safe $safename -token $token
            ...
            $AllPSMServerIDs = Get-VPASAllPSMServers -token $token
            ...
            $AllConnectors = Get-VPASAllConnectionComponents -token $token
            ...
            $AllPlatforms = Get-VPASAllTargetPlatforms -token $token
            foreach($plat in $AllPlatforms.Platforms){
                ...
                if(![String]::IsNullOrEmpty($platID)){
                    ...
                    $platdetails = Get-VPASPSMSettingsByPlatformID -PlatformID $platID -token $token
            ...
            foreach($safe in $SystemComponents.SAFE.Keys){
                ...
                if($safe -ne "Uniquenames"){
                    foreach($AcctID in $SystemComponents.SAFE.$safe.Accounts.Keys){
                        $AcctDetails = Get-VPASAccountDetailsExtended -AcctID $AcctID -token $token
            ...
            $ignoreusers = @()
            $AllUsers = Get-VPASAllEPVUsers -token $token
            foreach($user in $AllUsers.Users){
                ...
                if($usertype -eq "EPVUser"){
                    $userdetails = Get-VPASEPVUserDetails -EPVUserID $userid -token $token
                    $lastlogin = $userdetails.lastSuccessfulLoginDate

                    if($lastlogin -le $cutTime){
                        $outputmatrix.InactiveUserCheck += @{
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUPp/0mCX2KyHZBbR6chb+1UFS
# Tk2ggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYE