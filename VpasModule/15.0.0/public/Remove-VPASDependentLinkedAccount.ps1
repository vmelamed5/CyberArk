<#
.Synopsis
   Unlink dependent account
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to remove a linked account from a dependent account (ReconcileAccount, LogonAccount, JumpAccount)
.LINK
   https://vpasmodule.com/commands.html#Remove-VPASDependentLinkedAccount
.NOTES
   Category: PAM
...
.PARAMETER DependentSearchQuery
   Search query to find the target dependent account
...
.EXAMPLE
   $UnlinkAcctActionStatus = Remove-VPASDependentLinkedAccount -AccountType {ACCOUNTTYPE VALUE} -safe {SAFE VALUE} -platform {PLATFORM VALUE} -address {ADDRESS VALUE} -username {USERNAME VALUE} -DependentSearchQuery {DEPENDENT SEARCHQUERY VALUE}
.EXAMPLE
   $UnlinkAcctActionStatus = Remove-VPASDependentLinkedAccount -AccountType {ACCOUNTTYPE VALUE} -AcctID {ACCTID VALUE} -DependentSearchQuery {DEPENDENT SEARCHQUERY VALUE}
.EXAMPLE
   $UnlinkAcctActionStatus = Remove-VPASDependentLinkedAccount -AccountType {ACCOUNTTYPE VALUE} -safe {SAFE VALUE} -platform {PLATFORM VALUE} -address {ADDRESS VALUE} -username {USERNAME VALUE} -DependentAcctID {DEPENDENT ACCTID VALUE}
.EXAMPLE
   $UnlinkAcctActionStatus = Remove-VPASDependentLinkedAccount -AccountType {ACCOUNTTYPE VALUE} -AcctID {ACCTID VALUE} -DependentAcctID {DEPENDENT ACCTID VALUE}
.EXAMPLE
   $InputParameters = @{
        ...
        DependentSearchQuery = "targetSearchQuery"
        WhatIf = $true|$false
        HideWhatIfOutput = $true|$false
   }
   $UnlinkAcctActionStatus = Remove-VPASDependentLinkedAccount -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        AccountType = "LogonAcct"|"JumpAcct"|"ReconAcct"
        AcctID = "22_123"
...
.OUTPUTS
   ...
   $false if failed
#>
function Remove-VPASDependentLinkedAccount{
    [OutputType([bool])]
    ...
    Param(
        ...
        [Parameter(Mandatory=$true,ParameterSetName='Set3',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter type of target link account (LogonAcct, JumpAcct, ReconAcct)")]
        [Parameter(Mandatory=$true,ParameterSetName='Set4',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter type of target link account (LogonAcct, JumpAcct, ReconAcct)")]
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("AccountType","safe","platform","username","address","DependentSearchQuery","WhatIf","HideWhatIfOutput")
                        MandatoryKeys = @("AccountType","DependentSearchQuery")
                    }
					set2 = @{
                        AcceptableKeys = @("AccountType","AcctID","DependentSearchQuery","WhatIf","HideWhatIfOutput")
                        MandatoryKeys = @("AccountType","AcctID","DependentSearchQuery")
                    }
                    set3 = @{
                        AcceptableKeys = @("AccountType","safe","platform","username","address","DependentAcctID","WhatIf","HideWhatIfOutput")
                        MandatoryKeys = @("AccountType","DependentAcctID")
                    }
                    set4 = @{
                        AcceptableKeys = @("AccountType","AcctID","DependentAcctID","WhatIf","HideWhatIfOutput")
                        MandatoryKeys = @("AccountType","AcctID","DependentAcctID")
                    }
                }
        ...
        try{
            ...
            elseif($AcctID -eq -2){
                ...
                if($NoSSL){
                    ...
                    if($ISPSS){
                        $uri = "http://$PVWA/api/accounts/$AcctID/account-dependents/$DependentAcctID/link-accounts/" + $AccountTypeINT
                    }
                    else{
                        $uri = "http://$PVWA/PasswordVault/api/Accounts/$AcctID/dependentAccounts/$DependentAcctID/Unlink"
                    }
                }
                else{
                    Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                    if($ISPSS){
                        $uri = "https://$PVWA/api/accounts/$AcctID/account-dependents/$DependentAcctID/link-accounts/" + $AccountTypeINT
                    }
                    else{
                        $uri = "https://$PVWA/PasswordVault/api/Accounts/$AcctID/dependentAccounts/$DependentAcctID/Unlink"
                    }
                }
                ...
                if($WhatIf){
                    ...
                    $WhatIfInfo = Get-VPASDependentAccountDetails -token $token -AcctID $AcctID -DependentAcctID $DependentAcctID
                    $WhatIfSafeName = $WhatIfInfo.safeName
                    $WhatIfFolderName = $WhatIfInfo.folderName
                    $WhatIfCreatedTime = $WhatIfInfo.createdTime
                    $WhatIfCategoryModificationTime = $WhatIfInfo.categoryModificationTime
                    $WhatIfDependentAccountId = $WhatIfInfo.dependentAccountId
                    $WhatIfAccountID = $WhatIfInfo.accountId
                    $WhatIfName = $WhatIfInfo.name
                    $WhatIfPlatformID = $WhatIfInfo.platformId

                    $AllSecretManagementProps = $WhatIfInfo.secretManagement.psobject.Properties.name
                    $AllPlatformAccountProperties = $WhatIfInfo.platformAccountProperties.psobject.Properties.name

                    if(!$HideWhatIfOutput){
                        Write-VPASOutput -str "====== BEGIN COMMAND SIMULATION ======" -type S
                        Write-VPASOutput -str "THE FOLLOWING DEPENDENT ACCOUNT WOULD HAVE THE LINKED ACCOUNT REMOVED FROM:" -type S
                        Write-VPASOutput -str "SafeName                 : $WhatIfSafeName" -type S
                        Write-VPASOutput -str "FolderName               : $WhatIfFolderName" -type S
                        Write-VPASOutput -str "CreatedTime              : $WhatIfCreatedTime" -type S
                        Write-VPASOutput -str "CategoryModificationTime : $WhatIfCategoryModificationTime" -type S
                        Write-VPASOutput -str "DependentAccountId       : $WhatIfDependentAccountId" -type S
                        Write-VPASOutput -str "AccountId                : $WhatIfAccountID" -type S
                        Write-VPASOutput -str "Name                     : $WhatIfName" -type S
                        Write-VPASOutput -str "PlatformId               : $WhatIfPlatformID" -type S

                        foreach($prop in $AllSecretManagementProps){
                            $propval = $WhatIfInfo.secretManagement.$prop
                            Write-VPASOutput -str "$prop                : $propval" -type S
                        }

                        foreach($prop in $AllPlatformAccountProperties){
                            $propval = $WhatIfInfo.platformAccountProperties.$prop
                            Write-VPASOutput -str "$prop                : $propval" -type S
                        }
                    ...
                    $WhatIfHash = @{
                        WhatIf = @{
                            ...
                            FolderName = $WhatIfFolderName
                            CreatedTime = $WhatIfCreatedTime
                            CategoryModificationTime = $WhatIfCategoryModificationTime
                            DependentAccountId = $WhatIfDependentAccountId
                            ...
                            RestURI = $uri
                            RestMethod = "DELETE"
                            Disclaimer = "THIS DEPENDENT ACCOUNT WILL HAVE THE LINKED ACCOUNT REMOVED IF -WhatIf FLAG IS REMOVED"
                        }
                    }

                    foreach($prop in $AllSecretManagementProps){
                        $propval = $WhatIfInfo.secretManagement.$prop
                        $WhatIfHash.WhatIf.SecretManagement += @{
                            $prop = $propval
                        }
                    }
                    ...
                    foreach($prop in $AllPlatformAccountProperties){
                        $propval = $WhatIfInfo.platformAccountProperties.$prop
                        $WhatIfHash.WhatIf.PlatformAccountProperties += @{
                            $prop = $propval
                        }
                    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUl3I3gDN58CEOsmkVWGSBMiEx
# i7mggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEF