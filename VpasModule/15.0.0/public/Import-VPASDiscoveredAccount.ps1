<#
.Synopsis
   Onboard discovered account
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to onboard a specific discovered account from the pending account list
.LINK
   https://vpasmodule.com/commands.html#Import-VPASDiscoveredAccount
.NOTES
   Category: PAM
...
.PARAMETER OnboardToSafe
   Target safe the discovered account will be onboarded to
.PARAMETER OnboardToPlatform
   Target platform the discovered account will be onboarded to
.PARAMETER additionalProperties
   Include extra properties that can be defined based on platform settings and configurations
   Pass extra properties in a hashtable following this pattern: @{ PropertyTag = "PropertyValue" }
   Example: -additionalProperties @{ Notes = "onboarded from discovery" }
.PARAMETER Secret
   Initial secret to be set for the discovered account upon onboarding
.PARAMETER RotateSecret
   Whether the secret of the discovered account will be rotated upon onboarding
.PARAMETER InputParameters
   HashTable of values containing the parameters required to make the API call
.EXAMPLE
   $ImportDiscoveredAccountJSON = Import-VPASDiscoveredAccount -SearchQuery {SEARCH QUERY VALUE} -OnboardToSafe {ONBOARDTOSAFE VALUE} -OnboardToPlatform {ONBOARDTOPLATFORM VALUE}
.EXAMPLE
   $ImportDiscoveredAccountJSON = Import-VPASDiscoveredAccount -DiscoveredAccountID {DISCOVERED ACCOUNT ID VALUE} -OnboardToSafe {ONBOARDTOSAFE VALUE} -OnboardToPlatform {ONBOARDTOPLATFORM VALUE}
.EXAMPLE
   $InputParameters = @{
        ...
        Enabled = "true"|"false"
        OnboardToSafe = "TestSafe01"
        OnboardToPlatform = "WinServerLocal"
   ...
   $ImportDiscoveredAccountJSON = Import-VPASDiscoveredAccount -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        DiscoveredAccountID = "12345-abcde-67890-fghijkl"
        OnboardToSafe = "TestSafe01"
        OnboardToPlatform = "WinServerLocal"
        additionalProperties = @{
            PropertyTag = "PropertyValue"
...
.OUTPUTS
   {
        "id":  "abcdefgh-ijkl-1234-5678-90abcdefgh",
        "accountId":  "13_12"
   }
   ---
   $false if failed
#>
function Import-VPASDiscoveredAccount{
    ...
    Param(
        ...
        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter safe that the account will be onboarded to (for example: TestSafe01)")]
        [Parameter(Mandatory=$true,ParameterSetName='Set2',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter safe that the account will be onboarded to (for example: TestSafe01)")]
        [String]$OnboardToSafe,

        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter safe that the account will be onboarded to (for example: TestSafe01)")]
        [Parameter(Mandatory=$true,ParameterSetName='Set2',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter safe that the account will be onboarded to (for example: TestSafe01)")]
        [String]$OnboardToPlatform,
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("SearchQuery","PlatformType","Privileged","Enabled","OnboardToSafe","OnboardToPlatform","additionalProperties","Secret","RotateSecret")
                        MandatoryKeys = @("SearchQuery","OnboardToSafe","OnboardToPlatform")
                    }
					set2 = @{
                        AcceptableKeys = @("DiscoveredAccountID","OnboardToSafe","OnboardToPlatform","additionalProperties","Secret","RotateSecret")
                        MandatoryKeys = @("DiscoveredAccountID","OnboardToSafe","OnboardToPlatform")
                    }
                }
            ...
            Write-Verbose "FAILED TO ONBOARD DISCOVERED ACCOUNTS"
        ...
        try{
            if(!$ISPSS -and ($PVWA -notmatch ".privilegecloud.cyberark.")){
                ...
                if($DiscoveredAccountID -eq -1){
                    $log = Write-VPASTextRecorder -inputval "COULD NOT FIND UNIQUE DISCOVERED ACCOUNT ENTRY TO ONBOARD, INCLUDE MORE SEARCH PARAMETERS" -token $token -LogType MISC
                    $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
                    Write-VPASOutput -str "COULD NOT FIND UNIQUE DISCOVERED ACCOUNT ENTRY TO ONBOARD, INCLUDE MORE SEARCH PARAMETERS" -type E
                ...
                elseif($DiscoveredAccountID -eq -2){
                    ...
                    $params = @{
                        coreAttributes = @{
                            safeName = $OnboardToSafe
                            platformId = $OnboardToPlatform
                        }
                    }

                    #ADDITIONAL PROPS SECTION
                    if($additionalProperties){
                        Write-Verbose "HANDLING ADDITIONAL PROPERTIES BEING PASSED"
                        $platformAccountProperties = @{}
                        $AllKeys = $additionalProperties.Keys
                        foreach($key in $AllKeys){
                            $val = $additionalProperties."$key"
                            $platformAccountProperties += @{
                                $key = $val
                            }
                        }
                        $params += @{
                            additionalProperties = $platformAccountProperties
                        }
                    }
                    ...
                    if($NoSSL){
                        ...
                        $uri = "http://$PVWA/api/discovered-accounts/$DiscoveredAccountID/onboard"
                    }
                    else{
                        Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
