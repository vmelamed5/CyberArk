...
.DESCRIPTION
   Use this function to trigger a password action on an account (Verify, Change, Reconcile, ChangeSetNew, ChangeOnlyInVault, GeneratePassword)
.LINK
   https://vpasmodule.com/commands.html#Invoke-VPASAccountPasswordAction
.NOTES
   Category: PAM
   Tag: Accounts
   Since: v10.10
...
.EXAMPLE
   $AccountPasswordActionJSON = Invoke-VPASAccountPasswordAction -action {ACTION VALUE} -AcctID {ACCTID VALUE}
.EXAMPLE
   $InputParameters = @{
                ...
                if($triggeraction -eq 1){
                    try{
                        ...
                        if($NoSSL){
                            ...
                            if($SRS){
                                $uri = "http://$PVWA/api/accounts/$AcctID/verify-secret"
                            }
                            else{
                            ...
                            if($SRS){
                                $uri = "https://$PVWA/api/accounts/$AcctID/verify-secret"
                            }
                            else{
                ...
                elseif($triggeraction -eq 2){
                    try{
                        ...
                        if($NoSSL){
                            Write-Verbose "NO SSL ENABLED, USING HTTP INSTEAD OF HTTPS"
                            if($SRS){
                                $uri = "http://$PVWA/api/accounts/$AcctID/reconcile-secret"
                            }
                            else{
                            ...
                            Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                            if($SRS){
                                $uri = "https://$PVWA/api/accounts/$AcctID/reconcile-secret"
                            }
                            else{
                ...
                elseif($triggeraction -eq 3){
                    try{
                        ...
                        if($SRS){
                            $params = @{
                                newSecret = $newPass
                                operationType = "VAULT_ONLY"
                            } | ConvertTo-Json

                            $logparams = @{
                                newSecret = "{NewCredentials}"
                                operationType = "VAULT_ONLY"
                            }
                        ...
                        if($NoSSL){
                            Write-Verbose "NO SSL ENABLED, USING HTTP INSTEAD OF HTTPS"
                            if($SRS){
                                $uri = "http://$PVWA/api/accounts/$AcctID/change-secret"
                            }
                            else{
                            ...
                            Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                            if($SRS){
                                $uri = "https://$PVWA/api/accounts/$AcctID/change-secret"
                            }
                            else{
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUAq0TIO2I2Vj56CepqStHU9XE
# hGyggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFCPY53mhDchjs9025jsyYjObgvLI
# MA0GCSqGSIb3DQEBAQUABIICAAcBnJp5sfsgadqM+8L90IBYHvzg69aoVFcJ44nZ
# 2vUJU18LbM/v1yR16wh1fFLM/9Y/vurrePcKyYlv+CYbtwPRvjVUf48eXo7T/b/1
# Yqb1Jxm4ouqjDAvKbXUBIq/elLHWWmvNpiAgO141Vu3s+RwPp0CBSe2uQaOZouHe
# vSZ6sgkA75ouAyDwUboyWSPltXGCZuY95yY2/Gm/iyRGdmJV+Su9Jz3hlypObrwy
# vzCadr0mBV32/TL6IX7atERPBhDRIfcdmuGCzrJO6Q/D1GnX12Y+LhNLopHlJ1B+
