<#
.Synopsis
   Approve incoming account request
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to approve an incoming account request
.LINK
   https://vpasmodule.com/commands.html#Approve-VPASIncomingRequest
.NOTES
   Category: PAM
...
.EXAMPLE
   $ApproveIncomingRequestStatus = Approve-VPASIncomingRequest -RequestedSafe {SAFE VALUE} -RequestedPlatform {PLATFORM VALUE} -RequestedUsername {USERNAME VALUE} -RequestedAddress {ADDRESS VALUE} -approveReason {REASON VALUE}
.EXAMPLE
   $ApproveIncomingRequestStatus = Approve-VPASIncomingRequest -RequestedAcctID {ACCTID VALUE} -approveReason {REASON VALUE}
.EXAMPLE
   $ApproveIncomingRequestStatus = Approve-VPASIncomingRequest -requestID {REQUEST ID VALUE} -approveReason {REASON VALUE}
.EXAMPLE
...
.EXAMPLE
   $InputParameters = @{
        ...
        approveReason = "Vman approve of this request"
        WhatIf = $true|$false
        HideWhatIfOutput = $true|$false
   }
   $ApproveIncomingRequestStatus = Approve-VPASIncomingRequest -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
...
function Approve-VPASIncomingRequest{
    ...
    Param(
        ...
        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter reason for approving the incoming request (for example: manager approved)")]
        [Parameter(Mandatory=$true,ParameterSetName='Set2',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter reason for approving the incoming request (for example: manager approved)")]
        [Parameter(Mandatory=$true,ParameterSetName='Set3',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter reason for approving the incoming request (for example: manager approved)")]
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("RequestedSafe","RequestedPlatform","RequestedUsername","RequestedAddress","RequestedReason","approveReason","WhatIf","HideWhatIfOutput")
                        MandatoryKeys = @("approveReason")
                    }
					set2 = @{
                        AcceptableKeys = @("RequestedAcctID","RequestedReason","approveReason","WhatIf","HideWhatIfOutput")
                        MandatoryKeys = @("RequestedAcctID","approveReason")
                    }
                    set3 = @{
                        AcceptableKeys = @("requestID","approveReason","WhatIf","HideWhatIfOutput")
                        MandatoryKeys = @("requestID","approveReason")
                    }
                }
            ...
            Write-Verbose "FAILED TO APPROVE PENDING ACCOUNT REQUEST"
        ...
        try{
                ...
                if($reqCount -eq 0 -or $reqCount -gt 1){
                    ...
                    if($WhatIf){
                        ...
                        $WhatIfInfo = Get-VPASIncomingRequestDetails -requestID "$requestID" -token $token
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUTBuA8BKW+COo8Y1LphUekyqW
# BGOggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFPbZqfyVeNunVpvTgB15M5BiKL9z
# MA0GCSqGSIb3DQEBAQUABIICALfrCk1ugbBhHYZewc8O7WSQ05FI3wpdjOyGX3Lh
# FA5mciOBhZ5tSuJxIA4fuxAMLMIhQCtvkymzWaA9HQo8sA3/9mdybgwSGNbvRae8
# 9mDtCp3yY/XwTGQqVUotoN/Hnjw4lygQ8lS2LJ/z6Th1uuaZfKvrn5FFQ8ATqtZk
# Of33NHKUn+OX2kkRYacUF6HGisc5acwZLMHcJ3/v7Ead7svipOxQGgZDEcAfugZ9
