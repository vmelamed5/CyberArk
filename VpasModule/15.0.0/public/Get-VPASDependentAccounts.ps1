...
.DESCRIPTION
   Use this function to retrieve dependent (usage) accounts for an account
.LINK
   https://vpasmodule.com/commands.html#Get-VPASDependentAccounts
.NOTES
   Category: PAM
...
.PARAMETER FailedOnly
   Retrieve only the failed dependent accounts
...
.EXAMPLE
   $DependentAccountsJSON = Get-VPASDependentAccounts -safe {SAFE VALUE} -username {USERNAME VALUE} -platform {PLATFORM VALUE} -address {ADDRESS VALUE}
.EXAMPLE
   $DependentAccountsJSON = Get-VPASDependentAccounts -AcctID {ACCTID VALUE} -FailedOnly
.EXAMPLE
   $InputParameters = @{
        safe = "TargetSafe"
   ...
   $DependentAccountsJSON = Get-VPASDependentAccounts -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        AcctID = "22_123"
        FailedOnly = $true|$false
...
.OUTPUTS
   If successful:
   {
        "safeName":  "testsafe",
        "folderName":  "Root",
        "createdTime":  1751951084,
        "categoryModificationTime":  1751951084,
        "dependentAccountId":  "30_6",
        "accountId":  "30_3",
        "name":  "Operating System-WinLocalVman-server2.vman.com-NetworkAdmin01-WinService-server1-servicename1",
        ...
        "secretManagement":  {
                                 ...
                                 "lastModifiedTime":  1751951084
                             }
   }
   ---
   $false if failed
#>
function Get-VPASDependentAccounts{
    ...
    process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("safe","platform","username","address","FailedOnly","ExportToCSV","CSVDirectory")
                        MandatoryKeys = @()
                    }
					set2 = @{
                        AcceptableKeys = @("AcctID","FailedOnly","ExportToCSV","CSVDirectory")
        ...
        try{
            ...
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-Verbose "UNABLE TO GET DEPENDENT ACCOUNTS"
            Write-VPASOutput -str $_ -type E
            return $false
        }
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU0VLJ1X9SDku/a+yKWrE6Pjwr
# NJyggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFKAVDSdXNlfWaUOQ+X6cBEj1TMH8
# MA0GCSqGSIb3DQEBAQUABIICAKVE3vKqbVL9voX4amNLp6lS8R9RG+EfFwxVNyva
# gGpFebcZN+q11WlmZZdyWMmHnXqkQpTsVGoDZsduKe8TWDGI+WvpVeflOEPZ/gKB
# oUOr+FAg5a848E4jIq8HB9DeQeUERrMV9RJtYyINO4myQTpdlE3zuAjgTvgpiu5G
# kkGtvCXRYnOs36UvQqa7yU8+XPpva3BsGMeNCHwW+Zjqms+1pAgGV3drQYOIqPzC
# 9S40XyHhEQd7KP3UJzPqL6Z+rEpnQb7wgyMhQMVSfAcReHtBwsGEooDdYdD0VCuM
# 2Ug5ALQIwCs/GknCPZDtHxrxxpEqb61fKXdueUom79KyN+/MZa7+JqjCSdS0J7fo
# FJjlOdtZJ54Rapyy1Ha5xT2J7aXePkB1kek7ODDbtKq0Z8UywMlBpQa3nCbXp+DR
# Cr9Iy1gg41N5KEGLtsjwKNxNXdRRk9yKvGF1R8Z46F61MGSVKAqbc1N+xIg+L/8A
# 6hXKHxn/DhbiHd1Ju5DvXZcour/cQ2DdxOuCxRd8RR78lK8w5TC8F2Tw5yeWEA4b
# eApm+srvZBfGvVcxE28hXJ3pGA+ByVKOt37rygjJoix9sGaZXo2NZDzpL4n9nBn7
# 5OeLvPsH2OPhEoYCOmUCW2by927THw7BR5rMY/p1eCytj44vQAydMu+Y4KjPb+Nf
# RUf5oYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkGA1UEBhMC
...
# 9w0BCQUxDxcNMjYwMjA0MDQ0NzQ0WjA/BgkqhkiG9w0BCQQxMgQwJvztJiqEW3Dx
# dFRRjIKRSwilb4ubJ9AZkouxzNedUZzhOExihpyD2WnNzOKnBQuqMA0GCSqGSIb3
# DQEBAQUABIICAFwHdIR3P7NdFCCfi6vvBPveM/t1QacJMPJgr6UYSTHq13Mxw34p
# zN+fKEKH92X9ofc1BJ/5mQ2B5FipJUu4CSxDwyWjExKPKHzUjNUD7D8XSwbFXS15
# zObcr43aGGBJDY9ZebknKoeCy2SzLLVDhRwfAoQpq+i58lhYD2rFqESSj5JmKVSj
# WeHoIYj+HGWzR2A6lTUFR72PspZAotIO6i82LhHYtEU0RpCecQHGqpgTBh6QE7BM
# FR1oCVF+2akkZM6qk+Erg6Beos0pYdcn8JMHvWCl52v9azKQBdw3iW9lda0dWHBB
# oBLu7D2IMFaStsksOK7/zJKhMSF5h3dE47z/qYNpAXQUcWhFNaKFEx1Uf2lDaG9s
# ormoQwfwokXik3iGSvuS5QY53EkcG18lEc4BUTbor6m5WiHIkt4UDx7WMAhMi6LB
# 65eP4pQPFLDFtXyDQDbJCjDnSjMa4FpKx37PqN8NJbMFiKIfd9l/hokqI/m3aDiu
# w3uKK6S22tWKbV5zUyOYhR5mSGjJPQGvNIdryg40OjvF2jAqC61oIvB0BomcwW27
# o4YrZwPLzmqShXHrDBq/39C/g2goAj9n0put/Dlypa9LRtBTfz4yZpZH/fxn7LUI
# 6J+NkvgomaGE82QXJB6baRhZeXFa3CNoqGTf3g3gT4xePeabxLB+lJmQ
