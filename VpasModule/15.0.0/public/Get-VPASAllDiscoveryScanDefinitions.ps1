<#
.Synopsis
   Get all discovery scan definitions
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to retrieve all discovery scan definitions currently configured in the environment
.LINK
   https://vpasmodule.com/commands.html#Get-VPASAllDiscoveryScanDefinitions
.NOTES
   Category: PAM
...
.EXAMPLE
   $AllDiscoveryScanDefinitionsJSON = Get-VPASAllDiscoveryScanDefinitions
.EXAMPLE
   $InputParameters = @{
        ExportToCSV = $true|$false
        CSVDirectory = "C:\Temp"
   }
   $AllDiscoveryScanDefinitionsJSON = Get-VPASAllDiscoveryScanDefinitions -InputParameters $InputParameters
.OUTPUTS
   If successful:
   {
        "totalCount":  7,
        "items":  [
                      ...
                      {
                          "id":  "123456-7890-abcd-efgh-ijklmn0987",
                          "name":  "VmanTestOU Win OU Scan - TestDiscoveryScan",
                          "type":  "WIN_NIX_DOMAIN",
                          "properties":  {
                                             "adGroups":  {
                                                              "groups":  true
                                             ...
                                             "ou":  "OU=VmanTestOU,DC=vman,DC=com",
                                             "domain":  "vman.com"
                                         },
                          ...
                          "creationTime":  "2025-11-26T06:43:12Z",
                          "updateTime":  "2026-01-16T17:31:27Z",
                          "tags":  [
                                       "alphas",
                                       "TestDiscoveryScan"
                                   ],
                          "lastScanInstance":  {
                                                   "creationTime":  "2026-01-16T17:30:23Z",
                                                   "reason":  "Can\u0027t find a suitable connector with an active agent that has a matching identifier or is related to the requested network.",
                                                   "updateTime":  "2026-01-16T17:31:27Z",
                                                   "id":  "123456-7890-abcd-efgh-ijklmn0987",
                                                   "status":  "FAILED"
                                               },
   ...
   $false if failed
#>
function Get-VPASAllDiscoveryScanDefinitions{
    ...
    Process{
        try{
            ...
            Write-Verbose "FAILED TO RETRIEVE DISCOVERY SCAN DEFINITIONS"
        ...
        try{
            if(!$ISPSS -and ($PVWA -notmatch ".privilegecloud.cyberark.")){
                ...
                if($NoSSL){
                    ...
                    $uri = "http://$PVWA/api/scan-definitions/?extendedDetails=$true"
                }
                else{
                    Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                    $uri = "https://$PVWA/api/scan-definitions/?extendedDetails=$true"
                }
            ...
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-Verbose "UNABLE TO GET ALL DISCOVERY SCAN DEFINITIONS"
            Write-VPASOutput -str $_ -type E
            return $false
        }
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUegJckePJIJ53KxDLbjY3zojn
# o0SggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFB1jLfjjF9EPoASrhP8JlV6BdzQf
# MA0GCSqGSIb3DQEBAQUABIICAK9rQd+EVDUcfuNRLlugItGIEw1ffmN10rNFK2hs
# u9u2FrhIp1xmOAP8XEbpUx4I1uTyuxK0j3c9Y+4qaa7Tq6wPL9WLPtpmP4/tEd9/
# 1VhjpP7uzGJIFCfKQxiyQ+owOgly50wl5N1Ut1rdsBuQl/u3X3cIC7Ybk6UtnZtM
# P3SQs51XnbCxar7MLi/RuQCAxxQFwGVoPZgc6qYskn3MIQwoBxq98A5/ycTUbR3g
