...
.DESCRIPTION
   Use this function to get an account's password history
.LINK
   https://vpasmodule.com/commands.html#Get-VPASPasswordHistory
.NOTES
   Category: PAM
   Tag: Accounts
   Since: v12.2.0
...
.EXAMPLE
   $AccountPasswordsHistoryJSON = Get-VPASPasswordHistory -ShowTemporary -AcctID {ACCTID VALUE}
.EXAMPLE
   $InputParameters = @{
        safe = "TargetSafe"
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("safe","platform","username","address","ShowTemporary","ExportToCSV","CSVDirectory")
                        MandatoryKeys = @()
                    }
					set2 = @{
                        AcceptableKeys = @("AcctID","ShowTemporary","ExportToCSV","CSVDirectory")
        ...
        if([String]::IsNullOrEmpty($AcctID)){
            ...
            elseif($AcctID -eq -2){
                ...
                try{
                    ...
                    if($ExportToCSV){
	                    $runexport = Write-VPASExportCSV -Data $response.Versions -CommandName $CommandName -CSVDirectory $CSVDirectory
                    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU60Kan6KzFSAGjeOKp7/wTZoo
# lrWggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFC1sAJwv19TWqucKN3qa6cIRAJ9/
# MA0GCSqGSIb3DQEBAQUABIICAH3AGe+u+GHY20Lt5rOyZ8jZ+IU8ns2Yx8CJT3y1
# 5kE2n/COr2ufp1AhPN1AVHkMcgGxmpGrkG02o1ReOZwz4aAbad1tdwNMZKQ7NXkr
# 2PbRgU47OTuqM4B0L850WVGqLYaLlklUn2Zs31B4iihnZDn7vYTv9DPRfr60JR38
# KBipSvGYoxCmiJ8Oq50DXwKBWRTnryRqMDAeSGGeIocerX+bAkmTctUBnA3FJDT2
# q+6QMI0Ml5jAYJLSENBVYE8vRB/HhjRcKVq85iEXe46VUUoQEYdBPoMU4+U5mCSJ
# nxwVYn0JE6PtJOLQZ7sYEeRIDqtCLGZYOVd1VNlvGW77w56X9hEacHmruN9nDD8Q
# aob2VazkWRrftO0bQxK+SEnDheeSu+KXx6UcBp0gUZJX7ryDv8bsBDQ12r9ZvVh7
# u1i0EVVdd8/hBtENTLN/EvsCaSTrYj1HER+Rjck8jv9v52Dag2/GhCsImnHjbJBl
# Sc8vgPYQXt1ZBE91mggcvdSF0DT5cIXhK87ZlsysScdPqW5Jtnz+mXBs2tTcs/Og
# JBygQiQTIJ3lLMocS1/fNXulRxUt8a5DZyx6U/hLTnB/06vIAO5rxNJVNbbCdnGi
# 27ZGN5HKWcmePkWCA7jXt0Pu3BfNnzv8JtIQXhyOD8klbhOMc3r8KzQ9xWfgo5/I
# dMS0oYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkGA1UEBhMC
...
# 9w0BCQUxDxcNMjYwMjA0MDQ0OTI3WjA/BgkqhkiG9w0BCQQxMgQwlwM2jsmtPqdj
# ZhKTOJJmEhyKOZ5GOAXD6MIbKZVFhQW+N8T18+q7N6ms6lAl7fT2MA0GCSqGSIb3
# DQEBAQUABIICAM98xwbnnVsZfU0tCBhvFHg7hZiyAN7+op4zNjFGGECZyZcUl/6d
# 8fip+yUKtjePd021XchxoVVNIPQA4PmaNy9uxeX/wjXGi5I8LXK8NuhXYfzkrYZh
# aejYfLgkwTDlzn77BGymyJOfEISKgnzO7xOXuLWnLC1I5uF0Kr7MSy/SFS2BDLGE
# gJZFqgQYDXuSgsh0064qRyOcLh1dBF+KDXRXg6a64sBXgfSdqqR3z/Zf2Q1+tD+9
# SXYe9jtGf53P0+oTcQeTP/Y8egcC5XLXZ383+HG9v1jQg3Iz7sQfKGE25FnhTm6o
# JX8QDbJ1HLDejVR0BVWA7nhKnN6Bi420scTxyjBogL3GUE+GNpMOlz8KJ0fXzbW6
# NQrYywIA/ALzxmn2rrHMcNSGojAudhmdmHzK/EJYDePHtn5yONwo/uSGsTJnriuF
# teWk/OwFUdje5051JsBMgQAqqhv+R3B++UEFN0NCMk494FW3NESVeCRzkaQW2Hf2
# 6X8tyTFrifRo7wn2eESEseH0Jnt9dIye5A/0p+5GOVi7t24e5fpJDDcdH19OMYAm
# lXNjmOGckcQ0MSDhqAEPYp5OcvFB+GFLPp/XtOT7ntvv8gWU2RV3lBruOO90EW/j
# ZpDz9ioz8Qz8xe6G5wuhSI0ppS79hLNIZg+JBf3BJW7ldtnJUCiMmfQd
