<#
.Synopsis
   Update platform plugin settings
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to update a platform plugin setting
.LINK
   https://vpasmodule.com/commands.html#Update-VPASPlatformPluginSettings
.NOTES
   Category: PAM
   Tag: Platforms
   Since: v15.0.0
...
.PARAMETER Field
   ...
   Possible values: XMLFile, PromptsFilename, ProcessFilename, Timeout, ExeName, DllName, Debug, ManagementType, UnrecoverableErrors, RCReconcileReasons, pluginSpecificProperty, customProperty
.PARAMETER FieldVal
   Target value to update the target field with
.PARAMETER InputParameters
   HashTable of values containing the parameters required to make the API call
.EXAMPLE
   $UpdatePlatformPluginSettings = Update-VPASPlatformPluginSettings -PlatformID {PLATFORMID VALUE} -Field {FIELD VALUE} -FieldVal {FIELDVAL VALUE}
.EXAMPLE
   $InputParameters = @{
        PlatformID = "TargetPlatformID"
        Field = "XMLFile"|"PromptsFilename"|"ProcessFilename"|"Timeout"|"ExeName"|"DllName"|"Debug"|"ManagementType"|"UnrecoverableErrors"|"RCReconcileReasons"|"pluginSpecificProperty"|"customProperty"
        FieldVal = "UpdatedValue"
   }
   $UpdatePlatformPluginSettings = Update-VPASPlatformPluginSettings -InputParameters $InputParameters
.OUTPUTS
   If successful:
   {
        "params":  {
                       "general":  {
                                       "XMLFile":  true,
                                       "PromptsFilename":  null,
                                       "ProcessFilename":  null,
                                       "Timeout":  120,
                                       "ExeName":  "PassChng.exe",
                       ...
                       "pluginSpecificList":  [
                                                  {
                                                      "name":  "DisableAddressResolving",
                                                      "value":  "No"
                                                  },
                                                  {
                                                      "name":  "VerifyMachineNameBeforeAction",
                                                      "value":  "No"
                                                  },
                                                  {
                                                      "name":  "UnlockUserOnReconcile",
                                                      "value":  "No"
                                                  }
                                              ],
                       "custom":  null,
                       ...
                       "isMigratedUnmappedPlugin":  false,
                       "migratedUnmappedPluginsComponentName":  "",
                       "migratedUnmappedPluginsComponentVersion":  "",
                       "isMigratedOriginalBin":  false,
                       "migratedOriginalBinComponentName":  "",
                       "migratedOriginalBinComponentVersion":  "",
                       "tempPackage":  null,
                       "frameworkName":  "",
                       "frameworkVersion":  "",
                       "autoInstallMsAddons":  true
                   },
        "pluginInfo":  {
                           "dateAdded":  "2026-01-12 04:15:57.986000+00:00",
                           "connectorManagementFolder":  "PMWindows_19.0.2",
                           "name":  "PMWindows",
                           "version":  "19.0.2",
                           "isOutOfTheBox":  true,
                           "id":  "1234567890987654321abcdefghijklmnopqrstuvwxyv55710d9b16c371eab33cfd2970joiu89689uik"
                       },
        "platformBaseType":  "Windows",
        "createdAt":  "2026-01-12T04:15:57.986Z",
        "updatedAt":  "2026-01-12T04:15:57.986Z",
        "createdBy":  null,
        "updatedBy":  null,
        "platformId":  "WinDomainDup2",
   ...
   $false if failed
#>
function Update-VPASPlatformPluginSettings{
    [OutputType([bool])]
    ...
    Param(
        ...
        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Target field that will be updated (for example: passwordLen)")]
        [ValidateSet('XMLFile','PromptsFilename','ProcessFilename','Timeout','ExeName','DllName','Debug','ManagementType','UnrecoverableErrors','RCReconcileReasons','pluginSpecificProperty','customProperty')]
        [String]$Field,

        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Updated value for the targeted field (for example: 'Mon,Tue,Wed')")]$FieldVal,
    ...
    Process{
        try{
            ...
            Write-Verbose "FAILED TO UPDATE PLATFORM PLUGIN SETTING"
        ...
        try{
            ...
            Write-Verbose "FETCHING CURRENT PLUGIN SETTINGS FOR $PlatformID"
            $currentValues = Get-VPASPlatformPluginSettings -platformID $PlatformID
            if(!$currentValues){
                Write-Verbose "COULD NOT FIND $PlatformID PLUGIN SETTINGS"
                Write-VPASOutput -str "COULD NOT FIND $PlatformID PLUGIN SETTINGS" -type E
                return $false
            }

            $general = @("XMLFile","PromptsFilename","ProcessFilename","Timeout","ExeName","DllName","Debug","ManagementType","UnrecoverableErrors","RCReconcileReasons")
            Write-Verbose "SETTING PARAMETERS"

            #HANDLE GENERAL PROPERTIES
            if($general.Contains($Field)){
                $currentValues.params.general.$Field = $FieldVal
            }

            if($Field -eq "pluginSpecificProperty"){
                if($currentValues.params.plpluginSpecificList){
                    $currentValues.params.pluginSpecificList += $FieldVal
                }
                else{
                    $currentValues.params.pluginSpecificList = @($FieldVal)
                }
            ...
            if($Field -eq "customProperty"){
                if($currentValues.params.custom){
                    $currentValues.params.custom += $FieldVal
                }
                else{
                    $currentValues.params.custom = @($FieldVal)
                }
            ...
            if($NoSSL){
                ...
                $uri = "http://$PVWA/api/platforms/$PlatformID/plugin-settings"
            }
            else{
                Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                $uri = "https://$PVWA/api/platforms/$PlatformID/plugin-settings"
            }
            $log = Write-VPASTextRecorder -inputval $uri -token $token -LogType URI
            $log = Write-VPASTextRecorder -inputval "PUT" -token $token -LogType METHOD

            if($sessionval){
                $response = Invoke-RestMethod -Headers @{"Authorization"=$Header} -Uri $uri -Body $currentValues -Method PUT -ContentType "application/json" -WebSession $sessionval
                $APICounter += 1
            }
            else{
                $response = Invoke-RestMethod -Headers @{"Authorization"=$Header} -Uri $uri -Body $currentValues -Method PUT -ContentType "application/json"
                $APICounter += 1
            }

            $outputlog = $response
            $log = Write-VPASTextRecorder -inputval $outputlog -token $token -LogType RETURN
            Write-Verbose "UPDATES MADE TO $PlatformID PLUGIN SETTINGS, RECEIVED JSON OBJECT"

            return $response
        }catch{
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-Verbose "UNABLE TO UPDATE $PlatformID PLUGIN SETTINGS"
            Write-VPASOutput -str $_ -type E
            return $false
        }
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUYbqd2KzBeMlMPBD3KncanINV
# l8SggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFLUG6vjkS5nDWtBnjH/P0cohKUlB
# MA0GCSqGSIb3DQEBAQUABIICAJGwy+CFEjSXwJudfY2deGvesXrJSznvilcEcFst
# 2CLmVDLOP6qXA/blydz2MGksdtnjSjmyVybbzIvAAHwCaxqoIHpS8BgZdPqlMEGT
# UYczRrfEXIv1k84l13WctnIWUOgvpkkBOQJE6nCMAtp/Famo5k8Ru7BDld7HNJEw
