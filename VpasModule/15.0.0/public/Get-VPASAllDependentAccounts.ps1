<#
.Synopsis
   Get all dependent accounts
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to retrieve all dependent (usages) accounts
.LINK
   https://vpasmodule.com/commands.html#Get-VPASAllDependentAccounts
.NOTES
   Category: PAM
...
.EXAMPLE
   $AllDependentsAccountsJSON = Get-VPASAllDependentAccounts
.EXAMPLE
   $InputParameters = @{
        ExportToCSV = $true|$false
        CSVDirectory = "C:\Temp"
   }
   $AllDependentsAccountsJSON = Get-VPASAllDependentAccounts -InputParameters $InputParameters
.OUTPUTS
   If successful:
   {
        "DependentAccounts":  [
                                  {
                                      ...
                                      "platformId":  "ComPlus",
                                      "platformAccountProperties":  "@{ApplicationName=comApp1; Address=server5; Restart=Yes}",
                                      "secretManagement":  "@{automaticManagementEnabled=True; lastModifiedTime=1751945098}"
                                  },
                                  {
                                      "safeName":  "TestSafe",
                                      "folderName":  "Root",
                                      "createdTime":  1751945100,
                                      "categoryModificationTime":  1751945101,
                                      "dependentAccountId":  "29_26",
                                      "accountId":  "29_3",
                                      "name":  "Operating System-WinLocalVman-server1.vman.com-LocalAdmin01-ComPlus-server5-comApp2",
                                      "platformId":  "ComPlus",
                                      "platformAccountProperties":  "@{ApplicationName=comApp2; Address=server5}",
                                      "secretManagement":  "@{automaticManagementEnabled=True; lastModifiedTime=1751945100}"
                                  }
                              ],
        "Total":  149
   }
   ...
   $false if failed
#>
function Get-VPASAllDependentAccounts{
    ...
    Process{
        try{
            ...
            Write-Verbose "FAILED TO RETRIEVE DEPENDENT ACCOUNTS"
            Write-VPASOutput -str $_ -type E
            return $false
        }

        try{
            $apiLimit = Get-Random -Minimum 95 -Maximum 100 #Terrible Temp fix if dependent accounts equal apilimit
            ...
            if($NoSSL){
                ...
                $uri = "http://$PVWA/PasswordVault/api/dependentAccounts?limit=$apiLimit&search=$searchQuery"
            }
            else{
                Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                $uri = "https://$PVWA/PasswordVault/api/dependentAccounts?limit=$apiLimit&search=$searchQuery"
            }
            ...
            $curtotal = $curcount
            if($ISPSS){
                $curItems = $response.dependentAccount
            }
            else{
                $curItems = $response.dependentAccounts
            }
            while($curcount -eq $apiLimit){
                ...
                if($NoSSL){
                    Write-Verbose "NO SSL ENABLED, USING HTTP INSTEAD OF HTTPS"
                    $uri = "http://$PVWA/PasswordVault/api/dependentAccounts?limit=$apiLimit&offset=$offset&search=$searchQuery"
                }
                else{
                    Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                    $uri = "https://$PVWA/PasswordVault/api/dependentAccounts?limit=$apiLimit&offset=$offset&search=$searchQuery"
                }
                Write-Verbose "SETTING URI: $uri"
                $log = Write-VPASTextRecorder -inputval $uri -token $token -LogType URI
                $log = Write-VPASTextRecorder -inputval "GET" -token $token -LogType METHOD
            ...
            if($ISPSS){
                $response.dependentAccount = $curItems
            }
            else{
                $response.dependentAccounts = $curItems
            }
            ...
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-Verbose "FAILED TO GET ALL DEPENDENT ACCOUNTS"
            Write-VPASOutput -str $_ -type E
            return $false
        }
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUsPYTkRvOcKtwKWO/Ko0CjAXd
# 5U2ggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYE