<#
.Synopsis
   Update MasterPolicy settings for platform
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to update Master Policy configurations for a platform (DualControl, ExclusiveAccess, OneTimePassword, Compliance period, etc.)
.LINK
   https://vpasmodule.com/commands.html#Update-VPASMasterPolicySettings
.NOTES
   Category: PAM
   Tag: Master Policy
   Since: v14.6.0
...
.PARAMETER TargetSetting
   Specify which master policy setting will be updated
   Possible values: DualControl, MultiLevelApproval, OnlyManagersApproval, ConfirmersNumber, EnforceExclusiveAccess, EnforceOneTimePassword, TransparentConnection, AllowViewPassword, RequireReason, AllowFreeText, PasswordChangeDays, PasswordVerificationDays, RequireMonitoringAndIsolation, RecordActivity, RetentionPeriod
.PARAMETER SettingValue
   Target value to update the target master policy setting with
.PARAMETER InputParameters
   HashTable of values containing the parameters required to make the API call
.EXAMPLE
   $UpdatePlatformMasterPolicy = Update-VPASMasterPolicySettings -PlatformID {PLATFORMID VALUE} -TargetSetting {TARGETSETTING VALUE} -SettingValue {SETTINGVALUE VALUE}
.EXAMPLE
   $InputParameters = @{
        PlatformID = "TargetSafeName"
		TargetSetting = "DualControl"|"MultiLevelApproval"|"OnlyManagersApproval"|"ConfirmersNumber"|"EnforceExclusiveAccess"|"EnforceOneTimePassword"|"TransparentConnection"|"AllowViewPassword"|"RequireReason"|"AllowFreeText"|"PasswordChangeDays"|"PasswordVerificationDays"|"RequireMonitoringAndIsolation"|"RecordActivity"|"RetentionPeriod"
		SettingValue = $true|$false|30|60
   }
   $UpdatePlatformMasterPolicy = Update-VPASMasterPolicySettings -InputParameters $InputParameters
.OUTPUTS
   $true if successful
   ---
   $false if failed
#>
function Update-VPASMasterPolicySettings{
    [OutputType([bool])]
    ...
    Param(
        ...
        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter which master policy setting to update (DualControl, MultiLevelApproval, OnlyManagersApproval, ConfirmersNumber, EnforceExclusiveAccess, EnforceOneTimePassword, TransparentConnection, AllowViewPassword, RequireReason, AllowFreeText, PasswordChangeDays, PasswordVerificationDays, RequireMonitoringAndIsolation, RecordActivity, RetentionPeriod)")]
        [ValidateSet('DualControl','MultiLevelApproval','OnlyManagersApproval','ConfirmersNumber','EnforceExclusiveAccess','EnforceOneTimePassword','TransparentConnection','AllowViewPassword','RequireReason','AllowFreeText','PasswordChangeDays','PasswordVerificationDays','RequireMonitoringAndIsolation','RecordActivity','RetentionPeriod')]
        [String]$TargetSetting,

        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Value to update the safe with")]
        [String]$SettingValue,
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("PlatformID","TargetSetting","SettingValue")
                        MandatoryKeys = @("PlatformID","TargetSetting","SettingValue")
                    }
                }
            ...
            Write-Verbose "FAILED TO UPDATE MASTER POLICY FOR PLATFORM"
        ...
        try{
            ...
            if($platID -eq -1){
                ...
                Write-Verbose "GETTING CURRENT PLATFORM MASTER POLICY SETTINGS"
                $curvals = Get-VPASMasterPolicySettings -token $token -PlatformID $PlatformID

                Write-Verbose "INITIALIZING API PARAMS"
                $params = @{
                    DualControl = $curvals.DualControl.Value
                    MultiLevelApproval = $curvals.MultiLevelApproval.Value
                    OnlyManagersApproval = $curvals.OnlyManagersApproval.Value
                    ConfirmersNumber = $curvals.ConfirmersNumber.Value
                    EnforceExclusiveAccess = $curvals.EnforceExclusiveAccess.Value
                    EnforceOneTimePassword = $curvals.EnforceOneTimePassword.Value
                    TransparentConnection = $curvals.TransparentConnection.Value
                    AllowViewPassword = $curvals.AllowViewPassword.Value
                    RequireReason = $curvals.RequireReason.Value
                    AllowFreeText = $curvals.AllowFreeText.Value
                    PasswordChangeDays = $curvals.PasswordChangeDays.Value
                    PasswordVerificationDays = $curvals.PasswordVerificationDays.Value
                    RequireMonitoringAndIsolation = $curvals.RequireMonitoringAndIsolation.Value
                    RecordActivity = $curvals.RecordActivity.Value
                    RetentionPeriod = $curvals.RetentionPeriod.Value
                }

                Write-Verbose "ADDING THE MASTER POLICY UPDATE TO PARAMETERS: $TargetSetting WITH A VALUE OF $SettingValue"
                $params.$TargetSetting = $SettingValue
                ...
                Write-Verbose "SUCCESSFULLY UPDATED PLATFORM MASTER POLICY SETTINGS FOR PLATFORM: $PlatformID"
            ...
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-Verbose "UNABLE TO UPDATE MASTER POLICY SETTINGS FOR PLATFORM: $PlatformID"
            Write-VPASOutput -str $_ -type E
            return $false
        }
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUhmMP+YUtysf8k7Fa26BiDv1t
# xVeggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFICrZwGyoj6LzxvYsE5MKIPuvGj0
# MA0GCSqGSIb3DQEBAQUABIICAL/jisxav7omewPvTuo7s2zahLJrgDvLf3InHyMc
# 7w1AITb6DZpjHx+lArXY8qn3VByZ+ulUirgdEEMXNQr885T5isrStND2Ub8/3aSV
# HOpl9lc1hIjdr8P+jdV/uPAMYamvbf4BJRTBVHRGfB459iYrVquKDSdGp8l4nXrm
# mVZW5EBmSz81Zns+gCX0spn/+IU6djr/MKCM+V3EfgqaHST+VQZIcLtxcVOKK1M0
# wrm6ye2LWuevQIBqBgtsQH+qCqFv4Xs+JCs6Ij3K26jXI8f/ri99eh10ztF4fy1v
# SJ7hQQiR9DF7ydXGkgsvL4zqhjIkDJPBmZbp2qEA2TZHZHkSHAepKhArWSKI9s/R
# C0CYge4bfk2ZTcmdKom4I3sCaoBXHPhONvecuVFyPQM/abzGJmmLTvKGpQhKsn2l
# INO2H1dCO0y4b7ZdXqNfaVYzCuD2xf+rVCrH/M/t53AP3jbK5R/olHfDcPsqnIjb
# ixj1rhmpDlE+Md2ozzdu/jy8h2fMykkuwc5k/w1YJHncfc3walotvX4MKSoueA1h
# mxIoLH79FlSgTS2ApDXV+5kgbDWi+7nZhOZoZVRylpE3Z8rnQb2uSAq5HBL7Q7rX
# S1opCkrlwSv7P0oITnAoYXJZunAdyGR3Q9tpTctJ6+Sh9DOEz1gE3CD3lDkVoZRU
# dfzhoYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkGA1UEBhMC
...
# 9w0BCQUxDxcNMjYwMjA0MDQ1NTI3WjA/BgkqhkiG9w0BCQQxMgQwcHBmRHLQkXan
# eXq0znYXhhZubABheuCVTpNnl/eETmAw7FLYQBlfHGUTT6qPLn6IMA0GCSqGSIb3
# DQEBAQUABIICALBjp4UD8lCGF+pyvIH+Dg4lw62azjPSaYxacHAbObrPpyzVFRaY
# fLH1VGzzflBHE/bpyh/Xzf9D4P9UKQEBxL3ccmKewauWAWtcQuS9sGzZkFwg6oZr
# Cz+gcT6cGoi3gii/0QNrBUeDTiJoA8g4oYYf67jXaSsSwe0xTi1WS/LK7ne31ScJ
# BoK/p69/vBjhsFH2jjYUSNGqjR+jH5xeXlZfcEcN98NwH4PiONQTN5dK2LgNhlfO
# 8oQ/HZpknAOPFeqjKosRNjGso+3QP/I0Q+T/MqzZA3z9xcTBoiCQIsvZz6vCh4N8
# S4ESmXuoMNnkFkq7/djyl26GEBmyj8dCjqd9iBr5+nPWWfHUwQRGfQ9TNuqc5sAW
# REa/UdN2mn14EjEKhj5X2bdYaOPRXkgZ2TeGiyYuklOLoS/veK7FwG6QDXlZ/4+Y
# /lsgVCkdfHcMdnvWa9N1ArF9LV7as/a7uABSwduxmA/zXdOyE3rtEGeO0tuWq3Xk
# xjCxHrntNOgKvs7pDW2KKJPO1xRpMbDS/Q0hSSqYJ//MI7xnV2Jnlm7Ml5BKYA9v
# B7fNFdwBaZQD1yWU45o8OYva7pv1NcnMLf7mX6JVGFuVylDyyhchKAxSH6kCLHkM
# LkXQmlThns8nNkMXCaVD6hip+OIv/ObnB7NsSQVEyC0Nbd6222DYnOWY
