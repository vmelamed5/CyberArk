<#
.Synopsis
   Check if application is in catalog in Identity
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to check if a target application in Identity exists in the Application Catalog
.LINK
   https://vpasmodule.com/commands.html#Test-VPASIdentityApplicationCatalog
.NOTES
   Category: Identity
...
.PARAMETER ApplicationName
   Unique ApplicationName in Identity to query for target ApplicationID
.PARAMETER ApplicationID
   Target ApplicationID that maps the target Application in Identity
   Supply the ApplicationID to skip querying for the target Application
.PARAMETER InputParameters
   HashTable of values containing the parameters required to make the API call
.EXAMPLE
   $TestApplicationStatus = Test-VPASIdentityApplicationCatalog -ApplicationName {APPLICATIONNAME VALUE}
.EXAMPLE
   $TestApplicationStatus = Test-VPASIdentityApplicationCatalog -ApplicationID {APPLICATIONID VALUE}
.EXAMPLE
   $InputParameters = @{
        ApplicationName = "venafi"
   }
   $TestApplicationStatus = Test-VPASIdentityApplicationCatalog -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        ApplicationID = "123456789-9876-abcd-zyxwv-abcdefghijk"
   }
   $TestApplicationStatus = Test-VPASIdentityApplicationCatalog -InputParameters $InputParameters
.OUTPUTS
   $true if successful
   ...
   $false if failed
#>
function Test-VPASIdentityApplicationCatalog{
    [OutputType([bool])]
    [CmdletBinding(DefaultParameterSetName='Set1')]
    Param(

        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Unique application name of the target application (for example: venafi)")]
        [String]$ApplicationName,

        [Parameter(Mandatory=$true,ParameterSetName='Set2',ValueFromPipelineByPropertyName=$true,HelpMessage="Unique application ID of the target application (for example: aec52ec7-1234-abcd-efgh-5678d1fce0ae)")]
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("ApplicationName")
                        MandatoryKeys = @("ApplicationName")
                    }
					set2 = @{
                        AcceptableKeys = @("ApplicationID")
                        MandatoryKeys = @("ApplicationID")
                    }
                }
            ...
            Write-Verbose "FAILED TO TEST APPLICATION IN CATALOG"
        ...
        try{
            ...
            if([String]::IsNullOrEmpty($ApplicationID)){
                Write-Verbose "NO APPLICATION ID PASSED"
                Write-Verbose "INVOKING HELPER FUNCTION TO RETRIEVE APPLICATION ID"

                $ApplicationID = Get-VPASApplicationIDIdentityHelper -token $token -ApplicationName $ApplicationName

                if($ApplicationID -eq -1){
                    $log = Write-VPASTextRecorder -inputval "MULTIPLE APPLICATION ENTRIES WERE RETURNED, ADD MORE TO NAME TO NARROW RESULTS" -token $token -LogType MISC
                    $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
                    Write-VPASOutput -str "MULTIPLE APPLICATION ENTRIES WERE RETURNED, ADD MORE TO NAME TO NARROW RESULTS" -type E
                    Write-VPASOutput -str "RETURNING FALSE" -type E
                    return $false
                }
                elseif($ApplicationID -eq -2){
                    $log = Write-VPASTextRecorder -inputval "NO APPLICATION ENTRIES WERE RETURNED" -token $token -LogType MISC
                    $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
                    Write-VPASOutput -str "NO APPLICATION ENTRIES WERE RETURNED" -type E
                    Write-VPASOutput -str "RETURNING FALSE" -type E
                    return $false
                }
                else{
                ...
                Write-Verbose "APPLICATION ID PASSED, SKIPPING HELPER FUNCTION"
            }
            ...
            Write-Verbose "ADDING APPLICATION ID: $ApplicationID TO PARAMS"
            $log = Write-VPASTextRecorder -inputval $params -token $token -LogType PARAMS
            $params = $params | ConvertTo-Json

            if($NoSSL){
                Write-Verbose "NO SSL ENABLED, USING HTTP INSTEAD OF HTTPS"
                $uri = "http://$IdentityURL/SaasManage/IsApplicationAvailableInCatalog"
            }
            else{
                Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                $uri = "https://$IdentityURL/SaasManage/IsApplicationAvailableInCatalog"
            }
            ...
            if($response.success){
                if($response.Result){
                    $log = Write-VPASTextRecorder -inputval "APPLICATION DOES EXIST IN APPLICATION CATALOG" -token $token -LogType MISC
                    $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: TRUE" -token $token -LogType MISC
                    Write-Verbose "APPLICATION DOES EXIST IN APPLICATION CATALOG"
                    Write-Verbose "RETURNING TRUE"
                    return $true
                }
                else{
                    $log = Write-VPASTextRecorder -inputval "APPLICATION DOES NOT EXIST IN APPLICATION CATALOG" -token $token -LogType MISC
                    $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
                    Write-Verbose "APPLICATION DOES NOT EXIST IN APPLICATION CATALOG"
                ...
                $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
