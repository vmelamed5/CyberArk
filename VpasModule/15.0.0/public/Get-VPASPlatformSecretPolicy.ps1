<#
.Synopsis
   Get platform secret policy
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to retrieve the secret policy set for a platform
.LINK
   https://vpasmodule.com/commands.html#Get-VPASPlatformSecretPolicy
.NOTES
   Category: PAM
   Tag: Platforms
   Since: v15.0.0
...
.EXAMPLE
   $PlatformSecretPolicyJSON = Get-VPASPlatformSecretPolicy -platformID {PLATFORMID VALUE}
.EXAMPLE
   $InputParameters = @{
   ...
   $PlatformSecretPolicyJSON = Get-VPASPlatformSecretPolicy -InputParameters $InputParameters
.OUTPUTS
   If successful:
   {
   ...
   $false if failed
#>
function Get-VPASPlatformSecretPolicy{
    ...
    Process{
        try{
            ...
            Write-Verbose "FAILED TO RETRIEVE PLATFORM SECRET POLICY"
        ...
        try{
            ...
            Write-Verbose "RETURNING PLATFORM SECRET POLICY"
            return $response
        }catch{
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-Verbose "UNABLE TO RETRIEVE PLATFORM SECRET POLICY"
            Write-VPASOutput -str $_ -type E
            return $false
        }
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUFmIBwaNS70i9S07/rCNSYjd2
# WmaggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFMWAc8ky/t58keG0CUvA8O2mdiE4
# MA0GCSqGSIb3DQEBAQUABIICAMCmT/IOJYpSAJeUnVTGyxlMjDa+Q7MTPTI3ee+x
# lL3Cs3I57RdYBi3evBX4ezGZ/IhbyR6FvUt0PpGtraoFJ/snVHJ3afE13TjUYcsW
# HFnhYKgo7K8jTvYRtEM/Dc9Qkkb09ZFgbusk/YOlzK30RJ/F77Rs9ao8slsoHRzU
# Lwa0O7GUMvB26x8b9p/9+HvEVQswSLlJVXBYIsabYXOxWjUdmOLzeFpTO5SS9TyV
# nbrbpiD7mHUFeRwwWPUSn26BxNtdnWQ2TXJcTJ+NXZV6FA74UaBRVGGmifndqNtl
# 8+mgzf6xdAeUq2RqC4o3v9WX75WwxsKnxbCapmrw6jCkn75mujCQj4TTtFvrYDWA
# jtCVKvlllnYXuKOF//UvetDUW+elfutfxYpYUizxfHTUGtT75JnwW/uSqlC4myYI
# CxbblgZp0Vfn9qwloqf81tUcpHNp9jbBIPzS8ksF9xwlX+IdTYP2y83kAw0yt6Pv
# H7b14Qz4ETF7VhsstSCtumfDzWS6FnPEUr2B0tCEMe/EhLk6u850vDV7HkBjlgTS
# oK+5wys9+mo+f+DixpTDrMjokDDFgooODQ0Iv9mtYDhixlXRKmze+Stfpslb7VgY
# nd7sGf5p3BzHZxXVPqeuLTry3vXQOWZ/12hgohh56Pj73HZHK3lGo5gTlDLjkqsW
# y+HAoYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkGA1UEBhMC
...
# 9w0BCQUxDxcNMjYwMjA0MDQ0OTQ1WjA/BgkqhkiG9w0BCQQxMgQwFagrOmWTicTN
# BwH5sOQxo8uS94M7Vs1g+VWabjVadG3MFwkVAC5v/YYPQNokT+ZUMA0GCSqGSIb3
# DQEBAQUABIICADF1GNY9gS9/LlU/gKPGqUO+XRyctkFn/JPMseumnel06CkW5eDp
# I391ci+Zk1tkhJy+0sFhUrlfvHeJBqYbNoxANT/Hp1HMbVZCK1YtfU/J4+YYGYqu
# GSKRg/f9CIlL0l2aqKyLnjzBi/EpSmN/wN1D/qdPBCIhrXukvf/0b7rsuDOZOR1y
# nCj1RPqyyrzOTqkB5dLaxoBpXoyqmLsRDv8Oysje3JJU8Fe/BWuJno0kWQTOGPBp
# UxhknNSxDMDybeXlwmyNR+4VMa0FK0SiKFBkSv0K9g/v2qtdMxQzR6YtFzWr+Mx1
# HaMvPQUD2Ig3zuJvzD+vqdoc7odBO2eyeAiSk7HzHgm4PsGIc5NsLa1fgX3LYQ+0
# Hf3gKgLzEhLpZutZuk6ipqz7KMWLmOug9dOirKyPpKrCu9gCM5agTULHtUA6Pq53
# cy33h/8soUop+U+82CYGQvPF+qyJj5+4MvWTGgVM09XrltgZCiHwoGc9OquoNgbd
# OUGM0Mk+RkvJHA7dUht3g7lOqFA7fabu+GGDD6PEL4clFKJPyUCm5iGDAXpQnVMl
# /Udm3vmvCwyapJIz3AlM1zcVTCMoSiPyHEJNe0k9wrAauaaPo28GVylkdVW9TMgc
# lrxeIoIB5t+QfAvpfINwQxc6puXzKM5sxPmOn3HYwN0ORIv+iGAhB0Mc
