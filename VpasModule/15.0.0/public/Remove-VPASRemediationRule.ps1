...
.DESCRIPTION
   Use this function to delete a remediation rule
.LINK
   https://vpasmodule.com/commands.html#Remove-VPASRemediationRule
.NOTES
   Category: PAM
...
.PARAMETER RemediationRuleID
   Unique ID that maps to a single remediation rule, passing this variable will skip any query functions
.PARAMETER SearchQuery
   Search string that will be used to query for the target remediation rule if no RemediationRuleID is passed
.PARAMETER WhatIf
...
.EXAMPLE
   $DeleteRemediationRuleStatus = Remove-VPASRemediationRule -searchquery {SEARCH QUERY VALUE}
.EXAMPLE
   $DeleteRemediationRuleStatus = Remove-VPASRemediationRule -RemediationRuleID {REMEDIATION ID VALUE}
.EXAMPLE
   $InputParameters = @{
        RemediationRuleID = 3
        WhatIf = $true|$false
   ...
   $DeleteRemediationRuleStatus = Remove-VPASRemediationRule -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
...
.OUTPUTS
   ...
   $false if failed
#>
function Remove-VPASRemediationRule{
    ...
    Param(
        ...
        [Parameter(Mandatory=$true,ParameterSetName='Set2',ValueFromPipelineByPropertyName=$true,HelpMessage="Unique RemediationRuleID that references a unique remediation rule (for example: 1234-abcd-12345678-abc123-123456789abcd)")]
        [String]$RemediationRuleID,
        ...
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    ...
                    set2 = @{
                        AcceptableKeys = @("RemediationRuleID","WhatIf","HideWhatIfOutput")
            ...
            Write-Verbose "FAILED TO REMOVE REMEDIATION RULE"
            Write-VPASOutput -str $_ -type E
            return $false
        }

        try{
            if([String]::IsNullOrEmpty($RemediationRuleID)){
                Write-Verbose "NO REMEDIATION RULE ID PROVIDED...INVOKING HELPER FUNCTION TO RETRIEVE UNIQUE REMEDIATION RULE ID BASED ON SPECIFIED PARAMETERS"
                $RemediationRuleID = Get-VPASRemediationRuleIDHelper -token $token -SearchQuery $SearchQuery

                if($RemediationRuleID -eq -1){
                    $log = Write-VPASTextRecorder -inputval "COULD NOT FIND UNIQUE REMEDIATION RULE ENTRY, INCLUDE MORE SEARCH PARAMETERS" -token $token -LogType MISC
                    $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
                    Write-Verbose "COULD NOT FIND UNIQUE REMEDIATION RULE ENTRY, INCLUDE MORE SEARCH PARAMETERS"
                    Write-VPASOutput -str "COULD NOT FIND UNIQUE REMEDIATION RULE ENTRY, INCLUDE MORE SEARCH PARAMETERS" -type E
                    return $false
                }
                elseif($RemediationRuleID -eq -2){
                    $log = Write-VPASTextRecorder -inputval "NO REMEDIATION RULES FOUND" -token $token -LogType MISC
                    $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
                    Write-Verbose "NO REMEDIATION RULES FOUND"
                    Write-VPASOutput -str "NO REMEDIATION RULES FOUND" -type E
                    return $false
                }
                else{
                    Write-Verbose "RETURNING REMEDIATION RULE ID: $RemediationRuleID"
                }
            }
            else{
                Write-Verbose "REMEDIATION RULE ID SUPPLIED, SKIPPING HELPER FUNCTION"
            }

            if($NoSSL){
                Write-Verbose "NO SSL ENABLED, USING HTTP INSTEAD OF HTTPS"
                $uri = "http://$PVWA/api/discovery-rule-sets/$RemediationRuleID"
            }
            else{
                Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                $uri = "https://$PVWA/api/discovery-rule-sets/$RemediationRuleID"
            }
            ...
            if($WhatIf){
                ...
                $WhatIfInfo = Get-VPASAllRemediationRules -token $token
                foreach($rule in $WhatIfInfo.value){
                    $ruleId = $rule.id
                    $ruleName = $rule.name
                    $ruleStatus = $rule.status
                    $rulerulesCount = $rule.rulesCount
                    $ruleOnboardedActions = $rule.actionsPerformed.ONBOARD
                    $ruleMarkForDeletionActions = $rule.actionsPerformed.MARK_FOR_DELETION
                    $ruleDeleteActions = $rule.actionsPerformed.DELETE
                    $ruleMarkThenDeleteActions = $rule.actionsPerformed.MARK_THEN_DELETE
                    $rulelastModificationTime = $rule.lastModificationTime

                    if($ruleId -eq $RemediationRuleID){
                        if(!$HideWhatIfOutput){
                            Write-VPASOutput -str "====== BEGIN COMMAND SIMULATION ======" -type S
                            Write-VPASOutput -str "THE FOLLOWING REMEDIATION RULE WOULD BE DELETED:" -type S
                            Write-VPASOutput -str "Id                      : $ruleId" -type S
                            Write-VPASOutput -str "Name                    : $ruleName" -type S
                            Write-VPASOutput -str "Status                  : $ruleStatus" -type S
                            Write-VPASOutput -str "RulesCount              : $rulerulesCount" -type S
                            Write-VPASOutput -str "OnboardedActions        : $ruleOnboardedActions" -type S
                            Write-VPASOutput -str "MarkForDeletionActions  : $ruleMarkForDeletionActions" -type S
                            Write-VPASOutput -str "DeleteActions           : $ruleDeleteActions" -type S
                            Write-VPASOutput -str "MarkThenDeleteActions   : $ruleMarkThenDeleteActions" -type S
                            Write-VPASOutput -str "lastModificationTime    : $rulelastModificationTime" -type S
                        ...
                        $WhatIfHash = @{
                            WhatIf = @{
                                ...
                                name = $ruleName
                                status = $ruleStatus
                                rulesCount = $rulerulesCount
                                OnboardedActions = $ruleOnboardedActions
                                MarkForDeletionActions = $ruleMarkForDeletionActions
                                DeleteActions = $ruleDeleteActions
                                MarkThenDeleteActions = $ruleMarkThenDeleteActions
                                lastModificationTime = $rulelastModificationTime
                                RestURI = $uri
                                RestMethod = "DELETE"
                                Disclaimer = "THIS REMEDIATION RULE WILL BE DELETED IF -WhatIf FLAG IS REMOVED"
                            }
                        }
                ...
                Write-Verbose "REMEDIATION RULE WAS SUCCESSFULLY DELETED FROM CYBERARK"
                return $true
            }
        }catch{
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-VPASOutput -str $_ -type E
            Write-Verbose "UNABLE TO DELETE REMEDIATION RULE FROM CYBERARK"
            return $false
        }
    }
    End{
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUeWcuYccDf2g0hBiSLKQNecyK
# QouggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYE