<#
.Synopsis
   Get ConnectorManagement connector component log list
...
.LINK
   https://vpasmodule.com/commands.html#Get-VPASCMComponentLogList
.NOTES
...
.EXAMPLE
   $ComponentLogList = Get-VPASCMComponentLogList -ConnectorID {CONNECTOR ID VALUE} -ComponentName {COMPONENT NAME VALUE}
.EXAMPLE
   $ComponentLogList = Get-VPASCMComponentLogList -ConnectorHost {CONNECTOR NAME VALUE} -ComponentID {COMPONENT ID VALUE}
.EXAMPLE
...
.EXAMPLE
   ...
   $ComponentLogList = Get-VPASCMComponentLogList -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        ConnectorID = "ManagementAgent_fslkdj34890-1234-1234-asdf-0239kdf"
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("ConnectorID","ComponentID","ExportToCSV","CSVDirectory")
                        MandatoryKeys = @("ConnectorID","ComponentID")
                    }
					set2 = @{
                        AcceptableKeys = @("ConnectorID","ComponentName","ExportToCSV","CSVDirectory")
                        MandatoryKeys = @("ConnectorID","ComponentName")
                    }
					set3 = @{
                        AcceptableKeys = @("ConnectorHost","ComponentID","ExportToCSV","CSVDirectory")
                        MandatoryKeys = @("ConnectorHost","ComponentID")
                    }
					set4 = @{
                        AcceptableKeys = @("ConnectorHost","ComponentName","ExportToCSV","CSVDirectory")
            ...
            Write-Verbose "FAILED TO RETRIEVE COMPONENT LOG LIST"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUOQ6qZ5HX0xddJZOTRBw3reWh
# jruggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFKvV0Z7azinWWN994Yf2MJpRmQBi
# MA0GCSqGSIb3DQEBAQUABIICABfoBDLWcrrDB8bDle1vUCtrn+uwTh8FYS3mDLWc
# RfQOQsovIbawUxIx3mCwFt8SxC5/8vxD4tgiLIwZOSqxFeAgcwld2IzmG1NJaBfQ
# 2FozDK2y7nMCQD9lOrZzqIM8stxwUV/JgytJ0HjDrkPeEpTAoiSqI/+BAOg/yLuN
# nqm544SIE6gmb2AOOXGrCIjjKXszRa5qQ72S6UDgvq5KahWHq1OIIv9rJyEkfawk
# wNdYI+dfOzoA8eeX1V2fNdCahHeo0mLEhjTocTJRzK91g4Y0SUvumTKUAHSgHLKH
# LD3Ew0pI+OnXv7TxGIy6urTbaaCN5uQ6x7TbovauYcTsh+R1UrTRPbYAsplmcI2y
# zPI/y3d66Sfig6+JG4fjijbhLCO8g/8ZYzOgwveGMbomzDPD7gaJHZWP0ED9ILz0
# Q9HvpVDEZDj6RFaTIXPhCffvtieQbNdVdAjvQFa0nXbNPritoGKX2fWw7VyQAwPh
# QKqc3/71vWffDKB1n4xHd48xTynjnmiTyPnodPWIdV7JsLU2sLmtfeOWlKK5hqIg
# 7B3EFyFNsYUIDNamTdzjvFCprPgsRc6LjA3Xo6E2aVgpHlZdwHJf0MkTy/Nicpm+
# WosiEwZaOB1gkMwPjceDRUdsWcv5DnpvX9SGslO8cmmEi3GSg/7+boPwJ47GXjHF
# GC6goYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkGA1UEBhMC
...
# 9w0BCQUxDxcNMjYwMjA0MDQ0NzE1WjA/BgkqhkiG9w0BCQQxMgQwA8/8W/fN/HYo
# W1/Q5Iwkgfhar1Q7fdXMAQNNSvZG1pmS2LBIiJ1FML8udy3TF26hMA0GCSqGSIb3
# DQEBAQUABIICAFNV50X110Ck+1smj/0krvx+LL5HZe7jSIub2v4681eVVSYKr/jG
# BLXj367uLYv31hj7FV34JS5L66xFlB4DWA2psJ6aWZ40KrFHrIBitsU46jgNHB7N
# xTb9UeiQcsGFqcdKC/azRZGrMV90qw0osXibfVLEiXukYHw/AhVGMxw7TaK0aNoP
# TMm397/C811CAFqeGTmBmW15n4CJIzqChg3oPKdZfHBs82PtHbtlg6dCaxSoyXK6
# OpPM4ouoGdUB9OLEqnVw1N1ag/VOUgNHIoonV0SSDWq9EHZ6wa9wY//7OkQZVo6C
# gIvQJRrKZhSXG0W7LKkImbISPOUNhYoJfVWKd3GpAuxFTLzVsuNxTsIFusYOuusV
# 2vl3F2b2vvCTNRqpDLeMvuzdEvPd4MHaxhoXgbufWdXBAPEbmrA2Ei1XOrJQWEIc
# 3KQ0Xp1vm94DR1zBMla/cPk7xndDFfZ6hQdlw4LFE4/fDQfaKmWM+jw3dRTHexY2
# osCMo0jf/QH5QYfhsQnoW5CRUfzsa1cRIsN2k4R2YzK2mmJGftnGMweUK/8xqazt
# f5i7ckl/BaW3rGhfq1ioSL757qyAp+Yhx98cotbjb+5bDMUV9AMh4Qwh9eqpm2Ag
# NRIzx246Rm4Lc/hLdIT1qVtOWz+7GrEczaU9FGWj5sXrxlhiUYznp3PM
