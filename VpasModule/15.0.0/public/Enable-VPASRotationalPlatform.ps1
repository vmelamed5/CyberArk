...
.DESCRIPTION
   Use this function to activate a disabled rotational platform
.LINK
   https://vpasmodule.com/commands.html#Enable-VPASRotationalPlatform
.NOTES
   Category: PAM
...
.EXAMPLE
   $InputParameters = @{
        ActivateRotationalPlatformID = "TargetRotationalPlatformID"
   }
   $ActivateRotationalPlatformStatus = Enable-VPASRotationalPlatform -InputParameters $InputParameters
.OUTPUTS
   $true if successful
   ---
   $false if failed
...
function Enable-VPASRotationalPlatform{
    ...
    Param(
        ...
        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="PlatformID of target platform to be enabled (for example: RotationalPlat)")]
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("ActivateRotationalPlatformID")
                        MandatoryKeys = @("ActivateRotationalPlatformID")
                    }
                }
            ...
            Write-Verbose "FAILED TO EXPORT ROTATIONAL PLATFORM"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUuM3sk8u404T3luOgq/2f2IcE
# ibOggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFDKmGJzEQyosmdCjwrmLc/l9Z1iU
# MA0GCSqGSIb3DQEBAQUABIICADeY1Z3LwQjNmqyfu252NxN9isbuOKezKRkw1lbj
# eSSqKpqj/kz6P+zIVIa1nSLAP92H/n0f2G+npKydyva9/Tfg2gtTINs7U8JGYoRf
# kGkG0ajrhIV1t6zihIk5mLsSf4dCExtoL/TM3t2AMrQoIL2uhbrpfATQwB7BFD0V
# QdJO2X7ICwyQsSEl8GHebwQExvwYHKFp4Ld6c5z0QSL9gSGv1UTmdn5uXuNb5MjJ
# 8swLrQ7VqUAA5UirDxrY9ZCh9NIJxrE4dej6oqHkoo30ZRQLB+N+whcSCNB1I8kE
# /xARQFd/FdTqhqvmQFp8yDmMisXf/fNvCgbef8c4RiwQu3y01DdaHA/ShQFruUso
# 39am3mNTy5VUjsRqnGkWaYjPKZFfqbq3rP8XeZ+PzT3C5+PpjjdPu4yaEUIrQ/q/
# JQLPF4gtQqLKhpy6zrboyk+BBbyaXxJ7nPTVTxhQL0ef0FlZQNf8X9vRTRSkqo+R
# HxHomljIJlLZcL8BK+dGnV0a/bF5mSxq6DY+WiaNVDma5mrgVm60sYGrwpizZ28j
# YuK3aLPIz2crrLVYNCOa+K+FKp2vGSQkJmHYs7v7TwAQGAq2rVSlNAXPirNkTgKZ
# kxLCkKf1qemKohN7t7G2RAzSTuAqUofUv/HyTEYP2OiTBSkMW+JetV7MSRVuTc7r
# 5qNDoYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkGA1UEBhMC
...
# 9w0BCQUxDxcNMjYwMjA0MDQ0NDM1WjA/BgkqhkiG9w0BCQQxMgQwPYLULqgHpOtT
# vQMQSO+rOzYw6LZyM0E5u7h90ATKH0w2apQMj6Mmjg+INYm+J0XtMA0GCSqGSIb3
# DQEBAQUABIICAFJsQg4xwRYM8rTPe1x+R6FcAO+lZFUrQDd5I5n0NdplGaUb4/Tx
# gJMpnw66miDu5Ruh+mpk7P5FQOOZhGAB/6PIxidBbCEGYWptwQsx+GOkK6YXHFzq
# NHVJ1Kb0venMFY1Bj5u5gW3mmi6hBFFiaZmWi4B2ua6wt6rf27KBqV5wkT9j+CgA
# DseOw+zlXwMwOF5Ft6DfLMMBnF2Ij7euLlAzzYCSHmIcX3kLfKUmhcDH6aiCK0iC
# Q9n/l79n2Qg7Gcb013Pr5znBodj0A2YNkHxc7OMJANTxPA+yBMmhFFb018Nk6T2r
# XENoa6IopQv1j6oPV8Sq3WmnJg4ucnz0MeK07xsO7Y2zAFCbl4ePdz7W4nJJJZoR
# MICyL5p7klkyc/Dz+65sIuslbEMGf6AMXRdBwFxF9JEPq2dAB0t8eLjMomxsHl5t
# wuNTrTZbiA9aKQXSliyR6QfqNg/42MYmNAT4/96ubQVGDvRAs/2HXVWNVn40Ws3a
# QInrGUHxxMdR7+ZOsbLEMmAfgbfkyiDOm+G/w7o12ZJ+TiTxFm3BrJ1ETf/+wHr1
# 0xo9cikPxsg8UDEZwU8gLARr78uE5ZJITwtB+ZA8pI995PqA4hKawO2VohTRTovw
# zwlTcArXewPcBhIXoOhn+p2qB7ABj1o/p4C9IfvufBKZELDRDW6KBW+H
