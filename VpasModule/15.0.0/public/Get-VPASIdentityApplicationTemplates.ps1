<#
.Synopsis
   Get application templates from Identity
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to retrieve application templates from Identity
.LINK
   https://vpasmodule.com/commands.html#Get-VPASIdentityApplicationTemplates
.NOTES
   Category: Identity
...
.PARAMETER TemplateName
   Unique TemplateName in Identity to query for target application template
...
.EXAMPLE
   $ApplicationTemplateDetailsArray = Get-VPASIdentityApplicationTemplates -TemplateName {TEMPLATENAME VALUE}
.EXAMPLE
   $InputParameters = @{
   ...
   $ApplicationTemplateDetailsArray = Get-VPASIdentityApplicationTemplates -InputParameters $InputParameters
.OUTPUTS
   If successful:
   {
        "Entities":  [
        ...
        "Row":  {
                    ...
                    "Description":  "Venafi is the cybersecurity market leader and inventor of machine identity management, securing machine-to-machine connections and communications.",
                    ...
                    "ParentApp":  null,
                    "DeepLinkUrl":  null,
                    "Icon":  "/vfslow/lib/application/icons/Venafi.png",
                    "PolicyScript":  null,
                    ...
                    "AuthChallengeDefinitionId":  null,
                    "IsIdentityAsIdpEnabled":  null,
                    "Handler":  "cloudlib;Centrify.Saas.apphandlers.BrowserExtensionScriptHandler",
                    "WebUPAppType":  "BrowserExtensionScript",
                    "State":  null,
                    "NotSelfService":  null,
                    "IsScaEnabled":  null,
   ...
   $false if failed
#>
function Get-VPASIdentityApplicationTemplates{
    [OutputType('System.Object',[bool])]
    [CmdletBinding(DefaultParameterSetName='Set1')]
    Param(

        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Template name of the target template (for example: Venafi)")]
        [String]$TemplateName,
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("TemplateName","ExportToCSV","CSVDirectory")
                        MandatoryKeys = @("TemplateName")
                    }
                }
        ...
        try{
            ...
            if($response.success){
                foreach($rec in $result.Results){
                    $rowName = $rec.Row.Name
                    $rowDisplayName = $rec.Row.DisplayName
                    $rowID = $rec.Row.ID
                    $rowDescription = $rec.Row.Description
                    $rowCategory = $rec.Row.Category

                    if($rowName -match $TemplateName -or $rowDisplayName -match $TemplateName -or $rowID -match $TemplateName -or $rowDescription -match $TemplateName -or $rowCategory -match $TemplateName){
                        $resultoutput += $rec
                        Write-Verbose "FOUND MATCHING TEMPLATE...ADDING TEMPLATE TO RETURN VALUES"
                    }
                }

                if($ExportToCSV){
                    $runexport = Write-VPASExportCSV -Data $resultoutput -CommandName $CommandName -CSVDirectory $CSVDirectory
                }

                $log = Write-VPASTextRecorder -inputval $resultoutput -token $token -LogType RETURN
                Write-Verbose "RETURNING APPLICATION TEMPLATES DATA"
                return $resultoutput
            }
            else{
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU0U7Zk8Ho4Ii55jN36kyaeGyq
# f0iggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFESSUqrHNpQmEX+NP8Do/2qzsXtL
# MA0GCSqGSIb3DQEBAQUABIICAGkovPqq7352IoWHqCi131sGM9627hHqFRwEIVId
# PxtNsRmQw3ixaK9Ze7fg4Rrur1JgjCyh7UTGTcRlM0aybei5yZHLlmP2qgnisTa2
# 7+bhMjuqk7+XSqLFO3XXMCZyHuehMAHU2DewNqo/c8ED70pAGUwyaQ6Nxzo2LuBA
