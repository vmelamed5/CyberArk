<#
.Synopsis
   Get account request ID
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Helper function to retrieve account request IDs from CyberArk
#>
function Get-VPASAccountRequestIDHelper{
    ...
    Process{
        try{
            Write-Verbose "RETRIEVING ALL ACCOUNT REQUESTS"
            $FoundRequests = @()
            $AllAccountRequests = Get-VPASAllAccountRequests -token $token
            foreach($req in $AllAccountRequests.MyRequests){
                $curMatch = $true
                $requestID = $req.RequestID
                $requestAcctID = $req.AccountDetails.AccountID
                $requestReason = $req.UserReason
            ...
            if($counter -gt 0){
                Write-Verbose "FOUND TARGET ACCOUNT REQUESTS THAT MATCH THE SEARCHQUERY"
                $log = Write-VPASTextRecorder -inputval "FOUND TARGET ACCOUNT REQUESTS THAT MATCH THE SEARCHQUERY" -token $token -LogType MISC -Helper
                $log = Write-VPASTextRecorder -inputval $FoundRequests -token $token -LogType RETURN -Helper
                return $FoundRequests
            }
            else{
                Write-Verbose "NO ACCOUNT REQUESTS FOUND"
                Write-VPASOutput -str "NO ACCOUNT REQUESTS FOUND" -type E
                $log = Write-VPASTextRecorder -inputval "NO ACCOUNT REQUESTS FOUND" -token $token -LogType MISC -Helper
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUQRnCUsq6Q2h3asSFDKFso9jb
# nmCggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFMeIbp32jB6GwCKQ3RzRzUKOgkFj
# MA0GCSqGSIb3DQEBAQUABIICAHB00IbMFYF2Q5cL05C8yuZsxZvbYa0BvgARzo+i
# eSKSpb04oD5ps8tpr4hiVvCS9r0UANcBJndvl/d83EnKmy9a2XtR5kWnz1ztc3Ok
# 6pgh5+aw2vNDGsQ4N7wABR41WyshL7qZ8/B8l9h4vmuP+2jEdzWWQEJDK0/j9NOX
# Fu4qL/Ic59L63C3/8NzTHVOHBzgOTXAVkuuVhWf14cws0UZJiVIEBl7Snv6IHuNl
# vuRIOWnJmYyYmgS+qXXDtEsaUJCl2V7rvzWCZ46zBJORz9MWpWrlN9+s4nn+dmfb
# JHh0gf12nSzIaGD92zU/flT4XN+wPyMZJ8g1K/VfjPPu88wBGival0BnOaef3Kta
# KPXdANDb4L3UF1gglwF2qIpWNTYU8DKH3TDKBJwnLi2N7DLUMtUOR1TLrrfVpSKr
# nDa+E+SURUpMfM4dI5Xmk4X6dRGtJBdtK7W/QQdptB/sCta7Nzvn6xc9Jun97vNl
# oVLbiPkA+GKngWsxoLSFQ8YWJPTGCnf4pfGmi3M/5YULFRS+kRC9d4oeupS/1ppn
# QTlmi20UHP+RatZ/AZfEx3li6CmkC+ynLT0esyy0GaL93gepGS7SsscOUYi8eKVX
# DrnPsNpyqE3t7JaPuYo3B/MPcmZ0IV+69mQ0C8PkqLCnM2bkJLoMaYIw8jGP3Fl2
# QTrSoYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkGA1UEBhMC
...
# 9w0BCQUxDxcNMjYwMjA0MDQ0MDUwWjA/BgkqhkiG9w0BCQQxMgQwBiYSyqppfy7W
# BaPrTqlAK7eNERSNYW+gipRYhobeiwIlEVIaFZ76sx18NZS44MI9MA0GCSqGSIb3
# DQEBAQUABIICALZ+1GhpygrjfAQaKnWVJgxAX15HZUMXpYBoJKWy9Xux+NjX0sYX
# lL/yrJMTeeT8YHYjZ4osEuIoNCCym3rIEjuDV0IltUdGGd8hxX8C2nMMtNQ/u7ak
# GWkqkX6W4IpvzhXsdXXsITzDquVP2sDJyq/QmzkmupBwLU7SM8IRBC6EGKjiFV3Q
# vgiy7XVaV+cMDKcm9QMdPyRq7wIVaPoSc96gEy8utWcVJCoG68a3902gg5G53icc
# xzGCUxw/yeH+Yg+271XWcnUySrQ31avfGEIJ38w41iJvHjr8/CYNWqzxhCXClSFk
# M56Tiar7ucH08dTcs5v1mULjEN5GrUjwfb8vGLm3fI9aIOi0206HduWDsvHgyzaZ
# OnisPbxRjEItzDWDaUGKnUBBx8bLp/YaXWlObZdmLdBHpljR+3QKZcoTiD/17YuD
# pCiAYJt+6hkGMTN+mzkWXv3ZIhlL53KfGyPfSIwh5knxj3acVY9mg+BkJFN37Pxi
# f4TuWkKkq12g+nt8uSoNvzFT7Aa/Sq86ul+qBFqWzPnQky272FWYkJLZyLZuzzbM
# uz8X4GtWfPIA71to/FkKfBRT1IZUAIWsaWwvoOPYuXHr5+yZL1s4nsfUYwiBi8V2
# gzG3nc5H7QcOcJ7HBCYzfczoAm3nunruiWukDdkBi6tQwBO21ytjlb8K
