<#
.Synopsis
   Get onboarding rule ID
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Helper function to retrieve onboarding rule IDs from CyberArk
#>
function Get-VPASOnboardingRuleIDHelper{
    ...
    Process{
        try{
            ...
            Write-Verbose "FETCHING ALL ONBOARDING RULES"
            $AllOnboardingRukes = Get-VPASAllOnboardingRules -token $token
            foreach($rule in $AllOnboardingRukes.AutomaticOnboardingRules){
                $matched = $false
                $RuleId = $rule.RuleId
                ...
                $UserNameMethod = $rule.UserNameMethod
                $AddressFilter = $rule.AddressFilter
                $AddressMethod = $rule.AddressMethod
                $AccountTypeFilter = $rule.AccountTypeFilter
                $AccountCategoryFilter = $rule.AccountCategoryFilter
                $ReconcileAccountId = $rule.ReconcileAccountId
                $LastOnboardedTime = $rule.LastOnboardedTime

                if(!$matched){
                    if($RuleName -match $SearchQuery){
                        Write-Verbose "ONBOARDING RULE MATCHED ON RuleName...ADDING TO OUTPUT"
                        $output.Value += $rule
                        $matched = $true
                    }
                }
                ...
                if(!$matched){
                    if($RuleDescription -match $SearchQuery){
                        Write-Verbose "ONBOARDING RULE MATCHED ON RuleDescription...ADDING TO OUTPUT"
                        $output.Value += $rule
                        $matched = $true
                    }
                }
                ...
                if(!$matched){
                    if($TargetPlatformId -match $SearchQuery){
                        Write-Verbose "ONBOARDING RULE MATCHED ON TargetPlatformId...ADDING TO OUTPUT"
                        $output.Value += $rule
                        $matched = $true
                    }
                }
                ...
                if(!$matched){
                    if($TargetSafeName -match $SearchQuery){
                        Write-Verbose "ONBOARDING RULE MATCHED ON TargetSafeName...ADDING TO OUTPUT"
                        $output.Value += $rule
                        $matched = $true
                    }
                }
                ...
                if(!$matched){
                    if($MachineTypeFilter -match $SearchQuery){
                        Write-Verbose "ONBOARDING RULE MATCHED ON MachineTypeFilter...ADDING TO OUTPUT"
                        $output.Value += $rule
                        $matched = $true
                    }
                }
                ...
                if(!$matched){
                    if($SystemTypeFilter -match $SearchQuery){
                        Write-Verbose "ONBOARDING RULE MATCHED ON SystemTypeFilter...ADDING TO OUTPUT"
                        $output.Value += $rule
                        $matched = $true
                    }
                }
                ...
                if(!$matched){
                    if($UserNameFilter -match $SearchQuery){
                        Write-Verbose "ONBOARDING RULE MATCHED ON UserNameFilter...ADDING TO OUTPUT"
                        $output.Value += $rule
                        $matched = $true
                    }
                }
                ...
                if(!$matched){
                    if($AddressFilter -match $SearchQuery){
                        Write-Verbose "ONBOARDING RULE MATCHED ON AddressFilter...ADDING TO OUTPUT"
                        $output.Value += $rule
                        $matched = $true
                    }
                }
                ...
                if(!$matched){
                    if($AccountCategoryFilter -match $SearchQuery){
                        Write-Verbose "ONBOARDING RULE MATCHED ON AccountCategoryFilter...ADDING TO OUTPUT"
                        $output.Value += $rule
                        $matched = $true
                    }
                }
            ...
            $counter = $output.value.count
            if($counter -gt 1){
                #Write-VPASOutput -str "MULTIPLE ENTRIES FOUND, ADD MORE SEARCH FIELDS TO NARROW DOWN RESULTS" -type E
                Write-Verbose "MULTIPLE ONBOARDING RULES WERE RETURNED, ADD MORE SEARCH FIELDS TO NARROW RESULTS"
                $log = Write-VPASTextRecorder -inputval "MULTIPLE ONBOARDING RULES WERE RETURNED, ADD MORE SEARCH FIELDS TO NARROW RESULTS" -token $token -LogType MISC -Helper
                $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: -1" -token $token -LogType MISC -Helper
                return -1
            }
            elseif($counter -eq 0){
                Write-Verbose "NO ONBOARDING RULES FOUND"
                Write-VPASOutput -str "NO ONBOARDING RULES FOUND" -type E
                $log = Write-VPASTextRecorder -inputval "NO ONBOARDING RULES FOUND" -token $token -LogType MISC -Helper
                $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: -2" -token $token -LogType MISC -Helper
                return -2
            }
            else{
                write-verbose "FOUND UNIQUE ONBOARDING RULE ID"
                Write-Verbose "RETURNING UNIQUE ONBOARDING RULE ID"
                $logoutput = $output.value
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUjjrC08byVw33afI7QDEh4iO8
# fU6ggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFIxOjsa3B5y7RJgEUpRjtekjD9dx
# MA0GCSqGSIb3DQEBAQUABIICAES40nW5gHqGdw/uJpxuviCmD+lu/h8JgGP3/MK7
# n7cmINruCoQCPjAv+XrLJ62yPt9n6tBLjTriH4NEVUSba6fF2ZPJWbaMkz+4PSYa
# Ada0ILfQvhKMp+yemGGIoSseUjaQmoI8Y0mGEZfM+bPsAS5uHLo64Kh2qsnTv/3G
