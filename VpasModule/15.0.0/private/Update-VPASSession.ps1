...
.DESCRIPTION
   Helper function to update current session variables and reauthenticate if needed
#>
function Update-VPASSession{
    ...
    Process{
        try{
            $active = if ($PSBoundParameters.ContainsKey('InputToken') -and $InputToken) { $InputToken } else { $script:VPAStoken }
            if(-not $active){
                Write-VPASOutput -str "CREATE A SESSION TOKEN BY RUNNING New-VPASToken" -type E -Initialized
                return $false
            }

            $APICounter = [int]$active.APICounter
            $APICounter = $AddCounter

            $threshold = 900
            if($APICounter -gt $threshold){
                Write-Verbose "APPROACHING API LIMIT...BEGINNING REAUTH"
                Write-VPASOutput -str "APPROACHING API LIMIT...BEGINNING REAUTH" -type M

                $tokenval = $active.token
                $sessionval = $active.session
                $PVWA = $active.pvwa
                $Header = $active.HeaderType
                $ISPSS = $active.ISPSS
                $IdentityURL = $active.IdentityURL
                $EnableTextRecorder = $active.EnableTextRecorder
                $AuditTimeStamp = $active.AuditTimeStamp
                $NoSSL = $active.NoSSL
                $VaultVersion = $active.VaultVersion
                $HideWarnings = $active.HideWarnings
                $AuthenticatedAs = $active.AuthenticatedAs
                $SubDomain = $active.SubDomain
                $EnableTroubleshooting = $active.EnableTroubleshooting
                $APICounter = $active.APICounter
                $Creds = $active.Creds
                $AuthType = $active.AuthType
                $IDPLogin = $active.IDPLogin
                $SRS = $active.SRS

                if($PVWA -eq "Identity_Only"){
                    $new = New-VPASToken -PVWA $PVWA -AuthType $AuthType -IdentityURL $IdentityURL -creds $creds -EnableTextRecorder:$EnableTextRecorder -HideAscii -NoSS:$NoSSL -InitiateCookie:$sessionval -IDPLogin:$IDPLogin -HideWarnings:$HideWarnings -IdentityOnly
                }
                else{
                    $new = New-VPASToken -PVWA $PVWA -AuthType $AuthType -IdentityURL $IdentityURL -creds $creds -EnableTextRecorder:$EnableTextRecorder -HideAscii -NoSS:$NoSSL -InitiateCookie:$sessionval -IDPLogin:$IDPLogin -HideWarnings:$HideWarnings
                }

                if($new -and $new -ne $false){
                    if($PSBoundParameters.ContainsKey('InputToken') -and $InputToken){
                        $InputToken.Clear()
                        ...
                        $script:VPAStoken = $new
                        $active = $script:VPAStoken
                    }
                }
                else{
                    if($PSBoundParameters.ContainsKey('InputToken') -and $InputToken){
                        $InputToken.APICounter = $APICounter
                    }
                    else{
                        $script:VPAStoken.APICounter = $APICounter
                    }
                    return $false
                }
            }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUvX7rrMyDO+W/36Tm2kU13iHk
# Q4mggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEF