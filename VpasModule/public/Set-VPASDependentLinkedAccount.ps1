<#
.Synopsis
   Link a dependent account
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to link an account to a target account (ReconcileAccount, LogonAccount, JumpAccount)
.LINK
   https://vpasmodule.com/commands.html#Set-VPASDependentLinkedAccount
.NOTES
   Category: PAM
...
.PARAMETER LinkedAcctID
   Unique ID that maps to a single account to link
.PARAMETER InputParameters
   HashTable of values containing the parameters required to make the API call
.EXAMPLE
   $LinkAcctActionStatus = Set-VPASDependentLinkedAccount -AccountType {ACCOUNTTYPE VALUE} -safe {SAFE VALUE} -platform {PLATFORM VALUE} -username {USERNAME VALUE} -address {ADDRESS VALUE} -DependentSearchQuery {DEPENDENT SEARCHQUERY VALUE} -LinkedAcctID {LINKED ACCTID VALUE}
.EXAMPLE
   $LinkAcctActionStatus = Set-VPASDependentLinkedAccount -AccountType {ACCOUNTTYPE VALUE} -AcctID {ACCTID VALUE} -DependentSearchQuery {DEPENDENT SEARCHQUERY VALUE} -LinkedAcctID {LINKED ACCTID VALUE}
.EXAMPLE
   $LinkAcctActionStatus = Set-VPASDependentLinkedAccount -AccountType {ACCOUNTTYPE VALUE} -safe {SAFE VALUE} -platform {PLATFORM VALUE} -username {USERNAME VALUE} -address {ADDRESS VALUE} -DependentAcctID {DEPENDENT ACCTID VALUE} -LinkedAcctID {LINKED ACCTID VALUE}
.EXAMPLE
   $LinkAcctActionStatus = Set-VPASDependentLinkedAccount -AccountType {ACCOUNTTYPE VALUE} -AcctID {ACCTID VALUE} -DependentAcctID {DEPENDENT ACCTID VALUE} -LinkedAcctID {LINKED ACCTID VALUE}
.EXAMPLE
   $InputParameters = @{
   ...
   $LinkAcctActionStatus = Set-VPASDependentLinkedAccount -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        AccountType = "LogonAcct"|"JumpAcct"|"ReconAcct"
        AcctID = "22_123"
...
.OUTPUTS
   ...
   $false if failed
#>
function Set-VPASDependentLinkedAccount{
    [OutputType([bool])]
    ...
    Param(
        ...
        [Parameter(Mandatory=$true,ParameterSetName='Set3',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter type of target linked account (LogonAcct, JumpAcct, ReconAcct)")]
        [Parameter(Mandatory=$true,ParameterSetName='Set4',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter type of target linked account (LogonAcct, JumpAcct, ReconAcct)")]
        [ValidateSet('LogonAcct','JumpAcct','ReconAcct')]
        [String]$AccountType,

        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter AcctID of the account that will be linked (for example: 12_24)")]
        [Parameter(Mandatory=$true,ParameterSetName='Set2',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter AcctID of the account that will be linked (for example: 12_24)")]
        [Parameter(Mandatory=$true,ParameterSetName='Set3',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter AcctID of the account that will be linked (for example: 12_24)")]
        [Parameter(Mandatory=$true,ParameterSetName='Set4',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter AcctID of the account that will be linked (for example: 12_24)")]
        [String]$LinkedAcctID,
            ...
            elseif($AcctID -eq -2){
                ...
                if($NoSSL){
                    ...
                    if($ISPSS){
                        ...
                        $params = @{
                            index = $AccountTypeINT
                            accountId = $LinkedAcctID
                        }
                        $log = Write-VPASTextRecorder -inputval $params -token $token -LogType PARAMS
                        Write-Verbose "PARAMS: $params"
                        $params = $params | ConvertTo-Json
                        $uri = "http://$PVWA/api/accounts/$AcctID/account-dependents/$DependentAcctID/link-accounts"
                    }
                    else{
                        $AcctDetails = Get-VPASAccountDetails -AcctID $AcctID
                        $linksafe = $AcctDetails.safeName
                        $linkname = $AcctDetails.name
                        ...
                        Write-Verbose "PARAMS: $params"
                        $params = $params | ConvertTo-Json
                        $uri = "http://$PVWA/PasswordVault/api/Accounts/$AcctID/dependentAccounts/$DependentAcctID/Link"
                    }
                }
                else{
                    ...
                    if($ISPSS){
                        ...
                        Write-Verbose "PARAMS: $params"
                        $params = $params | ConvertTo-Json
                        $uri = "https://$PVWA/api/accounts/$AcctID/account-dependents/$DependentAcctID/link-accounts"
                    }
                    else{
                        $AcctDetails = Get-VPASAccountDetails -AcctID $AcctID
                        $linksafe = $AcctDetails.safeName
                        ...
                        Write-Verbose "PARAMS: $params"
                        $params = $params | ConvertTo-Json
                        $uri = "https://$PVWA/PasswordVault/api/Accounts/$AcctID/dependentAccounts/$DependentAcctID/Link"
                    }
                }
                ...
                Write-Verbose "ACCOUNT DEPENDENCY SUCCESSFULLY LINKED"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUa9P25QpOr8UUp1SyFHZnPwSr
# 56uggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEF