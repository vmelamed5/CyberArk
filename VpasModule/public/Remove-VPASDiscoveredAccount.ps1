<#
.Synopsis
   Delete discovered account
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to delete a specific discovered account from the pending account list
.LINK
   https://vpasmodule.com/commands.html#Remove-VPASDiscoveredAccount
.NOTES
   Category: PAM
   Tag: Accounts Discovery
   Since: v15.0.0
   Version: 2
   SelfHosted: FALSE
...
.EXAMPLE
   $DeleteDiscoveredAccountStatus = Remove-VPASDiscoveredAccount -SearchQuery {SEARCH QUERY VALUE}
.EXAMPLE
   $DeleteDiscoveredAccountStatus = Remove-VPASDiscoveredAccount -DiscoveredAccountID {DISCOVERED ACCOUNT ID VALUE}
.EXAMPLE
   $InputParameters = @{
        SearchQuery = "DiscoveredAccount01"
   ...
   $DeleteDiscoveredAccountStatus = Remove-VPASDiscoveredAccount -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        DiscoveredAccountID = "12345-abcdef-67890-ghijkl"
		WhatIf = $true|$false
		HideOutput = $true|$false
   }
   $DeleteDiscoveredAccountStatus = Remove-VPASDiscoveredAccount -InputParameters $InputParameters
.OUTPUTS
   ...
   $false if failed
#>
function Remove-VPASDiscoveredAccount{
    ...
    Param(
        ...
        [Parameter(Mandatory=$true,ParameterSetName='Set2',ValueFromPipelineByPropertyName=$true,HelpMessage="Unique DiscoveredAccountID that references the target account (for example: 12345-abcdef-67890-ghijkl)")]
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("SearchQuery","PlatformType","Privileged","Enabled","WhatIf","HideWhatIfOutput")
                        MandatoryKeys = @("SearchQuery")
                    }
					set2 = @{
                        AcceptableKeys = @("DiscoveredAccountID","WhatIf","HideWhatIfOutput")
                        MandatoryKeys = @("DiscoveredAccountID")
                    }
                }
        ...
        try{
            if(!$ISPSS -and ($PVWA -notmatch ".privilegecloud.cyberark.")){
                Write-VPASOutput -str "SelfHosted/Priv Cloud Standard solution does not support this API Call, returning false" -type E
                $log = Write-VPASTextRecorder -inputval "SelfHosted/Priv Cloud STandard solution does not support this API Call, returning false" -token $token -LogType MISC
                $log = Write-VPASTextRecorder -inputval $false -token $token -LogType RETURN
                return $false
            }
            else{
                if([String]::IsNullOrEmpty($DiscoveredAccountID)){
                    Write-Verbose "NO DISCOVERED ACCOUNT ID PROVIDED...INVOKING HELPER FUNCTION TO RETRIEVE UNIQUE DISCOVERED ACCOUNT ID BASED ON SPECIFIED PARAMETERS"
                    $DiscoveredAccountID = Get-VPASDiscoveredAccountIDHelper -token $token -SearchQuery $SearchQuery -PlatformType $PlatformType -Privileged $Privileged -Enabled $Enabled
                    Write-Verbose "RETURNING DISCOVERED ACCOUNT ID: $DiscoveredAccountID"
                }
                else{
                    Write-Verbose "DISCOVERED ACCOUNT ID SUPPLIED, SKIPPING HELPER FUNCTION"
                }

                if($DiscoveredAccountID -eq -1){
                    $log = Write-VPASTextRecorder -inputval "COULD NOT FIND UNIQUE DISCOVERED ACCOUNT ENTRY TO DELETE, INCLUDE MORE SEARCH PARAMETERS" -token $token -LogType MISC
                    $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
                    Write-VPASOutput -str "COULD NOT FIND UNIQUE DISCOVERED ACCOUNT ENTRY TO DELETE, INCLUDE MORE SEARCH PARAMETERS" -type E
                    Write-Verbose "UNABLE TO FIND UNIQUE DISCOVERED ACCOUNT ENTRY WITH SPECIFIED PARAMETERS"
                    return $false
                }
                elseif($DiscoveredAccountID -eq -2){
                    $log = Write-VPASTextRecorder -inputval "UNABLE TO FIND ANY DISCOVERED ACCOUNT WITH SPECIFIED PARAMETERS" -token $token -LogType MISC
                    $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
                    Write-Verbose "UNABLE TO FIND ANY DISCOVERED ACCOUNT WITH SPECIFIED PARAMETERS"
                    ...
                    if($NoSSL){
                        ...
                        $uri = "http://$PVWA/api/discovered-accounts/$DiscoveredAccountID"
                    }
                    else{
                        Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                        $uri = "https://$PVWA/api/discovered-accounts/$DiscoveredAccountID"
                    }
                    ...
                    if($WhatIf){
                        ...
                        $WhatIfInfo = Get-VPASDiscoveredAccountDetails -DiscoveredAccountID $DiscoveredAccountID
                        if(!$WhatIfInfo){
                            $log = Write-VPASTextRecorder -inputval "UNABLE TO QUERY DISCOVERED ACCOUNTS...RETURNING FALSE" -token $token -LogType MISC
                            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
                            $log = Write-VPASTextRecorder -token $token -LogType WHATIF2
                            Write-Verbose "UNABLE TO QUERY DISCOVERED ACCOUNTS...RETURNING FALSE"
                            Write-VPASOutput -str "UNABLE TO QUERY DISCOVERED ACCOUNTS" -type E
                            return $false
                        }
                        foreach($WhatIfDiscoveredAcct in $WhatIfInfo.Value){
                            $WhatIfID = $WhatIfDiscoveredAcct.id
                            $WhatIfType = $WhatIfDiscoveredAcct.type
                            $WhatIfSubType = $WhatIfDiscoveredAcct.subType
                            $WhatIfAddress = $WhatIfDiscoveredAcct.identifiers.address
                            $WhatIfUsername = $WhatIfDiscoveredAcct.identifiers.username
                            $WhatIfSource = $WhatIfDiscoveredAcct.source
                            $WhatIfIsPrivileged = $WhatIfDiscoveredAcct.isPrivileged
                            $WhatIfDomain = $WhatIfDiscoveredAcct.customProperties.domain
                            $WhatIfEnabled = $WhatIfDiscoveredAcct.customProperties.enabled
                            $WhatIfDescription = $WhatIfDiscoveredAcct.customProperties.description
                            $WhatIfNumberOfDependencies = $WhatIfDiscoveredAcct.numberOfDependencies

                            if($WhatIfID -eq $DiscoveredAccountID){
                                if(!$HideWhatIfOutput){
                                    Write-VPASOutput -str "====== BEGIN COMMAND SIMULATION ======" -type S
                                    Write-VPASOutput -str "THE FOLLOWING DISCOVERED ACCOUNT WOULD BE DELETED:" -type S
                                    Write-VPASOutput -str "ID                   : $WhatIfID" -type S
                                    Write-VPASOutput -str "Type                 : $WhatIfType" -type S
                                    Write-VPASOutput -str "SubType              : $WhatIfSubType" -type S
                                    Write-VPASOutput -str "Address              : $WhatIfAddress" -type S
                                    Write-VPASOutput -str "Username             : $WhatIfUsername" -type S
                                    Write-VPASOutput -str "Source               : $WhatIfSource" -type S
                                    Write-VPASOutput -str "IsPrivileged         : $WhatIfIsPrivileged" -type S
                                    Write-VPASOutput -str "Domain               : $WhatIfDomain" -type S
                                    Write-VPASOutput -str "Enabled              : $WhatIfEnabled" -type S
                                    Write-VPASOutput -str "Description          : $WhatIfDescription" -type S
                                    Write-VPASOutput -str "NumberOfDependencies : $WhatIfNumberOfDependencies" -type S
                                ...
                                $WhatIfHash = @{
                                    WhatIf = @{
                                        ID = $WhatIfID
                                        Type = $WhatIfType
                                        SubType = $WhatIfSubType
                                        Address = $WhatIfAddress
                                        Username = $WhatIfUsername
                                        Source = $WhatIfSource
                                        IsPrivileged = $WhatIfIsPrivileged
                                        Domain = $WhatIfDomain
                                        Enabled = $WhatIfEnabled
                                        Description = $WhatIfDescription
                                        NumberOfDependencies = $WhatIfNumberOfDependencies
                                        RestURI = $uri
                                        RestMethod = "DELETE"
                                        Disclaimer = "THIS DISCOVERED ACCOUNT WILL BE DELETED IF -WhatIf FLAG IS REMOVED"
                                    }
                                }
                        ...
                        $log = Write-VPASTextRecorder -inputval "UNABLE TO FIND TARGET DISCOVERED ACCOUNT...RETURNING FALSE" -token $token -LogType MISC
                        $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
                        $log = Write-VPASTextRecorder -token $token -LogType WHATIF2
                        Write-Verbose "UNABLE TO FIND TARGET DISCOVERED ACCOUNT...RETURNING FALSE"
                        Write-VPASOutput -str "UNABLE TO FIND TARGET DISCOVERED ACCOUNT" -type E
                        return $false
                    }
                    else{
                        ...
                        Write-Verbose "DISCOVERED ACCOUNT HAS BEEN DELETED"
            ...
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-Verbose "UNABLE TO DELETE DISCOVERED ACCOUNT"
            Write-VPASOutput -str $_ -type E
            return $false
        }
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUiD4tPKD3SAuUgQACVKy7spZh
# kKKggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFC3nMvsEiADPdj52INPAliBADjfG
# MA0GCSqGSIb3DQEBAQUABIICAEc/ZvygsL9/NcTcj8gXv8wYhyku43O0jzAyVk+b
# 1n2aE2KRXW/ovPQkcipvsR7QCDIIzYiHJ3ZZ5KEo+fY9Db14Ml1+dKB4oaTagDOh
# MBnQnbxspXYS1mRd7WE7uoO0qhRossXks2BxlAx2nI2JL5H6puqiWxa0Bp4TNYgY
# 4Ecx+Mac0wp+D4qQXeuYiurQngt5ho3/a9Ckye8d7A0FnBOKPSjhOCNujZPcVO6H
# PtnW1UUruqQVIrcf021mCXDL0iPJw9zABZYpLIuyuRfngAyXmmhftK20nTw3i30R
# +twj63jZ407fOUClt4IHtjWK07Y4Y5fgzrvIJZpwJuTSIVNv4ztnBsLTQFhsX4FY
# 5MZ1Ew0l+rLQIlPfktZTHQMXTnWzdHNkz4h4NhqON4+ogWlCKy2EsT11CHl5VVgG
# /u/N3Xeq+/iwXCENbnU1yygwTKlzIGuyyrd+LgFQt+xyIL5r1zha7y80sSHpOKFw
# Pg9oPY2pKhiXiaoTbTaMl0kIryBqB0YrP9EoXswVhE4nNlpf432zUQl7UyjFWA4y
# OOHBy0ZLQ+zMLJ+G+r3bs64boW2cm6xH+l4iqN5oyy10rXXxqZsfFCM2KSU7LSfa
# cuzzUgO3KioGVEXY8uHTkV5ihdgkWXkhTMCOm1/EzmYO3ojx6OswJUqK1s31ruph
# hRksoYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkGA1UEBhMC
...
# 9w0BCQUxDxcNMjYwMjA0MDQ1MzE1WjA/BgkqhkiG9w0BCQQxMgQwL7Y0rzTN+OON
# hpZreWHhqI2yaYibOgnp8Vq2PBLPq81o3UN7AJKKM8psbx8qRn0XMA0GCSqGSIb3
# DQEBAQUABIICAIemOQRu9zWyBUypUeXaktlj7Y9W2uVtu04ZeqNb8BuL9UeXKSzy
# h7oDWCVVsZUvSsHDmfVbjhVEAxFcCGYaSgEkMbJ1CYgNDPOFVDfh7fH+r3olav2I
# Sp1c7lJ4o5c+hDrB/o1D+Ho+rrwk+Qm9SqMTOlWOxD4GmpPWgJ3KdUcZmWw5sVdL
# BsaspE2Ohziakgl88Ov4idB4IVjkseKG1T0uHSTK0tHwOO2nJ5q1c5V4zLXSNOxM
# jpUJaw6od8x9dVhaTIrHpbjpY3ELToFbL0vO0asj4uxtSpsy0Ioc6f3ZW0mpAdHp
# AobIgVs1GoJVVpGHhFWOkZqis6r4ifyIK+mDVuNYYfg5SsnriFYHBtn4BHicVDUi
# /0s1ZzqztOX4FjNTGAwJCj24JwFd0I4zGeV0dVN+LXrmYrGYup9x71ANqezRV3zZ
# mA2rt0sGNIFhZIbgbLQqEh8oIP3CS4HYhWC6kYa5kOaq0aTWZaHTWF8lt5xckCuB
# xU7ElZKDc5af7c7MDiVboslEWzha8AmhqSVjKI3+EUIMdHxu/05GHRKQtMJ3fYx7
# oPP+s0KL2djF3qfUxWOXvo8NHaHHpfrmD1HcM1Wz0FycL1jlZvGRZI7AyGLhSoI7
# mi9+eqRleIujVAKJ9sER9ENPIILhciTDj+xfxXxiw3nJmvkZDy9GTdyd
