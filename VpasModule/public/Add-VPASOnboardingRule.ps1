...
.DESCRIPTION
   Use this function to create a new onboarding rule
.LINK
   https://vpasmodule.com/commands.html#Add-VPASOnboardingRule
.NOTES
   Category: PAM
...
.PARAMETER TargetPlatformId
   PlatformID that will be assigned to accounts
.PARAMETER TargetSafeName
   Safe that will be assigned to accounts
.PARAMETER SystemTypeFilter
   Filter by system type
   Possible values: Windows, Unix
.PARAMETER IsAdminIDFilter
   Filter to target only the highest privileged accounts
   UID 0 for Unix
   SID ending in 500 for Windows
   Default: false
.PARAMETER MachineTypeFilter
   Filter by machine type
   Possible values: Any, Workstation, Server
   Default: Any
.PARAMETER UserNameFilter
   Filter by account username
.PARAMETER UserNameFilterMethod
   Method of filtering done via username
   Possible values: Equals, Begins, Ends
   Default: Equals
.PARAMETER AddressFilter
   Filter by account address
.PARAMETER AddressFilterMethod
   Method of filtering done via address
   Possible values: Equals, Begins, Ends
   Default: Equals
.PARAMETER AccountCategoryFilter
   Filter accounts by level of access
   Possible values: Any, Privileged, Non-privileged
   Default: Any
.PARAMETER RuleName
   Name of the rule
   Default: Auto generated value
.PARAMETER RuleDescription
   An explanation/details of the target rule
   Best practice states to leave informative descriptions to help identify the resource purpose
.PARAMETER InputParameters
   HashTable of values containing the parameters required to make the API call
.EXAMPLE
   $CreateOnboardingRuleJSON = Add-VPASOnboardingRule -TargetPlatformId {TARGET PLATFORMID VALUE} -TargetSafeName {TARGET SAFENAME VALUE} -SystemTypeFilter {SYSTEM TYPE FILTER VALUE}
.EXAMPLE
   $InputParameters = @{
        TargetPlatformId = "TargetPlatformID"
        TargetSafeName = "TargetSafeName"
        SystemTypeFilter = "Windows"|"Unix"
        IsAdminIDFilter = $true|$false
        MachineTypeFilter = "Any"|"Workstation"|"Server"
        UserNameFilter = "TargetUsername"
        UserNameFilterMethod = "Equals"|"Begins"|"Ends"
        AddressFilter = "TargetAddress"
        AddressFilterMethod = "Equals"|"Begins"|"Ends"
        AccountCategoryFilter = "Any"|"Privileged"|"Non-privileged"
        RuleName = "TargetRuleName"
        RuleDescription = "Target Rule Description"
   }
   $VCreateOnboardingRuleJSON = Add-VPASOnboardingRule -InputParameters $InputParameters
.OUTPUTS
   If successful:
   {
        "RuleId":  1,
        "RuleName":  "ExampleOnboardingRule01",
        "RuleDescription":  "Rule created to demo API functionality",
        "RulePrecedence":  1,
        "TargetPlatformId":  "WinLocalVman",
        "TargetDeviceType":  "Operating System",
        "TargetSafeName":  "TestSafe",
        "IsAdminIDFilter":  false,
        "MachineTypeFilter":  "Any",
        "SystemTypeFilter":  "Windows",
        "CreationTime":  1752174903,
        "UserNameFilter":  "VMAN",
   ...
   $false if failed
#>
function Add-VPASOnboardingRule{
    [OutputType('System.Object',[bool])]
    [CmdletBinding(DefaultParameterSetName='Set1')]
    Param(

        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter the platformID that will be used to onboard accounts under this onboarding rule (for example: WinDomain)")]
        [String]$TargetPlatformId,

        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter the safe name that will be used to onboard accounts under this onboarding rule (for example: VmanDomainAccounts)")]
        [String]$TargetSafeName,

        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter the OS type filter to be targeted under this onboarding rule (possible values: Windows, Unix)")]
        ...
        [Switch]$IsAdminIDFilter,

        [Parameter(Mandatory=$false,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true)]
        [ValidateSet('Any','Workstation','Server')]
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("TargetPlatformId","TargetSafeName","SystemTypeFilter","IsAdminIDFilter","MachineTypeFilter","UserNameFilter","UserNameFilterMethod","AddressFilter","AddressFilterMethod","AccountCategoryFilter","RuleName","RuleDescription")
                        MandatoryKeys = @("TargetPlatformId","TargetSafeName","SystemTypeFilter")
                    }
                }
            ...
            Write-Verbose "UNABLE TO CREATE ONBOARDING RULE"
        ...
        try{
            ...
            Write-Verbose "ADDED TargetPlatformID: $TargetPlatformId TO API PARAMETERS"
            Write-Verbose "ADDED TargetSafeName: $TargetSafeName TO API PARAMETERS"
            Write-Verbose "ADDED SystemtypeFilter: $SystemTypeFilter TO API PARAMETERS"

            if($IsAdminIDFilter){
                $params += @{ IsAdminIDFilter = $true }
                Write-Verbose "ADDED IsAdminIDFilter: TRUE TO API PARAMETERS"
            }
            else{
                Write-Verbose "ADDED DEFAULT VALUE IsAdminIDFilter: FALSE TO API PARAMETERS"
            }

            if($MachineTypeFilter){
                $params += @{ MachineTypeFilter = $MachineTypeFilter }
                Write-Verbose "ADDED MachineTypeFilter: $MachineTypeFilter TO API PARAMETERS"
            }
            else{
                Write-Verbose "ADDED DEFAULT VALUE MachineTypeFilter: Any TO API PARAMETERS"
            }

            if($UserNameFilter){
                $params += @{ UserNameFilter = $UserNameFilter }
                Write-Verbose "ADDED UserNameFilter: $UserNameFilter TO API PARAMETERS"
            }
            else{
                Write-Verbose "NO UserNameFilter INCLUDED, SKIPPING"
            }

            if($UserNameFilterMethod){
                $params += @{ UserNameMethod = $UserNameFilterMethod }
                Write-Verbose "ADDED UserNameMethod: $UserNameFilterMethod TO API PARAMETERS"
            }
            else{
                Write-Verbose "ADDED DEFAULT VALUE UserNameMethod: Equal TO API PARAMETERS"
            }

            if($AddressFilter){
                $params += @{ AddressFilter = $AddressFilter }
                Write-Verbose "ADDED AddressFilter: $AddressFilter TO API PARAMETERS"
            }
            else{
                Write-Verbose "NO AddressFilter INCLUDED, SKIPPING"
            }

            if($AddressFilterMethod){
                $params += @{ AddressMethod = $AddressFilterMethod }
                Write-Verbose "ADDED AddressMethod: $AddressFilterMethod TO API PARAMETERS"
            }
            else{
                Write-Verbose "ADDED DEFAULT VALUE AddressMethod: Equal TO API PARAMETERS"
            }

            if($AccountCategoryFilter){
                $params += @{ AccountCategoryFilter = $AccountCategoryFilter }
                Write-Verbose "ADDED AccountCategoryFilter: $AccountCategoryFilter TO API PARAMETERS"
            }
            else{
                Write-Verbose "ADDED DEFAULT VALUE AccountCategoryFilter: Any TO API PARAMETERS"
            }

            if($RuleName){
                $params += @{ RuleName = $RuleName }
                Write-Verbose "ADDED RuleName: $RuleName TO API PARAMETERS"
            }
            else{
                Write-Verbose "NO RuleName INCLUDED, AUTO GENERATING A RULE NAME"
            }

            if($RuleDescription){
                $params += @{ RuleDescription = $RuleDescription }
                Write-Verbose "ADDED RuleDescription: $RuleDescription TO API PARAMETERS"
            }
            else{
                Write-Verbose "NO RuleDescription INCLUDED, SKIPPING"
            }
            ...
            if($NoSSL){
                ...
                $uri = "http://$PVWA/PasswordVault/API/AutomaticOnboardingRules/"
            }
            else{
                Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                $uri = "https://$PVWA/PasswordVault/API/AutomaticOnboardingRules/"
            }
            ...
            Write-Verbose "SUCCESSFULLY CREATED ONBOARDING RULE"
            Write-Verbose "RETURNING ONBOARDING RULE DETAILS"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQ