<#
.Synopsis
   Generate new MFA cache SSH key
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to generate an MFACache SSH key to login via PSM for SSH
.LINK
   https://vpasmodule.com/commands.html#New-VPASMFACacheSSHKey
.NOTES
   Category: PAM
   Tag: MFA Cache
   Since: v14.6.0
...
.PARAMETER KeyFormat
   What format the SSH Key will export as
   Possible values: PPK, PEM, OpenSSH
.PARAMETER Keyphrase
   Password or phrase that will be used to authenticate against the SSHKey provided to make a PSM for SSH connection
.PARAMETER CurrentUser
   Generated SSHKey will be used by the current user authenticated via the APIs
.PARAMETER TargetUsername
   Username of the target user that the SSHKey will be generated for if no UserID is passed
.PARAMETER TargetUserID
   Unique ID that maps to a single user that the SSHKey will be genarated for, passing this variable will skip any query functions
.PARAMETER InputParameters
   HashTable of values containing the parameters required to make the API call
.EXAMPLE
   $NewMFACacheSSHKey = New-VPASMFACacheSSHKey -KeyFormat {KEY FORMAT VALUE} -Keyphrase {KEYPHRASE VALUE} -CurrentUser
.EXAMPLE
   $NewMFACacheSSHKey = New-VPASMFACacheSSHKey -KeyFormat {KEY FORMAT VALUE} -Keyphrase {KEYPHRASE VALUE} -TargetUsername {TARGET USERNAME VALUE}
.EXAMPLE
   $NewMFACacheSSHKey = New-VPASMFACacheSSHKey -KeyFormat {KEY FORMAT VALUE} -Keyphrase {KEYPHRASE VALUE} -TargetUserID {TARGET USERID VALUE}
.EXAMPLE
   $InputParameters = @{
        KeyFormat = "PPK"|"PEM"|"OpenSSH"
        Keyphrase = "MySecretPassphrase"
        CurrentUser = $true|$false
   }
   $NewMFACacheSSHKey = New-VPASMFACacheSSHKey -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        KeyFormat = "PPK"|"PEM"|"OpenSSH"
        Keyphrase = "MySecretPassphrase"
...
.OUTPUTS
   If successful:
   {
        "value":  [
                      {
                          "format":  "PPK",
                          "privateKey":  "PuTTY-User-Key-File-2: ssh-rsa\r\nEncryption: aes256-cbc\r\nComment: CyberArk Auto-generated Key\r\nPublic-Lines: 12\r\nAAA...Y5WQ==\r\nPrivate-Lines: 28\r\nw4U...nUYpg==\r\nPrivate-MAC: a8df...895\r\n",
                          "keyAlg":  "RSA"
                      }
                  ],
        "creationTime":  1746805896,
        "expirationTime":  1746809496,
        "publicKey":  "ssh-rsa AAA...Y5WQ== CyberArk Auto-generated Key",
        "count":  1
   }
   ---
   $false if failed
#>
function New-VPASMFACacheSSHKey{
    [OutputType('System.Object',[bool])]
    [CmdletBinding(DefaultParameterSetName='Set1')]
    Param(

        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter target SSHKey format (valid values: PPK, PEM, OpenSSH)")][ValidateSet('PPK','PEM','OpenSSH')]
        [Parameter(Mandatory=$true,ParameterSetName='Set2',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter target SSHKey format (valid values: PPK, PEM, OpenSSH)")][ValidateSet('PPK','PEM','OpenSSH')]
        [Parameter(Mandatory=$true,ParameterSetName='Set3',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter target SSHKey format (valid values: PPK, PEM, OpenSSH)")][ValidateSet('PPK','PEM','OpenSSH')]
        [String]$KeyFormat,
        ...
        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Generate SSHKey for current user")]
        [Switch]$CurrentUser,

        [Parameter(Mandatory=$true,ParameterSetName='Set2',ValueFromPipelineByPropertyName=$true,HelpMessage="Generate SSHKey for a target user via Username (for example: Vman)")]
        [String]$TargetUsername,

        [Parameter(Mandatory=$true,ParameterSetName='Set3',ValueFromPipelineByPropertyName=$true,HelpMessage="Generate SSHKey for a target user via UserID (for example: 55)")]
        [String]$TargetUserID,
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("KeyFormat","Keyphrase","CurrentUser")
                        MandatoryKeys = @("KeyFormat","CurrentUser")
                    }
					set2 = @{
                        AcceptableKeys = @("KeyFormat","Keyphrase","TargetUsername")
                        MandatoryKeys = @("KeyFormat","TargetUsername")
                    }
                    set3 = @{
                        AcceptableKeys = @("KeyFormat","Keyphrase","TargetUserID")
                        MandatoryKeys = @("KeyFormat","TargetUserID")
                    }
                }
            ...
            Write-Verbose "FAILED TO GENERATE MFA CACHE SSH KEY"
        ...
        try{
            ...
            if($CurrentUser){
                ...
                if($NoSSL){
                    ...
                    $uri = "http://$PVWA/PasswordVault/API/Users/Secret/SSHKeys/Cache/"
                }
                else{
                    Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                    $uri = "https://$PVWA/PasswordVault/API/Users/Secret/SSHKeys/Cache/"
                }
                ...
                if([String]::IsNullOrEmpty($TargetUserID)){
                    Write-Verbose "NO USER ID PROVIDED...INVOKING HELPER FUNCTION TO RETRIEVE UNIQUE USER ID BASED ON SPECIFIED PARAMETERS"
                    $TargetUserID = Get-VPASEPVUserIDHelper -token $token -username $TargetUsername
                    Write-Verbose "RETURNING USER ID"
                }
                else{
                    Write-Verbose "USER ID SUPPLIED, SKIPPING HELPER FUNCTION"
                    Write-Verbose "FETCHING TARGET USERNAME"
                    $TargetUsername = (Get-VPASEPVUserDetails -EPVUserID $TargetUserID -token $token).username
                }
                ...
                if($NoSSL){
                    Write-Verbose "NO SSL ENABLED, USING HTTP INSTEAD OF HTTPS"
                    $uri = "http://$PVWA/PasswordVault/API/Users/$TargetUserID/Secret/SSHKeys/Cache/"
                }
                else{
                    Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                    $uri = "https://$PVWA/PasswordVault/API/Users/$TargetUserID/Secret/SSHKeys/Cache/"
                }
            ...
            Write-Verbose "RETURNING SSH KEY CONTENT"
            return $response
        }catch{
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-Verbose "UNABLE TO GENERATE MFA CACHE SSH KEY"
            Write-VPASOutput -str $_ -type E
            return $false
        }
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUldRelnnkeKPJgj6FvH7C015o
# o0iggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFAaPbXrSNIw3e5lTugw4RXSHtvnc
# MA0GCSqGSIb3DQEBAQUABIICAK5RGy2IirNFJKf5nHFrZR9EzM9DGlG0+khUtZXj
# rljUk0tkCazRVB5qlSWWXoveJJ0IqX3HQ32cbEsATOCH6RpBuFC+SWI1H9/r54kA
# 20w4Omtr9t7+wfbI3g3g66JlkUsd9M4SffK1xrUa8Np31Hx1yXTqnQH+Ie1lxeG9
# z1+UbsF2+iJGJv9kYrKvAFmcR8Eg5hAPBH4tfRVw8UarhnmzXZ65t7v6KfiQ+T2L
# +zH+UrNK4q/IACvOhvOZvRn+DWiNbspZS2K+yP9c5RTzz3he8jQA0O+IHAcFHfPP
# DalDtaNXvEE4nLzx9w9t30N3uVxUu28+Zg8VJv9f87o0WDtaVKPBi2UYdOL5Ovr1
# VXQEDCN52brr+WmOrbkYsChX0I3wSncJjUxluLGK0mvem1dCqaQF9RiqZ2tthMDf
# Fn0tFIB5c7JsegHMCvZ6h/VqraO88rRdESpCFjSOUMDCLHs7rwLfAS8WFCqqluR3
# 4lFjd+Oi8lqaV6DtbYYQoEuSCRVq5CpDuEUnElaJG66Mp7mLs2TlRkWWgK7vIC/8
# AvGQpwneMIgp1v2WLLm7fOfadQLQ5CiqVhYWebDrH/t4mtbwuTCzTW/x4UxPyMqa
# KYS7c7VV4ti2ujUmDnvzDcijE1/al3nNgW98I5LOb1XJ2RKs8+hD5mGN+5W1jIY9
# 5M9CoYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkGA1UEBhMC
...
# 9w0BCQUxDxcNMjYwMjA0MDQ1MjI0WjA/BgkqhkiG9w0BCQQxMgQwhavpv5RBCXPx
# iVOF+RpoearN931hR/cqm2vQzL1lk4is+lDmxvnZW939tcGADXGiMA0GCSqGSIb3
# DQEBAQUABIICAAig4LGc144K/BafliwCsf+xZOG8O1Pnr8I4i2N/Zzkw4L4yjaWB
# M4cWepWdoT38EEp8h4rW3eaUHjzxDOHJvTjFMbMM5fOD+y2ThTrxekSs/cOFeOEj
# dDZkjm5LOZ38lvOczIyPmuJa84mmkwAhxvhVIW+PrVLtl+pe0nsabrXLyOqiNcAi
# FgDAKRVxb/OZ2oua2GpqB/mzQzVSlK4vxOs9SiyKXJErKOl1Ran5Jv5Fkl1SXzwq
# DfxwPpXaXMqoNz8dqqoa4N4bkf43CYgF2ZHrEs/G5uZYOrqaGVPhbn5oJg9bF/I8
# oikxmRyGy1KK3z/R49IGBSiyDIyfb/WQAbVxbjbSkNzun+WigJ/wlwNF6Qzi4QKv
# sU9MxL8TXfhnqwmT9egpPX4Oj3/VmOqd5wuZBcfjsV2XMESolxt7alOXCrDcvfdi
# l2f4eAbzjV28OCOgl0GeETljJiuUku5QvzInTIj3yH4rOstmAlr48TKnv8KFhRH+
# YjitQaYsmjbh8r9JqyzvsQ7hioYVHISvgrbXCvuIAgNP+yqVrVzgNwXPkoDihyHF
# 5l4u0KE09ViEprTxuMDjotqW3XeHVCrRHzVxWdxrXtz/XO6rzieb62HS87d5YrL4
# /mpQjnu8cLmj67nJlfUzl+MyIqTfWBBPWe8yoPH//n7LHxEKm702fYv7
