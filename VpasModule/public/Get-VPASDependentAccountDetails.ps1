<#
.Synopsis
   Get dependent account details
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to retrieve details for a dependent (usage) account
.LINK
   https://vpasmodule.com/commands.html#Get-VPASDependentAccountDetails
.NOTES
   Category: PAM
...
.EXAMPLE
   $DependentAccountDetailsJSON = Get-VPASDependentAccountDetails -safe {SAFE VALUE} -username {USERNAME VALUE} -DependentSearchQuery {DEPENDENT SEARCH QUERY VALUE}
.EXAMPLE
   $DependentAccountDetailsJSON = Get-VPASDependentAccountDetails -AcctID {ACCTID VALUE} -DependentSearchQuery {DEPENDENT SEARCH QUERY VALUE}
.EXAMPLE
   $DependentAccountDetailsJSON = Get-VPASDependentAccountDetails -safe {SAFE VALUE} -username {USERNAME VALUE} -DependentAcctID {DEPENDENT ACCTID VALUE}
.EXAMPLE
   $DependentAccountDetailsJSON = Get-VPASDependentAccountDetails -AcctID {ACCTID VALUE} -DependentAcctID {DEPENDENT ACCTID VALUE}
.EXAMPLE
   $InputParameters = @{
        safe = "TargetSafeName"
   ...
   $DependentAccountDetailsJSON = Get-VPASDependentAccountDetails -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        AcctID = 3_12
        DependentSearchQuery = "WinService01"
...
.OUTPUTS
   If successful:
   {
        "safeName":  "TestSafe",
        "folderName":  "Root",
        "createdTime":  1751945098,
        "categoryModificationTime":  1751945098,
        "dependentAccountId":  "29_25",
        "accountId":  "29_3",
        "name":  "Operating System-WinLocalVman-server1.vman.com-LocalAdmin01-ComPlus-server5-comApp1",
        "platformId":  "ComPlus",
        "platformAccountProperties":  {
                                          "ApplicationName":  "comApp1",
                                          "Address":  "server5",
                                          "Restart":  "Yes"
                                      },
        "secretManagement":  {
                                 "automaticManagementEnabled":  true,
                                 "lastModifiedTime":  1751945098
                             }
   }
   ---
   $false if failed
#>
function Get-VPASDependentAccountDetails{
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("safe","platform","username","address","DependentSearchQuery","ExportToCSV","CSVDirectory")
                        MandatoryKeys = @("DependentSearchQuery")
                    }
                    set2 = @{
                        AcceptableKeys = @("AcctID","DependentSearchQuery","ExportToCSV","CSVDirectory")
                        MandatoryKeys = @("AcctID","DependentSearchQuery")
                    }
                    set3 = @{
                        AcceptableKeys = @("safe","platform","username","address","DependentAcctID","ExportToCSV","CSVDirectory")
                        MandatoryKeys = @("DependentAcctID")
                    }
                    set4 = @{
                        AcceptableKeys = @("AcctID","DependentAcctID","ExportToCSV","CSVDirectory")
            ...
            Write-Verbose "FAILED TO GET DEPENDENT ACCOUNT DETAILS"
        ...
        try{
            ...
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-Verbose "COULD NOT GET DEPENDENT ACCOUNT DETAILS"
            if(!$HideWarning){
                Write-VPASOutput -str $_ -type E
            }
            return $false
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUrodiSTQXZ7f8dG9lP1TifKzU
# w+yggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFMBmyQEKtEq8o0NPiJb28+3+Gxvw
# MA0GCSqGSIb3DQEBAQUABIICABIkpr3if3GV2sB921U9Me2mBUz5TqLQ52psnsCJ
# B8jxb9JvSNo3QTvKNoEcorSkeDCjxZSLDNm2PwfHsPFQ63HADEyXW+TtgE1A2Wg0
# 8hCZddORK4QzMFZJyCOHkFvGHvYEgQKMSCrp3Ga8PlvGwG6ajtq5Rkc6UYmbcyOG
# VuGhzLImp6yPt/pc7fLWtWQviVh8/2p+Zm/owWCXrzhO2yNxKI/Ur/O8WHcTTca+
