...
.DESCRIPTION
   Use this function to retrieve the network of an account
.LINK
   https://vpasmodule.com/commands.html#Get-VPASAccountNetwork
.NOTES
   Category: PAM
   Tag: Accounts
   Since: v15.0.0
...
.EXAMPLE
   $AccountNetworkJSON = Get-VPASAccountNetwork -safe {SAFE VALUE} -username {USERNAME VALUE} -platform {PLATFORM VALUE} -address {ADDRESS VALUE}
.EXAMPLE
   $AccountNetworkJSON = Get-VPASAccountNetwork -AcctID {ACCTID VALUE}
.EXAMPLE
   $InputParameters = @{
        safe = "TargetSafe"
   ...
   $AccountNetworkJSON = Get-VPASAccountNetwork -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        AcctID = "22_123"
        ExportToCSV = $true|$false
...
.OUTPUTS
   ...
   {
        "accountId":  "24_4",
        "networkId":  "123456789-abcd-1234-1a2b-abcdefghijklm",
        "networkName":  "VpasPool"
   }
   ---
   $false if failed
#>
function Get-VPASAccountNetwork{
    ...
    process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("safe","platform","username","address","ExportToCSV","CSVDirectory")
                        MandatoryKeys = @()
                    }
					set2 = @{
            ...
            Write-Verbose "FAILED TO RETRIEVE ACCOUNT NETWORK"
        ...
        try{
            ...
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-Verbose "UNABLE TO GET ACCOUNT NETWORK"
            Write-VPASOutput -str $_ -type E
            return $false
        }
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUj7tnhdxrXC86Mv+DkONSvy7C
# l0uggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFBXOUZhNm392vJg45M+NGB9i75c/
# MA0GCSqGSIb3DQEBAQUABIICAFhzTlA1Wp8bS7SNnBx45EAkDbU/AwCj+nESEpbx
# 8Ixro3VX4J82I41N3lHSQOj869cMXbD27K4sz0G5JO18p7yAX9awuFSgz3SnNwiK
# Zw0tgVN15GsHmxo0aikZv/fNSxMk96thpHIjA+WhVFLJSx/dUKwSXy1oaYFMKIWf
# tGiAkSPl4jeyqA1/mobjgVsp94FoTI0+JYyG9M2TlXhPSNtMJyIoYNBZRGZikIex
# 1u/clQ2AGSqUNpkm5EniQHkV0lstCmqt58ZuDXUtU+w76KOjzDM6Lh0FFC5gO6cu
# 0JquaHyXfLIUDBFl/FvdH0UTQQVutLOthu1Uq3XpTyNPUVaJ4JTEbgne0AacS9pg
# hKIaWrIL7m4p65HoLb+4ew2WjT5dHFcFtYG2mGz9P7a/QQb/iVZjOjQRFro2e/Ed
# /LTKc+Iw+LtqeaqtMs/kPxbUYuvnpJek24WVJA9ZcjJSm+0aZHQUjUWwYyZmlw//
# soNiB4qstTIeecb3lhnZVmvIbWiCAa4JdRnbzXHthFSgeJYxNDZTUyt0iXV5ITl6
# lBCf33rXES2Ef7AlCgNuU3AArp6riTfSDexUiRrvcF5sbwvzfgaHcluBziwz2XAL
# dzW+Z+R+DAxXU5jmaB+zafP1me2MedHPBnZowvZYeoFodd3/rG42f5KAZk/YZBTw
# tjEboYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkGA1UEBhMC
...
# 9w0BCQUxDxcNMjYwMjA0MDQ0NTAyWjA/BgkqhkiG9w0BCQQxMgQw+u7XLZe6JpVe
# blThApPF4MimDDFcukzf+ayeMsggwUMcsF6FtUAhuYz9iVtw0/BYMA0GCSqGSIb3
# DQEBAQUABIICAImcqcGtaCedaMDYho1UUocNScTQSOd9VIYTUFpEippqvQYcdk10
# d2d8pNSfzcSTxNVv7xcvOJgjarMoslqDQGF2ZMnsStY6BFE0szpafzvimavBUsi7
# Me4RmjA7Q7ByHonnP5aKmUhTHBJdTubgmyxe48hE3Y0mu/NKlKA5bUKfZscwYsfF
# z6Qdu6t+np5/6/B0L3uHkkyn0jghxC9Y75P917OTlldrtZpiuqJB341FVRMch38f
# cn65CNj0qWiQXJGmg8CBRUEeXAlTrdVsA2568vJi1G/UPNtashGJJiOIsos7KjU0
# DWbdpbp9ApuTJbSemXyIuTuJHhdLQ7YqwexiwizNj5+NcCbIwRfDPMKdZdjDBoS2
# q5wW8Cbnsn0VpeE5+GJ8OvQrgytSCls6qldmzJdUUxhzdnNKRQ21NVkxX7J9BAoC
# 0gwWliYlh/cd13hOjhIwMH6k5pXUKH8AYJN+2oJqsEvlzJmJ/lcEDee0peDueOvS
# yA1yRD9IFdAFRzGjKY/GilhX8NL9ny5QZSJmOffMTjKuo40e5vrIYcphO6Pi3kMZ
# VcCq+mVtQMFEoQh2lWZ/Ua+nq6kavWEZK68yk0ZvDBtllyY4RIkKeG5/JrIRyitW
# KvtkPYIIiLmRuUvgRReHM9tDycPRaI9EEcwBVDcAzcAVe0pXStML5HVY
