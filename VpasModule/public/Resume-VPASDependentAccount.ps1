<#
.Synopsis
   Resume dependent account
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to resume a disabled dependent account
.LINK
   https://vpasmodule.com/commands.html#Resume-VPASDependentAccount
.NOTES
   Category: PAM
...
.EXAMPLE
   $ResumeDependentAccountStatus = Resume-VPASDependentAccount -safe {SAFE VALUE} -username {USERNAME VALUE} -DependentSearchQuery {DEPENDENT SEARCH QUERY VALUE}
.EXAMPLE
   $ResumeDependentAccountStatus = Resume-VPASDependentAccount -AcctID {ACCTID VALUE} -DependentSearchQuery {DEPENDENT SEARCH QUERY VALUE}
.EXAMPLE
   $ResumeDependentAccountStatus = Resume-VPASDependentAccount -safe {SAFE VALUE} -username {USERNAME VALUE} -DependentAcctID {DEPENDENT ACCTID VALUE}
.EXAMPLE
   $ResumeDependentAccountStatus = Resume-VPASDependentAccount -AcctID {ACCTID VALUE} -DependentAcctID {DEPENDENT ACCTID VALUE}
.EXAMPLE
   $InputParameters = @{
        safe = "TargetSafeName"
   ...
   $ResumeDependentAccountStatus = Resume-VPASDependentAccount -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        AcctID = 3_12
        DependentSearchQuery = "WinService01"
...
.OUTPUTS
   ...
   $false if failed
#>
function Resume-VPASDependentAccount{
    ...
    Process{
        ...
        try{
            ...
            if($NoSSL){
                ...
                if($ISPSS){
                    $uri = "http://$PVWA/API/accounts/$AcctID/account-dependents/$DependentAcctID/resume"
                }
                else{
                    $uri = "http://$PVWA/PasswordVault/API/Accounts/$AcctID/dependentAccounts/$DependentAcctID/Resume"
                }
            }
            else{
                Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                if($ISPSS){
                    $uri = "https://$PVWA/API/accounts/$AcctID/account-dependents/$DependentAcctID/resume"
                }
                else{
                    $uri = "https://$PVWA/PasswordVault/API/Accounts/$AcctID/dependentAccounts/$DependentAcctID/Resume"
                }
            }
            ...
            Write-Verbose "DEPENDENT ACCOUNT WAS SUCCESSFULLY RESUMED"
            return $true
        }catch{
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-VPASOutput -str $_ -type E
            Write-Verbose "UNABLE TO RESUME DEPENDENT ACCOUNT"
            return $false
        }
    }
    End{
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUsJnzEa3ahPjIKCpOzQHbcRuy
# CimggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFFr5LxpIlSb7SM2FOOqHDartfauK
# MA0GCSqGSIb3DQEBAQUABIICAGh72ROzJ0IE77U09BP7RygVurZFSYYLrMUir2NC
# +AhMc8nzXBxIi84kLz4L//HNeikDolthjan1dmx0KjGNvSrY2AXwVIHuUoD4uNWD
# iinFN+zd/+Wmd00Z3tN6PUrxlcIxFL893gyt6GRqnOVs0mWl8X6GUTkzauDyZa/w
# RruqxrAY2cYX7NcT764bJcTJrFFIdwXQE/hSKiHX40rPrNhdxrXjihXM612WdVXX
# jQ1h6fBRwTFH2bVW/yZ81OibiiRryrglkgUTtTKCaQdB8vjPij/ASWNDsJqXVHxQ
# VXSVFWi3yjj6Nza9pZzuuGmdO2OAnP/d8S/zaxoQ+I6ba6DBNHoF49dd4BwZM2NC
# LN4KImJhP8gFyd2k2Xnn1lw92YInl+CtT/k5gaFvLUACKb1v8rmwETuZTTIAiLp7
# PdCSpflQOtNHK9m3ky3fJPiwi+fbjq8YF/GDkcELBN5XrwoINS7vIgfsHxWapASP
# Qza5BlkZKa4Ic/eIkwvvLaKI8h3AiyFXAJGWUTR2wOZutLugpPCzMgzL0A7TNqig
# 0pUYNv0KyX5kpijf5QcIe6/oXckuVDbpl3wl3NixEO5lX0k1gdS0/OPV5HElieFy
# kgoISLTmNll2h36aBamZlBwmBL0E5wotKuo07BVk1jlBVsCf7kK/qIyOTXMQmAcQ
# LEJ8oYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkGA1UEBhMC
...
# 9w0BCQUxDxcNMjYwMjA0MDQ1NDIxWjA/BgkqhkiG9w0BCQQxMgQw0GTHq05ugCaa
# TaXvrJilyABBy6YBy6ELEh5CZ2hPokwrTlC7zHKsKkFy8QvAm7kaMA0GCSqGSIb3
# DQEBAQUABIICAIYI8vaXj5fyrnvsXQQ9XHZrqjXn1CmdoNh5a6a33HbC3qB5TTqQ
# AdYtfduK0E1nelQTvBeCP/0z25huqmkjQ2nvMJQF9cU4ypPuy+ulC3t46uWIXyyG
# 0LmRmEVaUavxQ/cxWZRGdMxjjnexuB9Fh0Qej6cKrvOGNa5Cm0cis/yJtdnhBH0m
# x7XWVHYNBORThpisplvEzhG+jW4voCel35yWH53WZCqaVThTCL4qEJ/v4O1JoLrl
# dJOcmHTS029CXVVd3HhvFXjCdew03twefIpWLl7B0ls6yXdH+UE0cH8qCkWkfGz9
# tHN2vm1RtB2996F218Q+CHHXoSOCRi5Nfc5rdLLhrVcy6PHbkol3U9oxdHhGEvMc
# JseXta5CFePKdk3JX77k5Zf4xyZEbDC1ysvQdPITQhMTmgexdipwDv3r6f/THQvR
# NQlcsxfaniG4STd22yX/osi6C4YErC/FdPIOk58AbRBU9VunReywxL1FX79qrfBb
# lzKEOfGZ3I66rM7w20e305sy/+VVaiMaJTTgNniNGDBJi+S3M88puR2Has02U1b/
# 1C5RS2OhwLks3YJyUZYfHUOTEfnvgK+HTqI/kQPI52Kpd0+JXEX7F8ITJy2FTQND
# lAdMYK2s9a47bnZZterzTHEtmKue8hnfxzFLbIZH4J8SsdM012WnMEc1
