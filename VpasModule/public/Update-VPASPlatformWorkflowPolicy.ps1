<#
.Synopsis
   Update platform workflow policy
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to update a platform workflow policy
.LINK
   https://vpasmodule.com/commands.html#Update-VPASPlatformWorkflowPolicy
.NOTES
   Category: PAM
   Tag: Platforms
   Since: v15.0.0
...
.PARAMETER Field
   ...
   Possible values: minValidityPeriod, doNotExtendMinValidityPeriod, unlockIfFail
.PARAMETER FieldVal
   Target value to update the target field with
.PARAMETER InputParameters
   HashTable of values containing the parameters required to make the API call
.EXAMPLE
   $UpdatePlatformWorkflowPolicy = Update-VPASPlatformWorkflowPolicy -PlatformID {PLATFORMID VALUE} -Field {FIELD VALUE} -FieldVal {FIELDVAL VALUE}
.EXAMPLE
   $InputParameters = @{
        PlatformID = "TargetPlatformID"
        Field = "minValidityPeriod"|"doNotExtendMinValidityPeriod"|"unlockIfFail"
        FieldVal = "UpdatedValue"
   }
   $UpdatePlatformWorkflowPolicy = Update-VPASPlatformWorkflowPolicy -InputParameters $InputParameters
.OUTPUTS
   If successful:
   {
        "minValidityPeriod":  60,
        "doNotExtendMinValidityPeriod":  false,
        "unlockIfFail":  false
   }
   ---
   $false if failed
#>
function Update-VPASPlatformWorkflowPolicy{
    [OutputType([bool])]
    ...
    Param(
        ...
        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Target field that will be updated (for example: minValidityPeriod)")]
        [ValidateSet('minValidityPeriod','doNotExtendMinValidityPeriod','unlockIfFail')]
        [String]$Field,

        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Updated value for the targeted field (for example: 60)")]$FieldVal,
    ...
    Process{
        try{
            ...
            Write-Verbose "FAILED TO UPDATE PLATFORM WORKFLOW POLICY"
        ...
        try{
            ...
            Write-Verbose "FETCHING CURRENT WORKFLOW POLICY FOR $PlatformID"
            $currentValues = Get-VPASPlatformWorkflowPolicy -platformID $PlatformID
            if(!$currentValues){
                Write-Verbose "COULD NOT FIND $PlatformID WORKFLOW POLICY"
                Write-VPASOutput -str "COULD NOT FIND $PlatformID WORKFLOW POLICY" -type E
                return $false
            }

            $props = @("minValidityPeriod","doNotExtendMinValidityPeriod","unlockIfFail")
            ...
            if($NoSSL){
                ...
                $uri = "http://$PVWA/api/platforms/$PlatformID/workflows-policy"
            }
            else{
                Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                $uri = "https://$PVWA/api/platforms/$PlatformID/workflows-policy"
            }
            ...
            Write-Verbose "UPDATES MADE TO $PlatformID WORKFLOW POLICY, RECEIVED JSON OBJECT"

            return $response
        }catch{
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-Verbose "UNABLE TO UPDATE $PlatformID WORKFLOW POLICY"
            Write-VPASOutput -str $_ -type E
            return $false
        }
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUyK2uPJ5dK7H97pnLXh3WlRhh
# +VeggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFG8X4ZNsG8OAm7vT941/SdCdnfSS
# MA0GCSqGSIb3DQEBAQUABIICAE7ah45MPA9vRy/tUjKpedEto92nmQXp/xW9KcYt
# 29vWHGe/c4J+vrggplSfp+cdvPzziYr0br1HZtHa3DW3g3O+k211m/Km0j3A2PeG
# QzE6V1AXIfNsHVhc/UTHtWVMKnYsDcHnPiTDexIkfBuULvk2SUwQ7+W9cm5AwtXN
