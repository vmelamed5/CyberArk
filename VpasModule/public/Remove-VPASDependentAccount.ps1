...
.DESCRIPTION
   Use this function to delete a dependent (usage) account
.LINK
   https://vpasmodule.com/commands.html#Remove-VPASDependentAccount
.NOTES
   Category: PAM
...
.EXAMPLE
   $DeleteDependentAccountStatus = Remove-VPASDependentAccount -safe {SAFE VALUE} -username {USERNAME VALUE} -platform {PLATFORM VALUE} -address {ADDRESS VALUE} -DependentSearchQuery {DEPENDENT SEARCH QUERY VALUE}
.EXAMPLE
   $DeleteDependentAccountStatus = Remove-VPASDependentAccount -AcctID {ACCTID VALUE} -DependentSearchQuery {DEPENDENT SEARCH QUERY VALUE}
.EXAMPLE
   $DeleteDependentAccountStatus = Remove-VPASDependentAccount -safe {SAFE VALUE} -username {USERNAME VALUE} -platform {PLATFORM VALUE} -address {ADDRESS VALUE} -DependentAcctID {DEPENDENT ACCTID VALUE}
.EXAMPLE
   $DeleteDependentAccountStatus = Remove-VPASDependentAccount -AcctID {ACCTID VALUE} -DependentAcctID {DEPENDENT ACCTID VALUE}
.EXAMPLE
   $InputParameters = @{
        safe = "TargetSafeName"
   ...
   $DeleteDependentAccountStatus = Remove-VPASDependentAccount -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        AcctID = 3_12
        DependentSearchQuery = "WinService01"
...
.OUTPUTS
   ...
   $false if failed
#>
function Remove-VPASDependentAccount{
    ...
    Param(
        ...
        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Searh string to find target dependent account (for example: WinService01)")]
        [Parameter(Mandatory=$true,ParameterSetName='Set2',ValueFromPipelineByPropertyName=$true,HelpMessage="Searh string to find target dependent account (for example: WinService01)")]
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("safe","platform","username","address","DependentSearchQuery","WhatIf","HideWhatIfOutput")
                        MandatoryKeys = @("DependentSearchQuery")
                    }
                    set2 = @{
                        AcceptableKeys = @("AcctID","DependentSearchQuery","WhatIf","HideWhatIfOutput")
                        MandatoryKeys = @("AcctID","DependentSearchQuery")
                    }
                    set3 = @{
                        AcceptableKeys = @("safe","platform","username","address","DependentAcctID","WhatIf","HideWhatIfOutput")
                        MandatoryKeys = @("DependentAcctID")
                    }
                    set4 = @{
                        AcceptableKeys = @("AcctID","DependentAcctID","WhatIf","HideWhatIfOutput")
                        MandatoryKeys = @("AcctID","DependentAcctID")
                    }
                }
            ...
            Write-Verbose "FAILED TO REMOVE DEPENDENT ACCOUNT"
        ...
        try{
            ...
            if($WhatIf){
                ...
                if(!$HideWhatIfOutput){
                    ...
                    Write-VPASOutput -str "THE FOLLOWING DEPENDENT ACCOUNT WOULD BE DELETED:" -type S
                ...
                $WhatIfHash = @{
                    WhatIf = @{
                        ...
                        RestURI = $uri
                        RestMethod = "DELETE"
                        Disclaimer = "THIS DEPENDENT ACCOUNT WILL BE DELETED IF -WhatIf FLAG IS REMOVED"
                    }
                }
                ...
                Write-Verbose "DEPENDENT ACCOUNT WAS SUCCESSFULLY DELETED FROM CYBERARK"
                return $true
            }
        }catch{
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-VPASOutput -str $_ -type E
            Write-Verbose "UNABLE TO DELETE DEPENDENT ACCOUNT FROM CYBERARK"
            return $false
        }
    }
    End{
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUUs3S9R9AvVPhq6fWkzZnnuYj
# VSiggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFP4a/awRmx29GF/2vHbFwwwbhDL9
# MA0GCSqGSIb3DQEBAQUABIICAI/5J9x9nb31BZd4HMAJtZfBWqmBxaKKcRLXjmC0
# p959s+n3sjYOM395ZVj/qNULV6+KsR/nC0ouJrAA6T0s+Nqpu/uvyahppopiSHjD
# 9WiIQwG7hih+Eun/md0TOuLw32gRf9XOyLgxauneMtU9IHuVTAeo3UDoPJHgIubs
# BFXsbfwtpNrewyoL4cz57ABBlofRhUyekPPffQd6oo6yy5D0UknBMi1m+r/8tecT
# JiJhJrvMuRLA6lTTrV8zZmVHFdnU8vSVj4Xr0YdPDEHokDUpdYmSMkfdfGZZM1Um
# xA02thdZJLdw9sI1cgvdgMYXfzji42TSaF2OJaccGJS7ArGNtgfF13ab3BOzMila
# 9usC/QErCc9ndc8gqE36rto2M7YIYAyXZU3kNiOaSLo0J1uzKPKSh0L9fSmVSJGN
# NwPTCdK0xm0Bldi4xsieRNejcBulD9FVpbB0NtCwS2lDSL9Y1pqaB98p/PuXleeq
# flJnUUPhL27W9DyTilwVYwfCEqCbkDWGQsBuAxImF1nv5gVIKiuYVbEBZl6Z7nYQ
# XRuw/dXEI5JkbEm41++s0oddOQbuedNPIO+TKGnxZ/N+A68YSPigcq+2sM5NGptK
# yAbdeHFvpqcKy4+ojNlfeUAjBAqNQ7+a/jgRwfzLcvF1KdaUn+EROVNPo7x7TnVf
# 2wmIoYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkGA1UEBhMC
...
# 9w0BCQUxDxcNMjYwMjA0MDQ1MzA2WjA/BgkqhkiG9w0BCQQxMgQw+URPhV81N4jo
# 4dgmvfm9066HupKpkPTcbnGeYJbNhzX3jDGDVA269f9I5+xylAx6MA0GCSqGSIb3
# DQEBAQUABIICAMqw+YfeN2GJ2ijzSUCHTsW3UFknzyVbPJVnKItLA/kglLCmSwH3
# 0Q0J1cfsqVqQy19z1TQVF8gMq7fxma5yYrk3cRuiWz+oS38HbnnKvkpqX5o2U1aJ
# lLSNDGYOid0MWbRaouwllQhnlFc6NZJEsfW+p2dW5m3R4zvohP6BEB3zwuj2Fk+D
# oYSDM9wgHWWy0n/gP4NxonEu/QfrZBs4OV+r+d0IEW74+i9cRzk92ub4K8vwyE02
# McR2ctAajyo4FBsraDp03tDOmrA56CJIXTpSCcJzvnW/tL5fW35HDPuULkObUbK8
# /RlIuHm34SvpVcpLovFMmT8dkiX5rEyJOtq5Nb64AjZ5TM/Sn8hH1ZEFWHnfjULU
# +iw5d9ABY2i5X49pBgHjgvj7rBQnr6qoQZM0oVP0llwvGZsf85tZlL5lTvdMjgVZ
# VN1o2+On2yZUfK69rgU1lPAuoe9ZnMur19crls25Anm7vzF/79k22suTyRCHsxRs
# OWcPTB+bL4wUrQw31dUMtNmQ4Juwxmwj80vmfTxVveVkPM7f3T7ZcN7zGwBCFGuT
# iYnEx2LZc7yl9yqIzw3P7mSgmBuWWgUI7skUFLICRt4DY6FzA3xQqzvVLZR82iaL
# Jw/1T/h2ONJXK8fAT9jxWIU+Om5GMW48x0b1dyOz05NRIWvI1MgfvzOw
