...
.DESCRIPTION
   Use this function to get remedation rule(s) details based on a search query
.LINK
   https://vpasmodule.com/commands.html#Get-VPASRemediationRules
.NOTES
   Category: PAM
...
.PARAMETER SearchQuery
   Search string that will be used to query for the target remedtaion rule if no remedtaion ruleID is passed
...
.EXAMPLE
   $RemediationRulesJSON = Get-VPASRemediationRules -SearchQuery {SEARCH QUERY VALUE}
.EXAMPLE
   $InputParameters = @{
   ...
   $RemediationRulesJSON = Get-VPASRemediationRules -InputParameters $InputParameters
.OUTPUTS
   If successful:
   {
        "Value":  [
                      ...
                      {
                          "id":  "12345678-1234-abcd-1234-8767hjk87",
                          "name":  "Vman OU Scan",
                          "status":  "ACTIVE",
                          "rulesCount":  2,
                          "actionsPerformed":  "@{ONBOARD=5; MARK_FOR_DELETION=0; DELETE=0; MARK_THEN_DELETE=0}",
                          "lastModificationTime":  "2025-11-26T07:20:40"
                      },
                      {
                          "id":  "98765432-9876-zyxwv-9876-kjlaldskja798",
                          "name":  "VmanTestScan01",
                          "status":  "INACTIVE",
                          "rulesCount":  1,
                          "actionsPerformed":  "@{ONBOARD=0; MARK_FOR_DELETION=0; DELETE=7; MARK_THEN_DELETE=0}",
                          "lastModificationTime":  "2025-11-26T07:16:17"
                      },
                      ...
                  ]
    }
   ...
   $false if failed
#>
function Get-VPASRemediationRules{
    [OutputType('System.Object',[bool])]
    [CmdletBinding(DefaultParameterSetName='Set1')]
    Param(
        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Searh string to find target remediation rule (for example: TestRule01)")]
        [String]$SearchQuery,
    ...
    Process{
        try{
            ...
            Write-Verbose "FAILED TO RETRIEVE REMEDIATION RULES"
        ...
        try{
            ...
            Write-Verbose "FETCHING ALL REMEDIATION RULES"
            $AllRemediationRules = Get-VPASAllRemediationRules -token $token
            foreach($rule in $AllRemediationRules.value){
                $matched = $false
                $ruleName = $rule.name

                if(!$matched){
                    if($ruleName -match $SearchQuery){
                        Write-Verbose "REMEDIATION RULE MATCHED ON RuleName...ADDING TO OUTPUT"
                        $output.Value += $rule
                        $matched = $true
                    }
                }
            ...
            if($ExportToCSV){
	            $runexport = Write-VPASExportCSV -Data $output.value -CommandName $CommandName -CSVDirectory $CSVDirectory
            }
            ...
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-Verbose "UNABLE TO RETRIEVE REMEDIATION RULES FROM CYBERARK"
            Write-VPASOutput -str $_ -type E
            return $false
        }
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUnBZlP4mTWEP3H4TMEUWd0kGE
# NEOggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFAxsP6g0Q/2ULGh3Fan4h5ukn+hw
# MA0GCSqGSIb3DQEBAQUABIICAIA6OT29UkyAAWgACOJpM5HJJD8nUAfF/EgAjR5E
# e8o7E4jpEyoDxBrKJwUDukJ1JFhcsq9KcbFzvONLP6Aqa5i2tmdEok0hnF/QXJp3
# 41zoO2fryRAAz+lmTtIjatm39e2CBZCwhnnCO/M18R0ulx7li36DswRTpPogInaO
