<#
.Synopsis
   Run various account metrics
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to generate various Account related metrics and KPIs
.LINK
   https://vpasmodule.com/commands.html#Invoke-VPASMetricsAccounts
.NOTES
   Category: PAM
   Tag: Metrics
   Since: v14.1.0
...
.EXAMPLE
   $GenerateReport = Invoke-VPASMetricsAccounts -TargetMetric {TARGET METRIC VALUE} -MetricFormat {METRIC FORMAT VALUE} -OutputDirectory {OUTPUT DIRECTORY VALUE}
.EXAMPLE
   $InputParameters = @{
        TargetMetric = "OnboardedAccountTypes"|"AccountsOnboardedXDays"|"AccountComplianceStatus"
...
function Invoke-VPASMetricsAccounts{
    ...
    Param(
        ...
        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter TargetMetric to be generated (OnboardedAccountTypes, AccountsOnboardedXDays, AccountComplianceStatus)")]
    ...
    Process{
        ...
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("TargetMetric","MetricFormat","OutputDirectory","DayRange","AmtOfSets","HideRawData","IgnoreSafes","IgnorePlatforms","IgnoreUsernames","SafeSearchQuery","PlatformSearchQuery","UsernameSearchQuery")
        ...
        try{
            ...
            if($TargetMetric -eq "OnboardedAccountTypes"){
                ...
                $TempPlatforms = Get-VPASAllPlatforms -token $token
                if(!$TempPlatforms){
            ...
            if($TargetMetric -eq "AccountComplianceStatus"){
                ...
                foreach($rec in $AllAccounts.value){
                    ...
                    if(!$skipval){
                        $complianceDetails = Get-VPASAccountCompliance -AcctID $targetAcctID -token $token
                        $complianceStatus = $complianceDetails.value.iscompliant
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU5BmAYBa124Cx9FyKGR7gvo3U
# PRaggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFCzulC7LGMA3JbRLPooMZA7uqBDj
# MA0GCSqGSIb3DQEBAQUABIICAHs/aNJ7AwRHcVfqW1kzCH5EnM094By7Q84YhuSm
# xm6D31kDmc3/0TiJIRWK8glmmNrLniEevlKnT8CLtDV4kpPyQ+TGFKkicYcRjixw
# b5AlwH0xjnE2pY1Uta+v8YZnL0gHORvociqcqyCmKT+I+arvFG+NzFFsGbXJIyXQ
# Qnaq3qSUmE7aboAWusrBsKcDlPjQCQPlSRkRdjtmpJmQheq9REg43I+SNinu1T2K
# T9maGpWzGxzq48ITgnnOdvpTIs07viytvJfgdVGUpQPdSEW5AOhn8ai6eK/rsRBH
# BbuDz2ScP58oEL/x/4P3bfEPwvO4tvC7apWw1nhqb94b54KoCp5y3o9VA6tQi18i
# CQAT0c9j2Nk+M/nIOC5TEhh1D3wJq/MGVunCrG6/YYSVXv/jQ0/Qvr9X/l8Z151t
# k+fyJCSzrQXm2YTZL51Uxdo8Epat86Et+j4ycrYT01dGPsNEPuecXkLTog+SPKbv
# gxaHs42Y9g7gmmvkHxOfJVltGXZljmBRyndoTCjket1H+PzTq4/naGQ1GcU1Mrvx
# sBG88Z4uK0gVUObOetpMugQ6hMc9TuBgAlFaltf1uwyxJRZpIREK/TYChc9/uT7I
# q93FTnjYgpkoLZseWQ9xEFR40eGTOSlY4PZ8Po+CVEtXOKwFcSqIRdX8dGjFcHXg
# gG+OoYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkGA1UEBhMC
...
# 9w0BCQUxDxcNMjYwMjA0MDQ1MTU3WjA/BgkqhkiG9w0BCQQxMgQw5bQLrJpHA6W1
# 9HRaF+VwBQg+4/yuhe5woAE9oaFPdc9hox7s8KJcXImFvAmArCqzMA0GCSqGSIb3
# DQEBAQUABIICAGQdbWayEdhfG2fDbHcuNfu5GJ+lgvevuTicznQmoanRXtP3fuLh
# STs/zx9Czt53ZpsyapOHnJajU5ZJn3OYBRJ3WWXC0nM0H3R676mgA63+Ak1eX5dk
# u+yBuvlSgjOGwWIVUfE9jdDpA97O5vpDYa/gX7mfxNZTC44cV5u1jsOGT+BFSywx
# vfz+GfmfBJusUAkb+5Z432+fUQ7UeXZIOt6JIjjwLcuj7XfrvGlSqa11brfnlTuS
# Cf4TSLtiMtE75NhKEcGeecZUUfbTVVKunwcsG+/MfViCMfyPgEMMx2en34hmjLGL
# SPiMDj4NjIubzRMvqt1pHsg0SP/MUAU553c1/reBVS+ofNrtxw7G17t4DYuzLPzF
# pa9zUNEseCm9UTOGgtABViS1NrjOxX5oGH+CT4QNwA6PJcZ+xhBZYS15VVUsfZ44
# 3QrvXM08sTXGjDMAfMUv81XWsurPWnkALcCRIQSnN9NuXaK1+WNZ/QE6Cl0cUoAk
# tVl04KinxJtepHzGH/K7E0hlDmh5VHwBtF4GA6k/DB3yR2NfeskzlbpPUG/fH33u
# N7Wjn9NMntpsXLp/2C5lXyKe22Aajwk259cI8eJp8e/4Td5AGvKVnpKLNhhXn9kN
# sckMAUzoxJFBk7hxa9jYfqpaMTcwrzgKVo8VCXhN5si2Y8yUF9Sracu6
