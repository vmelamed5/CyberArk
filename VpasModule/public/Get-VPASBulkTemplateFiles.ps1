...
.DESCRIPTION
   Use this function to generate bulk template file examples for bulk actions
.LINK
   https://vpasmodule.com/commands.html#Get-VPASBulkTemplateFiles
.NOTES
   Category: PAM
   Tag: Bulk Actions
   Since: v12.2.1
...
.PARAMETER SRS
   For SRS enabled environments
   The APIs for adding safes no longer include a CPM. Enable this flag for SRS environments
.EXAMPLE
...
function Get-VPASBulkTemplateFiles{
    ...
    Param(
        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter which BulkTemplateFile is required (BulkSafeCreation, BulkAccountCreation, BulkSafeMembers)")]
    ...
    Process{
        ...
        try{
            ...
            if($BulkTemplate -eq "BulkSafeCreation"){
                ...
                if($SRS){
                    $logstr = "SafeName,VersionsRetention,DaysRetention,Description"
                    $logstr0 = "#DELETE THIS LINE AND THE LINES BELOW AND POPULATE WITH CORRECT DATA"
                    $logstr1 = "#SafeName is MANDATORY,If VersionRetention blank safe will use DaysRetention,If DaysRetention blank safe will default to 7 or VersionRetention"
                    $logstr2 = "ExampleSafeName,,7,Example Safe Creation"
                    $logstr3 = "ExampleSafeName2,,,"
                }
                else{
                    if($ISPSS){
                        $logstr = "SafeName,CPM,VersionsRetention,DaysRetention,Description"
                        $logstr0 = "#DELETE THIS LINE AND THE LINES BELOW AND POPULATE WITH CORRECT DATA"
                        $logstr1 = "#SafeName is MANDATORY,If CPM blank NO CPM will be attached,If VersionRetention blank safe will use DaysRetention,If DaysRetention blank safe will default to 7 or VersionRetention"
                        $logstr2 = "ExampleSafeName,PasswordManager,,7,Example Safe Creation"
                        $logstr3 = "ExampleSafeName2,,,,"
                    }
                    else{
            ...
            elseif($BulkTemplate -eq "BulkAccountCreation"){
                ...
                $logstr = "SafeName,PlatformID,Username,Address,CustomName,SecretType,SecretValue,AutomaticManagementEnabled,extrapass1Safe,extrapass1Username,extrapass1Name,extrapass1Folder,extrapass3Safe,extrapass3Username,extrapass3Name,extrapass3Folder,CPMAction,PlatformAccountProperties"
                $logstr0 = "#DELETE THIS LINE AND THE LINES BELOW AND POPULATE WITH CORRECT DATA"
                $logstr1 = "#SafeName + PlatformID + Username + Address is MANDATORY,If SecretType blank default is Password, If SecretValue blank Account will be created with blank values, if CustomName is blank Cyberark will generate a name, if AutomaticManagementEnabled blank default is TRUE, if assigning Logon Account all extrapass1 fields must be filled out, if assigning Recon Account all extrapass3 fields must be filled out, if CPMAction blank no action will be taken, PlatformAccountProperties need to be passed as a string comma separated PropertyName,PropertyValue"
                $logstr2 = "ExampleSafe1,WinDomain,TestAcct1,vman.com,TestAcct1_vman.com,Key,k3yS3cr3t,False,,,,,ExampleSafe1,TestAcct1,TestAcct1_vman.com,root,Verify,`"LogonTo,TargetDomain`""
                ...
                $logstr4 = "ExampleSafe3,UnixViaSSH,Root,UnixServer1,UnixServer1_root,,P@ssw0rd!,,,,,,,,,,Change,`"Tag,TagValue,UseSudoOnReconcile,True`""
                $logstr5 = "ExampleSafe4,UnixViaSSH,SudoUser1,UnixServer1,SudoUser1_UnixServer1,,,,,,,,ExampleSafe3,Root,UnixServer1_root,root,Reconcile,`"Database,TargetDBName1,Port,1521`""
            }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU