<#
.Synopsis
   Update platform secret policy
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to update a platform secret policy
.LINK
   https://vpasmodule.com/commands.html#Update-VPASPlatformSecretPolicy
.NOTES
   Category: PAM
   Tag: Platforms
   Since: v15.0.0
...
.PARAMETER Field
   ...
   Possible values: passwordLen, minUppercase, minLowercase, minDigit, minSpecial, preventRepeatingChars, blockReusePassword, forbiddenChars, privateKeyFormat, encryptionType, keyGenerationTimeout, publicSSHKeyPath, passwordReusedLimit
.PARAMETER FieldVal
   Target value to update the target field with
.PARAMETER InputParameters
   HashTable of values containing the parameters required to make the API call
.EXAMPLE
   $UpdatePlatformSecretPolicy = Update-VPASPlatformSecretPolicy -PlatformID {PLATFORMID VALUE} -Field {FIELD VALUE} -FieldVal {FIELDVAL VALUE}
.EXAMPLE
   $InputParameters = @{
        PlatformID = "TargetPlatformID"
        Field = "passwordLen"|"minUppercase"|"minLowercase"|"minDigit"|"minSpecial"|"preventRepeatingChars"|"blockReusePassword"|"forbiddenChars"|"privateKeyFormat"|"encryptionType"|"keyGenerationTimeout"|"publicSSHKeyPath"|"passwordReusedLimit"
        FieldVal = "UpdatedValue"
   }
   $UpdatePlatformSecretPolicy = Update-VPASPlatformSecretPolicy -InputParameters $InputParameters
.OUTPUTS
   If successful:
   {
        ...
        "passwordGenRules":  {
                                 ...
                                 "blockReusePassword":  null,
                                 "forbiddenChars":  "\u003c"
                             },
        "sshKeysRules":  null,
        "accessKeysRules":  null,
        "passwordReusedLimit":  5,
        "createdAt":  "2026-01-23T04:15:57.986Z",
        "updatedAt":  "2026-01-23T04:15:57.986Z",
        "createdBy":  null,
        "updatedBy":  null,
   ...
   $false if failed
#>
function Update-VPASPlatformSecretPolicy{
    [OutputType([bool])]
    ...
    Param(
        ...
        [ValidateSet('passwordLen','minUppercase','minLowercase','minDigit','minSpecial','preventRepeatingChars','blockReusePassword','forbiddenChars','privateKeyFormat','encryptionType','keyGenerationTimeout','publicSSHKeyPath','passwordReusedLimit')]
        [String]$Field,
    ...
    Process{
        try{
            ...
            Write-Verbose "FAILED TO UPDATE PLATFORM SECRET POLICY"
        ...
        try{
            ...
            Write-Verbose "FETCHING CURRENT SECRET POLICY VALUES FOR $PlatformID"
            $currentValues = Get-VPASPlatformSecretPolicy -platformID $PlatformID
            if(!$currentValues){
                Write-Verbose "COULD NOT FIND $PlatformID SECRET POLICY DETAILS"
                Write-VPASOutput -str "COULD NOT FIND $PlatformID SECRET POLICY DETAILS" -type E
                return $false
            }

            $passwordGenRules = @("passwordLen","minUppercase","minLowercase","minDigit","minSpecial","preventRepeatingChars","blockReusePassword","forbiddenChars")
            $sshKeysRules = @("privateKeyFormat","encryptionType","keyGenerationTimeout","publicSshKeyPath")
            Write-Verbose "SETTING PARAMETERS"

            #HANDLE PASSWORD GEN RULES PROPERTIES
            if($passwordGenRules.Contains($Field)){
                $currentValues.passwordGenRules.$Field = $FieldVal
            }

            #HANDLE SSH KEY RULES PROPERTIES
            if($sshKeysRules.Contains($Field)){
                $currentValues.sshKeysRules.$Field = $FieldVal
            }
            ...
            if($NoSSL){
                ...
                $uri = "http://$PVWA/api/platforms/$PlatformID/secrets-policy"
            }
            else{
                Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                $uri = "https://$PVWA/api/platforms/$PlatformID/secrets-policy"
            }
            ...
            Write-Verbose "UPDATES MADE TO $PlatformID SECRET POLICY, RECEIVED JSON OBJECT"

            return $response
        }catch{
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-Verbose "UNABLE TO UPDATE $PlatformID SECRET POLICY"
            Write-VPASOutput -str $_ -type E
            return $false
        }
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU