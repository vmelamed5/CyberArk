...
.DESCRIPTION
   Use this function to retrieve all platforms that have no accounts assigned to them
.LINK
   https://vpasmodule.com/commands.html#Get-VPASEmptyPlatforms
.NOTES
   Category: PAM
   Tag: Platforms
   Since: v14.4.1
...
function Get-VPASEmptyPlatforms{
    ...
    Process{
        ...
        try{
            ...
            $AllPlatforms1 = Get-VPASAllPlatforms -token $token
            foreach($plat in $AllPlatforms1.Platforms.general.id){
                $platformIDs += $plat
            }

            $AllPlatforms2 = Get-VPASAllTargetPlatforms -token $token
            ...
            foreach($plat in $platformIDs){
                if($plat -match "PSMSecureConnect"){
                    ...
                    $AllAccounts = Get-VPASAccountDetails -platform $plat -ExactMatch -HideWarning -token $token
                    if(!$AllAccounts){
                        $PlatformDetails = Get-VPASPlatformDetails -platformID $plat -token $token
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUECUe6KVQPpemMIBlZhOPScje
# cVqggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFMQVpCSrPo0+47hE4+8aOJWx9JnE
# MA0GCSqGSIb3DQEBAQUABIICAKLTJfKnQHX4OYriTFb1ghsp4yrgtUlFMiGcD5YN
# fqcPF0em3UsJDbhyAGDiva8DzCCpUq3hPQzKj1nP7wqqy6+k4RVsvzxLVc3t8sQp
# A3s6LH2EpvCn7zwjcC03U1VQDLglHbPAFFvDb5vQ+OBWL1wqM4MGYmQzsHvoTg2B
# jkOQU8Mgy36wMhenEkOv1b6kNH/wkbJYFrQNwixd5JuOkOOEe46qqaIdmsvFFzDA
# Q1OMjnIW7SC25X27se9EXfu1uC3bG603ezx5MxADvlNAF7PjSt6ppUc1G59Bun0q
# ZnktmN2GW1OioyAuY/VEadf++OZs4BydO+C+yAwTaQajHm0kqoYHF5Wy6mQCBAuG
# LmzKV4cHHS8rAWUZyctclxr8PIwUQ/PBvA2lAn+RBGXS4fFi4cNAubwjRPrYLKzn
# /LIcLHn/dZICX4ihV1pselodkb3+NqQkavdq8cicPi54BS+jL7q6Jmnjrhv3fHRA
# VLw6o1G0IFYuPE43TJECp6XCfVNq3T4+bb3JfsGscupdwGSCUf9ZG9YaZo5XhbpM
# 3O3cW1QUxHpiIx5Un1AfdGXm5V+mNWmdJrl9brikqsgLwc45FYaLuFHiYekpT3M0
# H/SmsZkHpSiZqrGpss1HqsetFaHiE2pxAxlQ++R/uPpHBGltuhVXNdZ7hliQtIg7
# FhCLoYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkGA1UEBhMC
...
# 9w0BCQUxDxcNMjYwMjA0MDQ0ODA4WjA/BgkqhkiG9w0BCQQxMgQwEEszprBXHQO4
# NZGA9zaq/UHIWQL2a4hK296i1e9zqrGzX0jxWMMJBVJ81MYnKUlmMA0GCSqGSIb3
# DQEBAQUABIICAI4BO/d4aOmsUrNyMebb5Ix9GdGkZMR/7lvS5EuNsvdt1XxKFTH9
# q6zGV+CzvBkRzmMHlqoCUrC26AlN1h81mPAu9V9cfrr2r2MtV3odt471nA51Xkz6
# Lb4u2cicLjbof5+2XVxJ2/UM3aU6BhhBIlHzN741C8k/hMnDIA1oZNQdMeZ9d1An
# IqmHyzZTbrePnSel9heRcn3NZg2JFcnnSle7OMrqKikF62I/a+ELquY58y1fJCxR
# vvnAB3V00XjV4ZuMwKeackngj2VfcmU3h2LAIHMZPBb5+YrbLvabhY3lxqCcTqff
# gf4iiuxpW+ZKZnihRNF1U7viNFeTJqhMnN/BnDEUW4tFdkNmHZFGudlabkhJgqxd
# qlM0sR4trRfdKSvqjyOZoMaAwEOw8eMYiCdLN9AVlZ3T64lXE80HJCMiDaaQPHS0
# 1v/m91eYcf7UbmDQ3RW+PKfjCSlY00JZSGZhtg1egOIVBkd3Svii8P/zfPw8ISAa
# 1blF7e2cplL1oCIKAXBiYoftOQzTmOQsKQB1/UyxWTZe5I5J7lLrEfG0JYllAJfe
# 6pnSQuQL4dhYiYMRKyORwDCJUSsshTFTXJonmo00UrdznZYby1T+8P1vi0mOkuN1
# Dk+1n7iyGP/hLpXRJq/fYw9BqisAt2lUj5iv1T9CDQXNirCEX+37XRCK
