...
.DESCRIPTION
   Use this function to delete an onboarding rule
.LINK
   https://vpasmodule.com/commands.html#Remove-VPASOnboardingRule
.NOTES
   Category: PAM
   Tag: Onboarding Rules
   Since: v14.6.0
   Version: 4
   SelfHosted: TRUE
...
.PARAMETER OnboardingRuleID
   Unique ID that maps to a single onboarding rule, passing this variable will skip any query functions
.PARAMETER SearchQuery
   Search string that will be used to query for the target onboarding rule if no OnboardingRuleID is passed
.PARAMETER WhatIf
...
.EXAMPLE
   $DeleteOnboardingRuleStatus = Remove-VPASOnboardingRule -searchquery {SEARCH QUERY VALUE}
.EXAMPLE
   $DeleteOnboardingRuleStatus = Remove-VPASOnboardingRule -OnboardingRuleID {ONBOARDING ID VALUE}
.EXAMPLE
   $InputParameters = @{
        OnboardingRuleID = 3
        WhatIf = $true|$false
   ...
   $DeleteOnboardingRuleStatus = Remove-VPASOnboardingRule -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        searchquery = "TestRule01"
        WhatIf = $true|$false
        HideWhatIfOutput = $true|$false
   }
   $DeleteOnboardingRuleStatus = Remove-VPASOnboardingRule -InputParameters $InputParameters
.OUTPUTS
   ...
   $false if failed
#>
function Remove-VPASOnboardingRule{
    [OutputType([bool],'System.Object')]
    [CmdletBinding(DefaultParameterSetName='Set1')]
    Param(
        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Searh string to find target onboarding rule (for example: OnboardingLocalAdmin)")]
        [String]$SearchQuery,

        [Parameter(Mandatory=$true,ParameterSetName='Set2',ValueFromPipelineByPropertyName=$true,HelpMessage="Unique OnboardingRuleID that references a unique onboarding rule (for example: 3)")]
        [String]$OnboardingRuleID,
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("SearchQuery","WhatIf","HideWhatIfOutput")
                        MandatoryKeys = @("SearchQuery")
                    }
                    set2 = @{
                        AcceptableKeys = @("OnboardingRuleID","WhatIf","HideWhatIfOutput")
                        MandatoryKeys = @("OnboardingRuleID")
                    }
                }
            ...
            Write-Verbose "FAILED TO REMOVE ONBOARDING RULE"
            Write-VPASOutput -str $_ -type E
            return $false
        }

        try{
            if([String]::IsNullOrEmpty($OnboardingRuleID)){
                Write-Verbose "NO ONBOARDING RULE ID PROVIDED...INVOKING HELPER FUNCTION TO RETRIEVE UNIQUE ONBOARDING RULE ID BASED ON SPECIFIED PARAMETERS"
                $OnboardingRuleID = Get-VPASOnboardingRuleIDHelper -token $token -SearchQuery $SearchQuery

                if($OnboardingRuleID -eq -1){
                    $log = Write-VPASTextRecorder -inputval "COULD NOT FIND UNIQUE ONBOARDING RULE ENTRY, INCLUDE MORE SEARCH PARAMETERS" -token $token -LogType MISC
                    $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
                    Write-Verbose "COULD NOT FIND UNIQUE ONBOARDING RULE ENTRY, INCLUDE MORE SEARCH PARAMETERS"
                    Write-VPASOutput -str "COULD NOT FIND UNIQUE ONBOARDING RULE ENTRY, INCLUDE MORE SEARCH PARAMETERS" -type E
                    return $false
                }
                elseif($OnboardingRuleID -eq -2){
                    $log = Write-VPASTextRecorder -inputval "NO ONBOARDING RULES FOUND" -token $token -LogType MISC
                    $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
                    ...
                    Write-Verbose "RETURNING ONBOARDING RULE ID: $OnboardingRuleID"
                }
            }
            else{
                Write-Verbose "ONBOARDING RULE ID SUPPLIED, SKIPPING HELPER FUNCTION"
            }

            if($NoSSL){
                Write-Verbose "NO SSL ENABLED, USING HTTP INSTEAD OF HTTPS"
                $uri = "http://$PVWA/PasswordVault/API/AutomaticOnboardingRules/$OnboardingRuleID/"
            }
            else{
                Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                $uri = "https://$PVWA/PasswordVault/API/AutomaticOnboardingRules/$OnboardingRuleID/"
            }
            ...
            if($WhatIf){
                ...
                $WhatIfInfo = Get-VPASAllOnboardingRules -token $token
                foreach($rule in $WhatIfInfo.AutomaticOnboardingRules){
                    $ruleRuleId = $rule.RuleId
                    $ruleRuleName = $rule.RuleName
                    $ruleRuleDescription = $rule.RuleDescription
                    $ruleRulePrecedence = $rule.RulePrecedence
                    $ruleTargetPlatformId = $rule.TargetPlatformId
                    $ruleTargetDeviceType = $rule.TargetDeviceType
                    $ruleTargetSafeName = $rule.TargetSafeName
                    $ruleIsAdminIDFilter = $rule.IsAdminIDFilter
                    $ruleMachineTypeFilter = $rule.MachineTypeFilter
                    $ruleSystemTypeFilter = $rule.SystemTypeFilter
                    $ruleCreationTime = $rule.CreationTime
                    $ruleUserNameFilter = $rule.UserNameFilter
                    $ruleUserNameMethod = $rule.UserNameMethod
                    $ruleAddressFilter = $rule.AddressFilter
                    $ruleAddressMethod = $rule.AddressMethod
                    $ruleAccountTypeFilter = $rule.AccountTypeFilter
                    $ruleAccountCategoryFilter = $rule.AccountCategoryFilter
                    $ruleReconcileAccountId = $rule.ReconcileAccountId
                    $ruleLastOnboardedTime = $rule.LastOnboardedTime

                    if($ruleRuleId -eq $OnboardingRuleID){
                        if(!$HideWhatIfOutput){
                            Write-VPASOutput -str "====== BEGIN COMMAND SIMULATION ======" -type S
                            Write-VPASOutput -str "THE FOLLOWING ONBOARDING RULE WOULD BE DELETED:" -type S
                            Write-VPASOutput -str "RuleId                : $ruleRuleId" -type S
                            Write-VPASOutput -str "RuleName              : $ruleRuleName" -type S
                            Write-VPASOutput -str "RuleDescription       : $ruleRuleDescription" -type S
                            Write-VPASOutput -str "RulePrecedence        : $ruleRulePrecedence" -type S
                            Write-VPASOutput -str "TargetPlatformId      : $ruleTargetPlatformId" -type S
                            Write-VPASOutput -str "TargetDeviceType      : $ruleTargetDeviceType" -type S
                            Write-VPASOutput -str "TargetSafeName        : $ruleTargetSafeName" -type S
                            Write-VPASOutput -str "IsAdminIDFilter       : $ruleIsAdminIDFilter" -type S
                            Write-VPASOutput -str "MachineTypeFilter     : $ruleMachineTypeFilter" -type S
                            Write-VPASOutput -str "SystemTypeFilter      : $ruleSystemTypeFilter" -type S
                            Write-VPASOutput -str "CreationTime          : $ruleCreationTime" -type S
                            Write-VPASOutput -str "UserNameFilter        : $ruleUserNameFilter" -type S
                            Write-VPASOutput -str "UserNameMethod        : $ruleUserNameMethod" -type S
                            Write-VPASOutput -str "AddressFilter         : $ruleAddressFilter" -type S
                            Write-VPASOutput -str "AddressMethod         : $ruleAddressMethod" -type S
                            Write-VPASOutput -str "AccountTypeFilter     : $ruleAccountTypeFilter" -type S
                            Write-VPASOutput -str "AccountCategoryFilter : $ruleAccountCategoryFilter" -type S
                            Write-VPASOutput -str "ReconcileAccountId    : $ruleReconcileAccountId" -type S
                            Write-VPASOutput -str "LastOnboardedTime     : $ruleLastOnboardedTime" -type S
                        ...
                        $WhatIfHash = @{
                            WhatIf = @{
                                RuleId = $ruleRuleId
                                RuleName = $ruleRuleName
                                RuleDescription = $ruleRuleDescription
                                RulePrecedence = $ruleRulePrecedence
                                TargetPlatformId = $ruleTargetPlatformId
                                TargetDeviceType = $ruleTargetDeviceType
                                TargetSafeName = $ruleTargetSafeName
                                IsAdminIDFilter = $ruleIsAdminIDFilter
                                MachineTypeFilter = $ruleMachineTypeFilter
                                SystemTypeFilter = $ruleSystemTypeFilter
                                CreationTime = $ruleCreationTime
                                UserNameFilter = $ruleUserNameFilter
                                UserNameMethod = $ruleUserNameMethod
                                AddressFilter = $ruleAddressFilter
                                AddressMethod = $ruleAddressMethod
                                AccountTypeFilter = $ruleAccountTypeFilter
                                AccountCategoryFilter = $ruleAccountCategoryFilter
                                ReconcileAccountId = $ruleReconcileAccountId
                                LastOnboardedTime = $ruleLastOnboardedTime
                                RestURI = $uri
                                RestMethod = "DELETE"
                                Disclaimer = "THIS ONBOARDING RULE WILL BE DELETED IF -WhatIf FLAG IS REMOVED"
                            }
                        }
                ...
                Write-Verbose "ONBOARDING RULE WAS SUCCESSFULLY DELETED FROM CYBERARK"
                return $true
            }
        }catch{
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write