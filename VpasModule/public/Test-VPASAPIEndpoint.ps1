...
.DESCRIPTION
   Use this function to test an API endpoint that has not been converted into a commandlet yet
.LINK
   https://vpasmodule.com/commands.html#Test-VPASAPIEndpoint
.NOTES
   Category: MISC
   Tag: Utility
   Since: v15.0.0
...
.PARAMETER APIEndpoint
   Target URI or APIEndpoint to test the API call against
.PARAMETER Method
   Target method for the API call to use
   Possible values: POST, GET, PUT, DELETE, PATCH
.PARAMETER Category
   Target service that the API call will test against
   Possible values: PAM, Identity
.PARAMETER Params
   JSON object containing parameters to include in the body of the test API call
   Example: -Params @{ Property1 = "Value1" }
.PARAMETER InputParameters
   HashTable of values containing the parameters required to make the API call
.EXAMPLE
   $TestAPICallReturn = Test-VPASAPIEndpoint -APIEndpoint {APIENDPOINT VALUE} -Method {METHOD VALUE} -Category {CATEGORY VALUE}
.EXAMPLE
   $InputParameters = @{
        APIEndpoint = "/api/undocumented-endpoint-Test"
        Method = "POST"|"GET"|"PUT"|"DELETE"|"PATCH"
        Category = "PAM"|"Identity"
        Params = @{
            Property1 = "Property1Value"
            Property2 = "Property2Value"
        }
   }
   $TestAPICallReturn = Test-VPASAPIEndpoint -InputParameters $InputParameters
.OUTPUTS
   If successful:
   Various returns possible depending on APIEndpoint
   ---
   $false if failed
#>
function Test-VPASAPIEndpoint{
    [CmdletBinding()]
    Param(
        [Parameter(Mandatory=$true,ValueFromPipelineByPropertyName=$true,HelpMessage="Enter API endpoint to test (for example: /api/platforms/WinDomain/rotation-policy)")]
        [String]$APIEndpoint,

        [Parameter(Mandatory=$true,ValueFromPipelineByPropertyName=$true,HelpMessage="Enter API method (possible values: POST, GET, PUT, DELETE, PATCH)")]
        [ValidateSet("POST","GET","PUT","DELETE","PATCH")]
        [String]$Method,

        [Parameter(Mandatory=$true,ValueFromPipelineByPropertyName=$true,HelpMessage="Enter API category (possible values: PAM, Identity)")]
        [ValidateSet("PAM","Identity")]
        [String]$Category,
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("APIEndpoint","Method","Category","Params","IncludeContentType")
                        MandatoryKeys = @("APIEndpoint","Method","Category")
                    }
                }
            ...
            Write-Verbose "FAILED TO TEST API ENDPOINT"
        ...
        try{
            $uri = ""
            Write-Verbose "URI Initiation: $uri"
            ...
            if($NoSSL){
                ...
                $uri += "http://"
                Write-Verbose "URI Handling https/http: $uri"
            }
            else{
            ...
            if($Category -eq "PAM"){
                $uri += "$PVWA"
                Write-Verbose "URI Handling PVWA: $uri"
                $APIEndpoint = $APIEndpoint -replace "$PVWA/",""
            }
            if($Category -eq "Identity"){
                $uri += "$IdentityURL"
                Write-Verbose "URI Handling IdentityURL: $uri"
                $APIEndpoint = $APIEndpoint -replace "$IdentityURL/",""
            }

            if($APIEndpoint[0] -eq "/"){
                $uri += $APIEndpoint
                Write-Verbose "URI Final: $uri"
            }
            else{
                $uri += "/$APIEndpoint"
                Write-Verbose "URI Final: $uri"
            ...
            if($sessionval){
                if($Params){
                    ...
                    Write-Verbose "ADDING PARAMETERS TO API CALL: $Params"
                    $response = Invoke-RestMethod -Headers @{"Authorization"=$Header} -Uri $uri -Method $Method -Body $Params -ContentType "application/json" -WebSession $sessionval
                    $APICounter += 1
                }
                else{
                    if($IncludeContentType){
                        $response = Invoke-RestMethod -Headers @{"Authorization"=$Header} -Uri $uri -Method $Method -WebSession $sessionval -ContentType "application/json"
                        $APICounter += 1
                    }
                    else{
                        $response = Invoke-RestMethod -Headers @{"Authorization"=$Header} -Uri $uri -Method $Method -WebSession $sessionval
                        $APICounter += 1
                    }
                }
            }
                ...
                if($Params){
                    $Params = $Params | ConvertTo-Json
                    Write-Verbose "ADDING PARAMETERS TO API CALL: $Params"
                    $response = Invoke-RestMethod -Headers @{"Authorization"=$Header} -Uri $uri -Method $Method -Body $Params -ContentType "application/json"
                    $APICounter += 1
                }
                else{
                    if($IncludeContentType){
                        $response = Invoke-RestMethod -Headers @{"Authorization"=$Header} -Uri $uri -Method $Method -ContentType "application/json"
                        $APICounter += 1
                    }
                    else{
                        $response = Invoke-RestMethod -Headers @{"Authorization"=$Header} -Uri $uri -