<#
.Synopsis
   Sync secret with a dependent account
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to sync or push a secret to a dependent (usage) account
.LINK
   https://vpasmodule.com/commands.html#Push-VPASDependentAccountSecret
.NOTES
   Category: PAM
...
.EXAMPLE
   $PushDependentAccountSecretStatus = Push-VPASDependentAccountSecret -safe {SAFE VALUE} -username {USERNAME VALUE} -DependentSearchQuery {DEPENDENT SEARCH QUERY VALUE}
.EXAMPLE
   $PushDependentAccountSecretStatus = Push-VPASDependentAccountSecret -AcctID {ACCTID VALUE} -DependentSearchQuery {DEPENDENT SEARCH QUERY VALUE}
.EXAMPLE
   $PushDependentAccountSecretStatus = Push-VPASDependentAccountSecret -safe {SAFE VALUE} -username {USERNAME VALUE} -DependentAcctID {DEPENDENT ACCTID VALUE}
.EXAMPLE
   $PushDependentAccountSecretStatus = Push-VPASDependentAccountSecret -AcctID {ACCTID VALUE} -DependentAcctID {DEPENDENT ACCTID VALUE}
.EXAMPLE
   $InputParameters = @{
        safe = "TargetSafeName"
   ...
   $PushDependentAccountSecretStatus = Push-VPASDependentAccountSecret -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        AcctID = 3_12
        DependentSearchQuery = "WinService01"
...
.OUTPUTS
   ...
   $false if failed
#>
function Push-VPASDependentAccountSecret{
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("safe","platform","username","address","DependentSearchQuery")
                        MandatoryKeys = @("DependentSearchQuery")
                    }
                    set2 = @{
                        AcceptableKeys = @("AcctID","DependentSearchQuery")
                        MandatoryKeys = @("AcctID","DependentSearchQuery")
                    }
                    set3 = @{
                        AcceptableKeys = @("safe","platform","username","address","DependentAcctID")
                        MandatoryKeys = @("DependentAcctID")
                    }
                    set4 = @{
                        AcceptableKeys = @("AcctID","DependentAcctID")
            ...
            Write-Verbose "FAILED TO PUSH SECRET TO DEPENDENT ACCOUNT"
        ...
        try{
            ...
            if($NoSSL){
                ...
                if($ISPSS){
                    $uri = "http://$PVWA/API/accounts/$AcctID/account-dependents/$DependentAcctID/sync"
                }
                else{
                    $uri = "http://$PVWA/PasswordVault/API/Accounts/$AcctID/dependentAccounts/$DependentAcctID/Sync"
                }
            }
            else{
                Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
                if($ISPSS){
                    $uri = "https://$PVWA/API/accounts/$AcctID/account-dependents/$DependentAcctID/sync"
                }
                else{
                    $uri = "https://$PVWA/PasswordVault/API/Accounts/$AcctID/dependentAccounts/$DependentAcctID/Sync"
                }
            }
            ...
            if($sessionval){
                $response = Invoke-RestMethod -Headers @{"Authorization"=$Header} -Uri $uri -Method POST -WebSession $sessionval
                $APICounter += 1
            }
            else{
                $response = Invoke-RestMethod -Headers @{"Authorization"=$Header} -Uri $uri -Method POST
                $APICounter += 1
            }
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: TRUE" -token $token -LogType MISC
            Write-Verbose "SECRET WAS PUSHED TO DEPENDENT ACCOUNT SUCCESSFULLY"
            return $true
        }catch{
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUxBjiPG7nwa0AVxYuz00EqsO0
# 87CggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFJPOdzR+Thz4rfG4m6m/Oz8k1dQa
# MA0GCSqGSIb3DQEBAQUABIICAGAla57c29Jy9td1uRzzv2CCfIeeDZJsweuzXS1g
# wfxExF4tixA2bFu6F73/+OhmxCiKc1ejjiXNPITn88mC/MOdD1w4SI2BmgtBr172
# CrIIbQjPbTDBRUoy3cvIQnzNfnVctz/uZctIZ7cOBXhoIjoas06ap6Ik+8R0lAip
# MlEL0lVMVgzSnKm0KK6Peo3SsS7yDcUNc3RKH8E9q3/Gy+lRXSKKhaWA70YuNYac
