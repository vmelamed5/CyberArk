...
.DESCRIPTION
   Use this function to get the password value of an account
.LINK
   https://vpasmodule.com/commands.html#Get-VPASPasswordValue
.NOTES
   Category: PAM
   Tag: Accounts
   Since: v10.10
...
.EXAMPLE
   $AccountPassword = Get-VPASPasswordValue -reason {REASON VALUE} -AcctID {ACCTID VALUE}
.EXAMPLE
   $InputParameters = @{
        safe = "TargetSafe"
   ...
   $AccountPassword = Get-VPASPasswordValue -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        AcctID = "25_13"
        reason = "enter reason here"
...
.OUTPUTS
   If successful:
   Password value of the target account
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("safe","platform","username","address","reason","CopyToClipboard","HideOutput")
                        MandatoryKeys = @("reason")
                    }
					set2 = @{
                        AcceptableKeys = @("AcctID","reason","CopyToClipboard","HideOutput")
            ...
            Write-Verbose "FAILED TO RETRIEVE PASSWORD VALUE"
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUQXMGM+Dm8rIq26rolhio0EaA
# Tu2ggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFC+vo06uQBw9QSsB76hMM9yCPZPl
# MA0GCSqGSIb3DQEBAQUABIICAFC/6JL2tIaEwlmUslk+kdSFzZ/SVihSAqZ0nROW
# pIhovzedIKb6bDd3JhJZ9BqT+7B/UVJ2k0UdYBuJOTu0i+nQsHV00ihVq2QiRkfX
# Yl1KM/ADdx+5qyZltPGLZVH/D9U8/O5ugRsRE+lUxD0dot1myeMJC7dQtyJR3t33
# Ojvu4K32gD+V0cF5eCEpfybjkiG1h7Fn8sJhmFst53geTW5abbkB40eoMvT+s+mz
# IQXe88wZZ1C7u9xLUmdAEyZ28qYmW+vqghEDxR0pknT06/0CijOkkjexLSXsOfGd
# hsLdVhKzILjYwnxITgsPrQha2ZEcnDBd6nbHkZ6i+WQ3w4sb8PX4C5GQ3aapqKBR
# 5DAkg5wS3J+WJGb1AANYJjPzqgEA14nFxcnpD/kdClWT12K+HcNmEq49G1s9Pcni
# kkAWRv2TosaV6hQ8XZVuOtnpD438qowl6EODJTgay4f+NAtvJal+6ZoMKgO0Hxfm
# xdDCCdXbHj7UimBTsPhTIXpTO6i5aJCsg+7sv+h7+OA/KFI115nT/4zB7AzwHLY4
# GMPMovmEnqXNTowomhDo2YndcEQ00uZWIhmWA7iG1PLhwaXzdoqWIg46/T5E4OJq
# 81AXsIIj9yPOs93PAwsUNKdYbbk/E9fDFc1iIW3bIAHLu6WNBjvGwFAZ0JAE7I4F
# yUrDoYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkGA1UEBhMC
...
# 9w0BCQUxDxcNMjYwMjA0MDQ0OTMwWjA/BgkqhkiG9w0BCQQxMgQwtp/iqFMyA58u
# Vj3Cuj1YSNvj0kUms5w4qb2Q7DjX2YkNuJ4eeKtYflj7ErkacW02MA0GCSqGSIb3
# DQEBAQUABIICAE5YgA5PXN0hTXsOmj4eFEGGcq/X54KipbPq+MavAlXlGznHFrtr
# 9RO1Z4Gi9F6VX0SEFx8+3aO+2mPwovfQbp/8a5ePlPXyACbMLbthDkAY8OWlxa5a
# E8CbUhid/pndUgAr18ikeeJCcd+1dMdCucVIpIhXJEv1eCZmqYaf1t4BIDLN7eT2
# 6+vj5YSlfANB8QmJecPpePX9cnhEqwzHiyYvfNsxwRr9ELuyLmZsosq0yA+zuZtv
# i9Gn+6+B3K9Idpw/wWpqDqPm01yJthNMeTx5j78dReIsaJgm8c0vuWd2a+F451W7
# 7KVh/S621TEONTkEXmndc4sSGNgnzfXtjU2pBvRL/PGj4EtVXVZ6TGAeVtk7Haku
# 4Anv8Huo4lGZSrtWFYRLNkOq4/TIQpuAwrPw7uayxNwX8acnHSr4nmaMKoud3/SE
# 388mHnI8WLXCWQUX5kkSHeLTqCECBuO4f7c8zAuy9sUErZ7IRMwPiIExSJ7BBTkG
# nFsCCd83s4PEbS8swooblVG6iuWuZm78wqgagqATiwxKR6BA5rSi+zTZ15+TRTf4
# Hj1cDajzMAM7vplIW7ArV6IyloGsRNs8NCFlC1F/FgtaPYmrlrQsruZCy05Qcam+
# uBIr6t2N0Uw1mKxMIn1SXYIONIR+ZBirr8BvEKbvxSYyEjJCsIs8tOYm
