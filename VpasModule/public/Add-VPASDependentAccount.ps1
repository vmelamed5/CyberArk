...
.DESCRIPTION
   Use this function to create a new dependent (usage) account
.LINK
   https://vpasmodule.com/commands.html#Add-VPASDependentAccount
.NOTES
   Category: PAM
...
.PARAMETER dependentPlatformID
   Dependent account platform that will be assigned to the new dependent account being created
...
.PARAMETER extraProps
   ...
   Pass extra properties in a hashtable following this pattern: @{ Property1Tag = "Property1Value" }
   WinService Example: -extraProps @{ servicename = "WinService01" }
.PARAMETER InputParameters
   HashTable of values containing the parameters required to make the API call
.EXAMPLE
   $CreateDependentAccountJSON = Add-VPASDependentAccount -platformID {PLATFORMID VALUE} -safeName {SAFENAME VALUE} -address {ADDRESS VALUE} -username {USERNAME VALUE} -dependentPlatformID {DEPENDENT PLATFORM ID VALUE} -extraProps @{servicename = "ServiceName"; address = "AddressValue"}
.EXAMPLE
   $CreateDependentAccountJSON = Add-VPASDependentAccount -AcctID {ACCTID VALUE} -dependentPlatformID {DEPENDENT PLATFORM ID VALUE} -extraProps @{servicename = "ServiceName"; address = "AddressValue"}
.EXAMPLE
   $InputParameters = @{
        safe = "TargetSafeName"
        ...
        name = "CustomAccountNameHere"
   }
   $CreateDependentAccountJSON = Add-VPASDependentAccount -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        AcctID = "3_12"
        dependentPlatformID = "TXTFile"
...
.OUTPUTS
   ...
   $false if failed
#>
function Add-VPASDependentAccount{
    ...
    Param(
        ...
        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter platform of new dependent account (for example: TXTFile)")]
        [Parameter(Mandatory=$true,ParameterSetName='Set2',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter platform of new dependent account (for example: TXTFile)")]
        [String]$dependentPlatformID,

        [Parameter(Mandatory=$false,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true)][ValidateSet('TRUE','FALSE')]
        [Parameter(Mandatory=$false,ParameterSetName='Set2',ValueFromPipelineByPropertyName=$true)][ValidateSet('TRUE','FALSE')]
        ...
        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter mandatory and optional properties and values based on target platformId in a key value pair")]
        [Parameter(Mandatory=$true,ParameterSetName='Set2',ValueFromPipelineByPropertyName=$true,HelpMessage="Enter mandatory and optional properties and values based on target platformId in a key value pair")]
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("safe","platform","username","address","dependentPlatformID","automaticManagementEnabled","manualManagementReason","extraProps","name")
                        MandatoryKeys = @("dependentPlatformID","extraProps")
                    }
                    set2 = @{
                        AcceptableKeys = @("AcctID","dependentPlatformID","automaticManagementEnabled","manualManagementReason","extraProps","name")
                        MandatoryKeys = @("AcctID","dependentPlatformID","extraProps")
                    }
                }
            ...
            Write-Verbose "FAILED TO CREATE DEPENDENT ACCOUNT"
        ...
        try{
            ...
            if($ISPSS){
                $params = @{
                    platformId = $dependentPlatformID
                    platformDependentProperties = $extraProps
                }
            }
            else{
                $params = @{
                    platformId = $dependentPlatformID
                    platformAccountProperties = $extraProps
                }
            }
            ...
            if($automaticManagementEnabled -eq "FALSE"){
                $params += @{
                    secretManagement = @{
                        automaticManagementEnabled = $false
                    }
                }
                if(![String]::IsNullOrEmpty($manualManagementReason)){
            ...
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-Verbose "UNABLE TO ADD DEPENDENT ACCOUNT INTO CYBERARK"
            Write-VPASOutput -str $_ -type E
            return $false
        }
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUz8qhbLcDHEM3+dQBo2lf8NL8
# xCiggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYE