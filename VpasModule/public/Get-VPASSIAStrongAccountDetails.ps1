<#
.Synopsis
   Get SIA strong account details
   CREATED BY: Vadim Melamed, EMAIL: vpasmodule@gmail.com
.DESCRIPTION
   Use this function to retrieve strong account details from SIA
.LINK
   https://vpasmodule.com/commands.html#Get-VPASSIAStrongAccountDetails
.NOTES
   Category: SIA
   Tag: Strong Accounts
   Since: v14.2.1
...
.PARAMETER StrongAccountID
   UniqueID of the target strong account in SIA
.PARAMETER StrongAccountName
   Unique name of the target strong account in SIA
...
.EXAMPLE
   $StrongAccountDetails = Get-VPASSIAStrongAccountDetails -StrongAccountID {STRONG ACCOUNT ID VALUE}
.EXAMPLE
   $StrongAccountDetails = Get-VPASSIAStrongAccountDetails -StrongAccountName {STRONG ACCOUNT NAME VALUE}
.EXAMPLE
   $InputParameters = @{
        StrongAccountID = "9284iujwedfh-3456-6788-dggd-flk03945"
        ExportToCSV = $true|$false
        CSVDirectory = "C:\Temp"
   }
   $StrongAccountDetails = Get-VPASSIAStrongAccountDetails -InputParameters $InputParameters
.EXAMPLE
   $InputParameters = @{
        StrongAccountName = "VpasAPIStrongAccount_VpasAPI"
        ExportToCSV = $true|$false
        CSVDirectory = "C:\Temp"
   }
   $StrongAccountDetails = Get-VPASSIAStrongAccountDetails -InputParameters $InputParameters
.OUTPUTS
   ...
   $false if failed
#>
function Get-VPASSIAStrongAccountDetails{
    [OutputType('System.Collections.Hashtable',[bool])]
    [CmdletBinding(DefaultParameterSetName='Set1')]
    Param(

        [Parameter(Mandatory=$true,ParameterSetName='Set1',ValueFromPipelineByPropertyName=$true,HelpMessage="Unique StrongAccountID of the target strong account (for example: 9284iujwedfh-3456-6788-dggd-flk03945)")]
        [String]$StrongAccountID,

        [Parameter(Mandatory=$true,ParameterSetName='Set2',ValueFromPipelineByPropertyName=$true,HelpMessage="Name of the target strong account (for example: VpasAPIStrongAccount2)")]
    ...
    Process{
        try{
            if($PSCmdlet.ParameterSetName -eq "InputParameters"){
                $KeyHash = @{
                    set1 = @{
                        AcceptableKeys = @("StrongAccountID","ExportToCSV","CSVDirectory")
                        MandatoryKeys = @("StrongAccountID")
                    }
					set2 = @{
                        AcceptableKeys = @("StrongAccountName","ExportToCSV","CSVDirectory")
            ...
            Write-Verbose "FAILED TO RETRIEVE STRONG ACCOUNT DETAILS"
        ...
        try{
            ...
            if([String]::IsNullOrEmpty($StrongAccountID)){
                ...
                $StrongAccountID = Get-VPASSIAStrongAccountIDHelper -token $token -SearchQuery $StrongAccountName
            ...
            $log = Write-VPASTextRecorder -inputval $_ -token $token -LogType ERROR
            $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC
            Write-Verbose "FAILED TO RETRIEVE SIA STRONG ACCOUNT DETAILS"
            Write-VPASOutput -str $_ -type E
            return $false
        }
    }
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUYLg2AYPzictgcPcTnUJz4Fdc
# bfWggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFDmV0cAoCYCpqV/9ZJwI0J86ZiCU
# MA0GCSqGSIb3DQEBAQUABIICAG2GEwnYlp7bwVKuy/7ec0omez9Yu2qbdMUCiaTb
# /0pjRvFC/ueiVIY3FboVqyH/RmP6qhOJc8bDIlN9oB0fJmZ7d1tc59JDxUrSdLWz
# RnyGZrlXSMHdEbLLR+VnvLKmxfyMhguJSsBGZCnCWM229BqGEw3U+ZrwKiDvsHQU
# 6NO8KnME0hRZJUZVqPrxmR54T5hQD/2GjeKtvu5/p9jXKs2drW9W9wiKO6bR3Ys9
