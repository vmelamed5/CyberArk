        ...
        try{
            ...
            if(!$ISPSS -and ($PVWA -notmatch ".privilegecloud.cyberark.")){
                ...
                if($output.count -gt 1){
                    Write-Verbose "MULTIPLE DISCOVERED ACCOUNT ENTRIES WERE RETURNED, ADD MORE SEARCH FIELDS TO NARROW RESULTS"
                    $log = Write-VPASTextRecorder -inputval "MULTIPLE DISCOVERED ACCOUNT ENTRIES WERE RETURNED, ADD MORE SEARCH FIELDS TO NARROW RESULTS" -token $token -LogType MISC -Helper
                    $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: -1" -token $token -LogType MISC -Helper
                    return -1
                }
                elseif($output.count -eq 0){
                    Write-Verbose "NO DISCOVERED ACCOUNTS FOUND"
                    Write-VPASOutput -str "NO DISCOVERED ACCOUNTS FOUND" -type E
                    $log = Write-VPASTextRecorder -inputval "NO DISCOVERED ACCOUNTS FOUND" -token $token -LogType MISC -Helper
                    $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: -2" -token $token -LogType MISC -Helper
                    return -2
                }
                else{
                    write-verbose "FOUND UNIQUE DISCOVERED ACCOUNT ID"
                    Write-Verbose "RETURNING UNIQUE DISCOVERED ACCOUNT ID"
                    $logoutput = $response.items
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUkiB6dGkz3DGN9d5AWLi31Q11
# biiggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFL4IhAtgDC0tigijxe6F8Apqgxcb
# MA0GCSqGSIb3DQEBAQUABIICAHTpJmlr0QOaluJvPOrO9Ql+zLRzsLoowIgJ9LsL
# ZtoRDUJYZIRlLsBR/eXyQbOykL3cymklheTzMkHll/Ic0vZ+VBsnRtoifpcFVWH3
# Deem2pkyiCGcvtc4KWqJPk3DxYZ9k1xdykHN+nHIo8Nb3Guw7GqpkTPiB9wlOUZA
# W6L7xxtggryJxbi1DyNC6JCXIFTdAx0rIz/q1GIbUZRNSskP3SNzr3T37ig5CaUZ
# CL555WsI9pUX9FwurLPjCbWCkbbLnZffPP8JCdUJEHEcQwp3484Blg1vfd7Hp/Vl
# CeWzikJpdSDlgzrjGzvJX4idEdItZowdqBF0/Ids5Mwz1A8EhG76QGnerNYN1jqF
# ap+v6YHe7YSuRWae0A9+RwXZTXh4l1K7VbTHG+lz0FCG9ZUiS78r1kqezYRPC22Q
# EOYkClDRnhQpLHllgpLJmsriMreVPrqDIYJdC8cHnOIeHwLtXTzB/JPRiBReP7T7
# AO83s0vM7L4ujisoRQrz/18UCzmsYP8gAyU/MitBQacHuWfOcQ2iXZp/MTmcWpxV
# hW1bX+irzuCDYZxHBN6SQUb71e3VOR2sb+0aB+6NGKcgnM7zzjFL4TGu0emYjJRB
# v7vR9NTwg4CoHWlK/oR4DGJCI+WStc5/rOFiVsqxtqi2NsLR9NMArGJSjQ8Zdyne
# HjNKoYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkGA1UEBhMC
...
# 9w0BCQUxDxcNMjYwMjA0MDQ0MTIzWjA/BgkqhkiG9w0BCQQxMgQwSP08o7J/AD9E
# nIgNP4knvoQaAzEHJt282brU83e/+gAAcK/BFsLD5uRzHmTx71i0MA0GCSqGSIb3
# DQEBAQUABIICAIQ2WDbusQeFILRZ1d5bQErj6jNqBsXclUCS1xj0kV6EcCur8cS/
# no/UY1VVTqqqayCSkNFNDfTTmUtEECCDcKBd+dgAFvhS7kVVyYytzirIKuOcrdVb
# C4tmF18O/y7aXgapZl1ejSHekBImZ4HLb3JlZdMcPw9mLY7cMRU+VFLff1TWNM82
# od274FU7MhDtfb9hAQBDhDc0INvclNOdYHLGKTIPWuvedIPT9/5RylpPuks9176I
# Q6GWxG7p1At7TD6pO3a8dGIbdP5mlkFcbQ2qSuy6nXdsJQwEbtfY7RoFlMg6De0G
# FN9peSfTrpBJYnoJsD5akSfMVXA6vMGavgrmBirS9GPpFH0XEfkvsFO+slzjxbR2
# BXbLnGn91gyQY4f7tGAgp54oS3/8LONbGUT8hdNvx9ZRBWh3xu6ABYk97TzGOVP5
# jUMQKmOWtKw88/FGQjYbUJfy4gtb82KgTvXdn/Cagjcf9poo/nf1Uju5yCFcw89N
# GcRO+6jpm5jXFiR5HuNe3D8iRcpNwlGgOghz60WODuHJDW2nCPFKkOpYGF09ZFr6
# pF8ESzcbfuRg5mChoGSBEU9rp/AGLGOOJew1tMTTp5MvZ6enV9uwP2pmVucmjY1j
# j4zytWnVEZ0MTZ2SeOrUmZn0Qom5ZeJqulqykGyhJm0Uww8lNFEwM4MT
