...
.DESCRIPTION
   Helper function to retrieve application IDs from Identity
#>
function Get-VPASApplicationIDIdentityHelper{
    ...
    Process{
        $log = Write-VPASTextRecorder -inputval "LOOKING FOR *$ApplicationName* IDENTITY APPLICATION" -token $token -LogType MISC -Helper
        try{
            if(!$IdentityURL){
            ...
            $counter = 0
            $returnID = ""
            foreach($app in $result.Result.Results.Row){
                $RECappName = $app.Name
                $RECappID = $app.ID

                if($RECappName -match $ApplicationName){
                    $counter += 1
                    $returnID = $RECappID
                }

                if($RECappName -eq $ApplicationName){
                    Write-Verbose "FOUND TARGET APPLICATION, RETURNING UNIQUE ID"
                    $log = Write-VPASTextRecorder -inputval $app -token $token -LogType RETURN -Helper
                    return $RECappID
                }
            }

            if($counter -gt 1){
                Write-Verbose "MULTIPLE APPLICATION ENTRIES WERE RETURNED, ADD MORE TO NAME TO NARROW RESULTS"
                $log = Write-VPASTextRecorder -inputval "MULTIPLE APPLICATION ENTRIES WERE RETURNED, ADD MORE TO NAME TO NARROW RESULTS" -token $token -LogType MISC -Helper
                $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC -Helper
                return -1
            }
            elseif($counter -eq 0){
                Write-Verbose "NO APPLICATIONS FOUND"
                Write-VPASOutput -str "NO APPLICATIONS FOUND" -type E
                $log = Write-VPASTextRecorder -inputval "NO APPLICATIONS FOUND" -token $token -LogType MISC -Helper
                $log = Write-VPASTextRecorder -inputval "REST API COMMAND RETURNED: FALSE" -token $token -LogType MISC -Helper
                return -2
            }
            else{
                write-verbose "FOUND UNIQUE APPLICATION ID"
                Write-Verbose "RETURNING UNIQUE APPLICATION ID"
                $log = Write-VPASTextRecorder -inputval "FOUND TARGET UNIQUE APPLICATION ID $returnID" -token $token -LogType MISC -Helper
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUM10WVnepoLFSDErfYg8PCPyM
# ZmaggiTgMIIFbzCCBFegAwIBAgIQSPyTtGBVlI02p8mKidaUFjANBgkqhkiG9w0B
...
# CisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFAXBXToCzGORI4y/2jY5J8yraeqL
# MA0GCSqGSIb3DQEBAQUABIICAAaR9LodteKVPss5uCe2s2W6KYct9n5tvqXVpofK
# jdwMx8YtRmbehaonH6cZ8NiV3xBjfmckwfv8PkqUc7o4kutvSDldft5qU6RX2Zdy
# l5XxBiCe86IAsIXM8wJvH10ZCtMUi/2/X9Vm75IC11VSvt7rbT44WabwfJ7oCYu7
# 21an9y85uI6yuWA9Gl9iNLCJaY6xF7uMGGBIRs48KxKTaaR6w0JvwaO6xakX9epY
# byS0wgXJcJnvF8kpGkFOY3PtHP8i2Uf4mhSA025bBChBWBSzrBPQb7QUyguf7n+g
