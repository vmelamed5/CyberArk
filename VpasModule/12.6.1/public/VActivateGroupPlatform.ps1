...
function VActivateGroupPlatform{
    ...
    try{
        ...
        if($NoSSL){
            ...
            $platID = VGetGroupPlatformIDHelper -token $token -groupplatformID $ActivateGroupPlatformID -NoSSL
        }
        else{
            Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
            $platID = VGetGroupPlatformIDHelper -token $token -groupplatformID $ActivateGroupPlatformID
        }
