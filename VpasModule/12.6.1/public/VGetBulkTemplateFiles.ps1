...
.OUTPUTS
   ...
   $false if failed
#>
function VGetBulkTemplateFiles{
    [CmdletBinding()]
    Param(
    ...
    try{
        ...
        if([String]::IsNullOrEmpty($OutputDirectory)){
            ...
            if(Test-Path -Path $OutputDirectory){
                ...
                vout -str "$OutputDirectory DOES NOT EXIST...PLEASE CONFIRM OUTPUT DIRECTORY OR LEAVE AS DEFAULT AND TRY AGAIN" -type E
                vout -str "EXITING UTILITY" -type E
                return $false
            }
        }
        ...
        elseif($BulkTemplate -eq "BulkSafeMembers"){
            ...
            $logstr4 = "ExampleSafe1,Vault Admins,Vault,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE,TRUE"
        ...
        vout -str "$targetFile WAS CREATED" -type C
        return $true     
    }catch{
        Write-Verbose "FAILED TO CREATE TEMPLATE FILE"
        Vout -str "FAILED TO CREATE TEAMPLATE FILE: $targetFile" -type E
        vout -str "$_" -type E
        return $false
    }
}
