    ...
    try{
        ...
        if($ReportType -eq "SafeContent"){
            ...
            if($SearchQuery -eq "all"){
                ...
                if([String]::IsNullOrEmpty($Limit)){
                    write-host "NO LIMIT SUPPLIED, DEFAULT SAFE LIMIT IS 25. ENTER UPPER LIMIT AMOUNT OF SAFES: " -ForegroundColor Yellow -NoNewline
                    $Limit = Read-Host
                }

                if($NoSSL){
                    $Safes =  VGetSafes -token $token -searchQuery " " -NoSSL -limit $Limit
                }
                else{
                    $Safes =  VGetSafes -token $token -searchQuery " " -limit $Limit
                }
                ...
                if($NoSSL){
                    if($WildCardSearch){
                        $Safes = VGetSafes -token $token -searchQuery $SearchQuery -NoSSL -limit $Limit
                    }
                    else{
                        $Safes = VGetSafeDetails -token $token -safe $SearchQuery -NoSSL
                    }
                    ...
                    if($WildCardSearch){
                        $Safes = VGetSafes -token $token -searchQuery $SearchQuery -limit $Limit
                    }
                    else{
                        $Safes = VGetSafeDetails -token $token -safe $SearchQuery 
                    }
            ...
            foreach($safe in $TargetSafes){
                if($NoSSL){
                    $FoundAccounts = VGetAccountDetails -token $token -safe $safe -NoSSL -HideWarnings
                }
                else{
                    $FoundAccounts = VGetAccountDetails -token $token -safe $safe -HideWarnings
                }

                if(!$FoundAccounts){
                    if(!$HideOutput){ Vout -str "NO ACCOUNTS FOUND IN SAFE: $safe" -type M }
            ...
            if($ReportFormat -eq "JSON" -or $ReportFormat -eq "ALL"){
                ...
                if(!$HideOutput){ Vout -str "FINISHED EXPORTING JSON FILE: $targetFile" -type C }
            ...
            if($ReportFormat -eq "TXT" -or $ReportFormat -eq "ALL"){
                ...
                if(!$HideOutput){ Vout -str "FINISHED EXPORTING TXT FILE: $targetFile" -type C }
            ...
            if($ReportFormat -eq "CSV" -or $ReportFormat -eq "ALL"){
                ...
                if(!$HideOutput){ Vout -str "FINISHED EXPORTING CSV FILE: $targetFile" -type C }
            ...
            if($ReportFormat -eq "HTML" -or $ReportFormat -eq "ALL"){
                ...
                if(!$HideOutput){ Vout -str "FINISHED EXPORTING HTML FILE: $targetFile" -type C }
            ...
            if($ReportFormat -eq "XML" -or $ReportFormat -eq "ALL"){
                ...
                if(!$HideOutput){ Vout -str "FINISHED EXPORTING XML FILE: $targetFile" -type C }
            ...
            foreach($safe in $TargetSafes){
                if($NoSSL){
                    if($IncludePredefinedSafeMembers){
                        $FoundMembers = VGetSafeMembers -token $token -safe $safe -IncludePredefinedMembers -NoSSL
                    }
                    else{
                        $FoundMembers = VGetSafeMembers -token $token -safe $safe -NoSSL
                    }
                }
                else{
                    if($IncludePredefinedSafeMembers){
                        $FoundMembers = VGetSafeMembers -token $token -safe $safe -IncludePredefinedMembers
                    }
                    else{
                        $FoundMembers = VGetSafeMembers -token $token -safe $safe
                    }
                }

                if(!$FoundMembers){
                    if(!$HideOutput){ Vout -str "NO SAFE MEMBERS FOUND IN SAFE: $safe" -type M }
        ...
        if($ReportType -eq "PlatformDetails"){
            ...
            if($SearchQuery -eq "all"){
                ...
                if($NoSSL){
                    if($WildCardSearch){
                        $AllPlatforms = VGetPlatformDetailsSearch -token $token -SearchQuery "$SearchQuery" -NoSSL
                    ...
                    if($WildCardSearch){
                        $AllPlatforms = VGetPlatformDetailsSearch -token $token -SearchQuery "$SearchQuery"
                        $AllPlatforms = $AllPlatforms | ConvertTo-Json
                        $AllPlatforms = $AllPlatforms | ConvertFrom-Json
                    }
                    else{
            ...
            foreach($platform in $AllPlatforms){
                ...
                if($NoSSL){
                    $response2 = VGetPSMSettingsByPlatformID -token $token -PlatformID $PFPlatformID -NoSSL
                }
                else{
                    $response2 = VGetPSMSettingsByPlatformID -token $token -PlatformID $PFPlatformID
                }
