...
function VDuplicateGroupPlatform{
    ...
    try{
        ...
        if($NoSSL){
            ...
            $platID = VGetGroupPlatformIDHelper -PVWA $PVWA -token $token -groupplatformID $DuplicateFromGroupPlatformID -NoSSL
        }
        else{
            Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
            $platID = VGetGroupPlatformIDHelper -PVWA $PVWA -token $token -groupplatformID $DuplicateFromGroupPlatformID
        }
