...
.OUTPUTS
   JSON Object (CPResults) if successful
   $false if failed
#>
function Invoke-VPASCredentialProvider{
    ...
    Process{
        try{
            ...
            Write-Verbose "$passwordSDK GetPassword /p AppDescs.AppID=$ApplicationID /p Query=`"Safe=$Safe;Folder=$Folder;Object=$ObjectName`" /o passprops.username"
            $username = & $passwordSDK GetPassword /p AppDescs.AppID=$ApplicationID /p Query="Safe=$Safe;Folder=$Folder;Object=$ObjectName" /o passprops.username
