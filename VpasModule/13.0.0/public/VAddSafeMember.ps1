...
function VAddSafeMember{
    ...
    Param(
        ...
        [Parameter(Mandatory=$true,ValueFromPipelineByPropertyName=$true,Position=2)][String]$searchin,
        ...
        [Parameter(Mandatory=$false,ValueFromPipelineByPropertyName=$true,Position=32)][ValidateSet("User","Group","Role")][String]$MemberType,
    ...
    if($ISPSS){
        if([String]::IsNullOrEmpty($MemberType)){
            Write-Host "ENTER MEMBER TYPE (User, Group, Role): " -ForegroundColor Yellow -NoNewline
