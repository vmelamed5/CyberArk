...
function VDeleteApplicationAuthentication{
    ...
    if([String]::IsNullOrEmpty($AuthID)){
        ...
        if($NoSSL){
            $AuthID = VGetApplicationAuthIDHelper -token $token -AppID $AppID -AuthType $AuthType -AuthValue $AuthValue -NoSSL
        }
        else{
            $AuthID = VGetApplicationAuthIDHelper -token $token -AppID $AppID -AuthType $AuthType -AuthValue $AuthValue
        }
