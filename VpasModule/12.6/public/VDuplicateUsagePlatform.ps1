...
function VDuplicateUsagePlatform{
    ...
    try{
        ...
        if($NoSSL){
            ...
            $platID = VGetUsagePlatformIDHelper -token $token -usageplatformID $DuplicateFromUsagePlatformID -NoSSL
        }
        else{
            Write-Verbose "SSL ENABLED BY DEFAULT, USING HTTPS"
            $platID = VGetUsagePlatformIDHelper -token $token -usageplatformID $DuplicateFromUsagePlatformID
        }
