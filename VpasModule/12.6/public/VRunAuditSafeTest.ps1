...
function VRunAuditSafeTest{
    ...
    if($NoSSL){
        $AllSafes = VGetSafes -token $token -searchQuery "$AuditSafeNameConvention" -limit 5000 -NoSSL
    }
    else{
        $AllSafes = VGetSafes -token $token -searchQuery "$AuditSafeNameConvention" -limit 5000
    }
        ...
        foreach($saferes in $AllSafes.value){ 
            ...
            if($AuditIgnoreSafes.Contains($safe)){
                ...
                if($AuditNumberOfSafeMembers -eq "0"){
                    ...
                    if($NoSSL){
                        $AllSafemembers = VGetSafeMembers -token $token -safe $safe -IncludePredefinedMembers -NoSSL
                    }
                    else{
                        $AllSafemembers = VGetSafeMembers -token $token -safe $safe -IncludePredefinedMembers
                    }
