...
function Remove-VPASAllDiscoveredAccounts{
    ...
    process{
        $log = Write-VPASTextRecorder -inputval "Remove-VPASAllDiscoveredAccounts" -token $token -LogType COMMAND
        ...
        try{
            ...
            if(!$WhatIf){
                if($Confirm){
                    ...
                    $log = Write-VPASTextRecorder -inputval "Remove-VPASAllDiscoveredAccounts" -token $token -LogType DIVIDER
            ...
            elseif($WhatIf){
                ...
                if($NoSSL){
                    $WhatIfInfo = Get-VPASDiscoveredAccounts -token $token -SearchQuery " " -NoSSL
                }
                else{
