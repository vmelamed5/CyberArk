...
function Get-VPASEPVUserIDHelper{
    ...
    Process{
        $log = Write-VPASTextRecorder -inputval "Get-VPASEPVUserIDHelper" -token $token -LogType COMMAND -Helper
        try{
            ...
            if($counter -gt 1){
                ...
                foreach($rec in $response.Users){
                    ...
                    if($recusername -eq $username){
                        ...
                        $log = Write-VPASTextRecorder -inputval "Get-VPASEPVUserIDHelper" -token $token -LogType DIVIDER -Helper
                        return $output
                    }
