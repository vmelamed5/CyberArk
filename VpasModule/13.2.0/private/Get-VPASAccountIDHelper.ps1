...
function Get-VPASAccountIDHelper{
    ...
    Process{
        try{
            $log = Write-VPASTextRecorder -inputval "Get-VPASAccountIDHelper" -token $token -LogType COMMAND -Helper
            ...
            if($counter -gt 1){
                ...
                $log = Write-VPASTextRecorder -inputval "Get-VPASAccountIDHelper" -token $token -LogType DIVIDER -Helper
                return -1
            }
            elseif($counter -eq 0){
