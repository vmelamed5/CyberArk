...
function Get-VPASActiveSessionIDHelper{
    ...
    Process{
        $log = Write-VPASTextRecorder -inputval "Get-VPASActiveSessionIDHelper" -token $token -LogType COMMAND -Helper
            ...
            foreach($rec in $response.LiveSessions){
                ...
                if($recSessionID -eq $SearchQuery -or $recUser -eq $SearchQuery -or $recTargetAcct -eq $SearchQuery -or $recTargetAddr -match $SearchQuery){
                    ...
                    if($output -eq -1){
                        ...
                        $log = Write-VPASTextRecorder -inputval "Get-VPASActiveSessionIDHelper" -token $token -LogType DIVIDER -Helper
                        $output = -2
                        return $output
                    }
                }
