...
function Get-VPASPlatformIDHelper{
    ...
    Process{
        $log = Write-VPASTextRecorder -inputval "Get-VPASPlatformIDHelper" -token $token -LogType COMMAND -Helper
        try{
            ...
            foreach($rec in $response.Platforms){
                ...
                if($recplatformid -eq $platformID -or $recname -eq $platformID){
                    ...
                    $log = Write-VPASTextRecorder -inputval "Get-VPASPlatformIDHelper" -token $token -LogType DIVIDER -Helper
                    return $output
                }
