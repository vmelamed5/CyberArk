...
function Get-VPASDiscoveredAccountIDHelper{
    ...
    Process{
        $log = Write-VPASTextRecorder -inputval "Get-VPASDiscoveredAccountIDHelper" -token $token -LogType COMMAND -Helper
        try{
            ...
            if($output -eq -1){
                ...
                $log = Write-VPASTextRecorder -inputval "Get-VPASDiscoveredAccountIDHelper" -token $token -LogType DIVIDER -Helper
                return $output
            }
            else{
