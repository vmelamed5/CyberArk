...
function Get-VPASAccountGroupIDHelper{
    ...
    Process{
        try{
            $log = Write-VPASTextRecorder -inputval "Get-VPASAccountGroupIDHelper" -token $token -LogType COMMAND -Helper
            ...
            foreach($rec in $response){
                ...
                if($recgroupName -eq $GroupName){
                    ...
                    $log = Write-VPASTextRecorder -inputval "Get-VPASAccountGroupIDHelper" -token $token -LogType DIVIDER -Helper
                    return $recgroupID
                }
                else{
